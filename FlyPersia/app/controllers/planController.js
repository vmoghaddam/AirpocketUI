'use strict';
app.controller('planController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', '$q', '$http', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route, $q, $http) {
    ...
    $scope.btn_delete = {
        ...
        onClick: function (e) {
            $scope.dg_leg_selected = $rootScope.getSelectedRow($scope.dg_leg_instance);
            if (!$scope.dg_leg_selected) {
            ...
            General.Confirm(Config.Text_DeleteConfirm, function (res) {
                if (res) {
                    ...
                    var dto = { Id: $scope.dg_leg_selected.Id, };
    ...
    $scope.btn_edit = {
        ...
        onClick: function (e) {
            ...
            $scope.loadingVisible = true;
            flightService.getFlightPlanItem($scope.dg_leg_selected.Id, offset).then(function (response) {
                $scope.loadingVisible = false;
                $scope.IsNew = false;
    ...
    $scope.btn_approve = {
        ...
        onClick: function (e) {
            ...
            $scope.loadingVisible = true;
            flightService.checkPlanErrors($scope.dg_selected.Id).then(function (response) {
                $scope.loadingVisible = false;
    ...
    $scope.getDayId = function (str) {
        var item = Enumerable.From(dayDs).Where('$.Title=="' + str + '"').FirstOrDefault();
        if (item)
            return item.Id;
    };
    $scope.getMonthId = function (str) {
        var item = Enumerable.From(monthDs).Where('$.Title=="' + str + '"').FirstOrDefault();
        if (item)
            return item.Id;
    };
    ...
    $scope.btn_interval = {
        ...
        onClick: function (e) {
            ...
            $scope.entityInterval.FlightPlanId = $scope.dg_selected.Id;
            ...
            $scope.entityInterval.Register = $scope.dg_selected.VirtualRegister;
            ...
            if ($scope.dg_selected.Customer) {
                var prts = $scope.dg_selected.Customer.split('-');
                var monthsStr = prts[0].split(',');
                monthsStr = Enumerable.From(monthsStr).Where('$.trim()').ToArray();
               
                
                if (monthsStr && monthsStr.length>0) {
                    
                    $scope.entityInterval.Months = Enumerable.From(monthsStr).Select(function (x) { return $scope.getMonthId(x); }).ToArray();
                    
                }
                var daysStr = prts[1].split(',');
                daysStr = Enumerable.From(daysStr).Where('$.trim()').ToArray();
                if (daysStr && daysStr.length>0) {
                    $scope.entityInterval.Days = Enumerable.From(daysStr).Select(function (x) { return $scope.getDayId(x); }).ToArray();
                }
    ...
    $scope.dg_columns = [
        ...
        //    fixed: true, fixedPosition: 'left', alignment: 'center'
        //},
        //{ dataField: 'Id', caption: 'No', allowResizing: true, dataType: 'number', allowEditing: false, width: 90, alignment: 'center', fixed: true, fixedPosition: 'left' },
          { dataField: 'VirtualRegister', caption: 'Line', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, fixed: false, fixedPosition: 'left', width: 90 },
        { dataField: 'DateFrom', caption: 'From', allowResizing: true, dataType: 'date', allowEditing: false, width: 130, alignment: 'center', sortIndex: 0, sortOrder: "desc", fixed: false, fixedPosition: 'right' },
        ...
        { dataField: 'BaseName', caption: 'Fights', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, fixed: false, fixedPosition: 'left', width: 300 },
        { dataField: 'Customer', caption: 'Days', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, fixed: false, fixedPosition: 'left', width: 250 },
         { dataField: 'Types', caption: 'Types', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, fixed: false, fixedPosition: 'left', width: 110, sortIndex: 1, sortOrder: "desc" },
        ...
        //{ dataField: 'TotalFlights', caption: 'Flights', allowResizing: true, dataType: 'number', allowEditing: false, width: 100, alignment: 'center' },
          ...
          { dataField: 'IntervalType', caption: 'Interval', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, fixed: false, fixedPosition: 'left', width: 90 },
    ...
    $scope.getPlans = function () {
        ...
        $http.get(serviceBase + 'odata/flightplans/approved/customer/' + Config.CustomerId + '/' + -1).then(function (response) {
    ...
    $scope.bind = function () {
        $scope.loadingVisible = true;
        $scope.getPlans().then(function (response) {
            $scope.loadingVisible = false;
