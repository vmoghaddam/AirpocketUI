'use strict';
app.controller('ceoFlightBoardController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
    ...
    function printCanvas(canvas) {
        ...
        frameDoc.document.write("<div style='height:100%'><div style='text-align:center;margin-top:30px'>Fly Persia</div><div style='text-align:center;margin-top:10px'>Flights</div><img style='margin-top:10px;height:75%'  src = '" + canvas.toDataURL() + "'/></div>");
        frameDoc.document.write('</body></html>');
        frameDoc.document.close();
        setTimeout(function () {
            window.frames["frame1"].focus();
    ...
    $scope.popup_date = {
        ...
        onShown: function (e) {
            ...
            pd1.setDate(new Date($scope.datefrom.getTime()));
    ...
    $scope.bindFlights = function (saveState) {
        ...
        flightService.getFlightsGantt(Config.CustomerId, (new Date($scope.datefrom)).toUTCDateTimeDigits(), ed, offset, ($scope.IsAdmin ? null : $scope.airportEntity.Id), filter).then(function (response) {
          ...
          //  console.log($scope.ganttData);
            ...
            if (IsIOS()) {
                $.each(response.flights, function (_j, _flt) {
                    _flt.STD = (new Date(_flt.STD)).addMinutes(-270);
                    _flt.STA = (new Date(_flt.STA)).addMinutes(-270);
                    _flt.ChocksIn = (new Date(_flt.ChocksIn)).addMinutes(-270);
                    _flt.ChocksOut = (new Date(_flt.ChocksOut)).addMinutes(-270);
                    _flt.Landing = (new Date(_flt.Landing)).addMinutes(-270);
                    _flt.STAPlanned = (new Date(_flt.STAPlanned)).addMinutes(-270);
                    _flt.STDPlanned = (new Date(_flt.STDPlanned)).addMinutes(-270);
                    _flt.Takeoff = (new Date(_flt.Takeoff)).addMinutes(-270);
                    _flt.baseStartDate = (new Date(_flt.baseStartDate)).addMinutes(-270);
                    _flt.baseEndDate = (new Date(_flt.baseEndDate)).addMinutes(-270);
                    _flt.endDate = (new Date(_flt.endDate)).addMinutes(-270);

                    _flt.startDate = (new Date(_flt.startDate)).addMinutes(-270);
    ...
    $scope.sb_delay = {
        ...
        bindingOptions: {
            ...
            //selectedItem:'selectedDelayItem',
    ...
    console.log($rootScope.detector.tablet());
    console.log($rootScope.detector.mobile());
    $scope.timeCellWidth = 0;
    $scope.selectedDate = null;
    //alert($(window).width());
   // alert($(window).height());
    //ati-2-17
    $scope.scaleFactor = 0.24;
    //alert($rootScope.detector.tablet());
    if ($rootScope.detector.tablet()) {
        $scope.scaleFactor = $scope.scaleFactor * ($(window).width()>960?1.4:2);
    ...
    $scope.createGantt = function (_scale) {
        ...
        var _headerHeight = 81;
        var _detector = new MobileDetect(window.navigator.userAgent);
        if (_detector.mobile())
            _headerHeight = 55;
        //alert(_headerHeight);
       ...
       // alert($(window).width());
        //if (!_scale)
        var taskTemplate = '#taskbarTemplateLightNewCeo';
        var spliterPos = 110;
        var _rowHeight = 57;
            //$scope.scaleFactor = 2;
        ...
        //alert(_ww);
        if (_ww <= 1024) {
            taskTemplate = '#taskbarTemplateLightNewCeoNarrow';
            _rowHeight = 40;
        }
            if (_ww > 1380) {
                $scope.scaleFactor = 0.24;
            ...
            else if (_ww > 640 && _ww <= 1380) {
                $scope.scaleFactor = 0.40;
                spliterPos = 110;
            }
            else if (_ww <= 360) {
                $scope.scaleFactor = 1;
            ...
            else if (_ww>360 && _ww <= 640) {
                $scope.scaleFactor =0.65;
                spliterPos = 70;
            }
          //  alert($scope.scaleFactor);
            _scale = $(window).width() * $scope.scaleFactor;
     ...
     //   alert($(window).width());
        ...
        h = $(window).height() - $rootScope.headerHeight - (50)-(36)-tilebar-8;
        h = h + 'px';
        if ($scope.IsMobile && $scope.IsLandscape) {
            h = $(window).height();
        }
        ...
        $("#resourceGanttba").ejGantt({
            ...
            load: function () {
               // console.log('load');
                ...
                this.getColumns()[0].width = spliterPos;
                //  console.log('load1');
                var customColumn = {
                ...
                try {
                   ...
                   // var scrollnextday = true;
                    ...
                    else
                    {
                        ...
                        var n = d.getHours();
                        //alert(n);
                        var _cw = ($('.e-schedule-day-headercell-content').width());
                        $('.e-ganttviewerbodyContianer').data("ejScroller").scrollX(_cw * n);
                    }
            ...
            taskbarTemplate:taskTemplate, //"#taskbarTemplateLightNewCeo",
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        $('.ceoflightboard').fadeIn(400, function () {
            if ($scope.airport) {
                $scope.loadingVisible = true;
    ...
    appWindow.bind('resize', function () {
        ...
        $scope.createGantt();
        console.log('Resized your browser ' + $(window).width() + '  ' + $(window).height());
    });
