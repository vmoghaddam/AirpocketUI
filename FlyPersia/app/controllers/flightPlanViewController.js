...
app.controller('flightPlanViewController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route) {
    ...
    $scope.dg = {
        ...
        onSelectionChanged: function (e) {
            ...
            if (!data) {
                ...
                if (!data.permits) {
                    $scope.loadingVisible = true;
                    flightService.getFlightPlanItemPermits(data.Id).then(function (response) {
                        $scope.loadingVisible = false;
                        if (response)
    ...
    $scope.bind = function () {
      //  var url = 'odata/flightplans/approved/customer/' + Config.CustomerId + '/' + $scope.typeId;
        ...
        var url = 'odata/plan/items/approved/' + Config.CustomerId + '/' + $scope.typeId;
    ...
    if (!authService.isAuthorized()) {
        ...
        $('.flightplanview').fadeIn(400, function () {
            //vmins = new viewModel();
