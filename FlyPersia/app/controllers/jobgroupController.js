'use strict';
app.controller('jobgroupController', ['$scope', '$location', '$routeParams', '$rootScope', 'jobgroupService', 'personService', 'authService','$http', function ($scope, $location, $routeParams, $rootScope, jobgroupService, personService, authService,$http) {
    ...
    $scope.btn_man = {
        ...
        onClick: function (e) {
            ...
            $scope._selected = $rootScope.getSelectedRows($scope.dg_instance);
            if (!$scope._selected) {
                General.ShowNotify(Config.Text_NoRowSelected, 'error');
                return;
            }
	...
	{ dataField: 'InActive', caption: 'DeActive', allowResizing: true, alignment: 'left', dataType: 'boolean', allowEditing: false, width: 130, fixed: true, fixedPosition: 'left', sortIndex: 0, sortOrder: "asc" },
        { dataField: 'JobGroup', caption: 'Group', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, width: 250, fixed: true, fixedPosition: 'left',  },
    ...
    $scope.dg_columns = [
        ...
        { dataField: 'ManagerTitle', caption: 'Manager', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, encodeHtml: false, width: 200  },
    ];
    ...
    $scope.tree = {
        ...
        onSelectionChanged: function (e) {
            var data = e.selectedRowsData[e.selectedRowsData.length-1];
            ...
            if (!data) {
                ...
                personService.getEmployeesByGroupCode(data.FullCode2).then(function (response) {
    ...
    $scope.bind = function () {
        ...
        $http.get('https://fleet.flypersia.aero/apitrn/' + 'api/jobgroup/' ).then(function (results) {
    ...
    $scope.popup_password.toolbarItems[0].options.onClick = function (e) {
        ...
        var dto = { managerId: $scope.manager, ids: Enumerable.From($scope._selected).Select('$.Id').ToArray().join('_') }

        $scope.loadingVisible = true;
        $http.post('https://fleet.flypersia.aero/apitrn/' + 'api/groups/manager/save', dto).then(function (response) {
            $scope.loadingVisible = false;
            $scope.bind();
            $scope.popup_password_visible = false;
        }, function (err, status) {
