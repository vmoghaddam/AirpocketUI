...
app.controller('reportEFBController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', 'flightBagService', '$sce', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route, flightBagService, $sce) {
    ...
    $scope.btn_jl = {
        ...
        onClick: function (e) {
			...
			//https://fleet.flypersia.aero/zapiapsb/api/jl/v3/xls/40451
             var _url = 'https://fleet.flypersia.aero/zapiapsb/api/jl/v3/xls/' + flt.FlightId;
                     // window.open(_url);
					 setTimeout(() => {
...
$scope.jlObj = null;
    ...
    function printElem($elem,ofp) {
        ...
        frameDoc.document.write('</head><body style="padding:0px">');
				...
				$scope.props = response.props;
				 ...
				 $.each($scope.props, function (_i, _d) {
                                                ...
                                                if (_d.PropName == 'prop_pax_adult' /*&& $scope.flight.PaxAdult != _d.PropValue*/) {
												 ...
												 if (_d.PropName == 'prop_pax_male' /*&& $scope.flight.PaxAdult != _d.PropValue*/) {
                                                    $('#' + _d.PropName).val($scope.selectedFlight.PaxMale);
                                                    updates.push({ OFPId: $scope.entity.Id, PropName: _d.PropName, User: $rootScope.userTitle, PropValue: $scope.selectedFlight.PaxMale });
                                                    
                                                }
												 if (_d.PropName == 'prop_pax_female' /*&& $scope.flight.PaxAdult != _d.PropValue*/) {
                                                    $('#' + _d.PropName).val($scope.selectedFlight.PaxFemale);
                                                    updates.push({ OFPId: $scope.entity.Id, PropName: _d.PropName, User: $rootScope.userTitle, PropValue: $scope.selectedFlight.PaxFemale });
