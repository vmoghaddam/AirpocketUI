'use strict';
app.controller('planBoardController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', function ($scope, $location, $routeParams, $rootScope, flightService, weatherService, aircraftService, authService, notificationService, $route) {
    $scope.prms = $routeParams.prms;
    authService.setModule(3);
    $rootScope.setTheme();
    $scope.IsPlanning = true; //$rootScope.HasMenuAccess('flight_planning', 3) ;
    ...
    $scope.IsJLAccess = $rootScope.HasMenuAccess('flight_board_jl', 3) || $scope.IsEditable;
    ...
    function printCanvas(canvas) {
        ...
        frameDoc.document.write("<div style='height:100%'><div style='text-align:center;margin-top:30px'></div><div style='text-align:center;margin-top:10px'></div><img style='margin-top:10px;height:70%'  src = '" + canvas.toDataURL() + "'/></div>");
        frameDoc.document.write('</body></html>');
        frameDoc.document.close();
        setTimeout(function () {
            window.frames["frame1"].focus();
    ...
    $scope.bindFlights = function (saveState) {
        ...
        $scope.tabs_date = [];
        //xati
        var oldTabIndex = $scope.selectedTabDateIndex;
        $scope.selectedTabDateIndex = -1;
        $scope.loadingVisible = true;
        var ed = (new Date($scope.dateEnd)).toUTCDateTimeDigits(); //(new Date($scope.dateto)).toUTCDateTimeDigits();
        flightService.getPlanGantt(Config.CustomerId, (new Date($scope.datefrom)).toUTCDateTimeDigits(), ed, offset, ($scope.IsAdmin ? null : $scope.airportEntity.Id), filter).then(function (response) {
            ...
            //}
            $.each(response.tabs, function (_j, _y) {
                $scope.tabs_date.push({ text: _y.DateRangeTitle, id: _j, date: moment(_y.DateFrom).format('YYYY/MM/DD'),dateFrom:new Date(_y.DateFrom),dateTo:new Date(_y.DateTo) });
            });
          ...
          //  $scope.calculateSummary();
            ...
            console.log('-------------------');
            console.log($scope.dataSource);
            if (oldTabIndex != -1 && response.tabs.length > oldTabIndex)
                $scope.selectedTabDateIndex = oldTabIndex;
            else
        ...
        $scope.nowlinetimer = setTimeout(function () {
            ...
            if ($scope.StartNowLineTimerFirst) {
               ...
               // var dtstr = (new Date($scope.datefrom)).yyyymmddtimenow($scope.IsUTC);
                ...
                ganttObj.setModel2({
                    "stripLines": [{ day: dtstr, lineWidth: "1", lineColor: "#ffdab3", lineStyle: "solid" }
    ...
    $scope.$watch("selectedTabDateIndex", function (newValue) {
        ...
        try {
           ...
           // console.log($scope.selectedTabDateIndex);
           ...
           // console.log($scope.selectedTab);
            ...
            //console.log('tabs init change  ' + $scope.tabsdatefirst + '   ' + newValue);
            //if (!$scope.tabsdatefirst && newValue!=-1) {
            //    console.log('tab changed'); 
            //    $scope.tabsdatefirst = false;
            //}
        ...
        onClick: function (e) {
            ...
            General.Confirm(Config.Text_DeleteConfirm, function (res) {
                if (res) {
                    ...
                    var dto = { Id: flight.Id, };
                    $scope.loadingVisible = true;
                    flightService.deletePlanItemBoard(dto).then(function (response) {
                        $scope.loadingVisible = false;
                        General.ShowNotify(Config.Text_SavedOk, 'success');
    ...
    $scope.btn_plan_edit = {
        ...
        onClick: function (e) {
            ...
            $scope.loadingVisible = true;
            flightService.getPlanItemBoard(flight.Id, offset).then(function (response) {
                $scope.loadingVisible = false;
                $scope.IsNew = false;
    ...
    $scope.popup_plan_add = {
        ...
        toolbarItems: [
...

            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'bplan', onClick: function (arg) {
                        //nook
                        ...
                        $scope.planEntity.Dep = (new Date($scope.planEntity.Dep)).toUTCString();

                        $scope.planEntity.Arr = (new Date($scope.planEntity.Arr)).toUTCString();
                        //startDate
                        $scope.planEntity.TypeId = 12;
                         
                        $scope.loadingVisible = true;
                        flightService.savePlanItemBoard($scope.planEntity).then(function (response) {
        ...
        onShown: function (e) {
            ...
            {
                //sosa
                $scope.planEntity.iDateFrom = new Date($scope.selectedTab.dateFrom);
                $scope.planEntity.iDateTo = new Date($scope.selectedTab.dateTo);
            }
        },
        onHiding: function () {
    ...
    $scope.planEntity = {
        ...
        iDateFrom: new Date(2019,11,22),
        iDateTo: new Date(2020,0,20),
    ...
    $scope.clearPlanEntity = function () {
        ...
        $scope.planEntity.DateFrom = new Date(2019, 11, 22);
        $scope.planEntity.DateTo = new Date(2020, 0, 20);
        
        $scope.planEntity.iDateFrom = new Date(2019, 11, 22);
        $scope.planEntity.iDateTo = new Date(2020, 0, 20);
        

        $scope.planEntity.TypeId = null;
        
        $scope.planEntity.FromId = null;
        $scope.planEntity.ToId = null;
        $scope.planEntity.Arr = null;
        $scope.planEntity.Dep = null;
        $scope.planEntity.FlightH = null;
        $scope.planEntity.FlightM = null;
        $scope.planEntity.FlightNumber = null;
       // $scope.planEntity.Day = null;
    ...
    $scope.bindPlanEntity = function (data) {
        ...
        $scope.planEntity.Day= data.Day;

        
        $scope.planEntity.TypeId = data.TypeId;
       
        $scope.planEntity.FlightNumber = data.FlightNumber;
        $scope.planEntity.FromId = data.FromId;
        $scope.planEntity.ToId = data.ToId;
        $scope.planEntity.Arr = data.STA;
        $scope.planEntity.Dep = data.STD;
        $scope.planEntity.FlightH = data.FlightH;
        $scope.planEntity.FlightM = data.FlightM;
       

        $scope.start = new Date($scope.planEntity.Dep);
      
        $scope.tempToAirport = $scope.planEntity.ToId;
    ...
    $scope.plan_date_from = {
        ...
        onValueChanged: function (arg) {
            // $scope.fillSchedule();
            $scope.planEntity.iDateFrom = arg.value;
        },
        bindingOptions: {
        ...
        onSelectionChanged: function (arg) {
            ...
            flightService.getRouteDestination(Config.AirlineId, arg.selectedItem.Id).then(function (response) {
                ...
                if ($scope.tempToAirport) {
                    $scope.planEntity.ToId = $scope.tempToAirport;
                    $scope.tempToAirport = null;
                }
    ...
    $scope.setArrivalPlan = function () {
        ...
        if ($scope.planEntity.Dep != null && $scope.planEntity.FlightH != null && $scope.planEntity.FlightM != null) {
            var std = new Date($scope.planEntity.Dep);
            
            $scope.planEntity.Arr = new Date(std.addHours($scope.planEntity.FlightH).addMinutes($scope.planEntity.FlightM));
    ...
    $scope.plan_sb_days = {
        ...
        showClearButton: true,
        searchEnabled: true,
        dataSource: ['Sat','Sun','Mon','Tue','Wed','Thu','Fri'],
        
        bindingOptions: {
            value: 'planEntity.Day',
    ...
    $scope.btn_fill = {
        ...
        onClick: function (e) {
            ...
            flightService.getJLDataLegs(idsStr).then(function (response) {
                ...
                $scope.jl.tel = "+982177665544";
                $scope.jl.email = "OpsEng@Airline.aero";
                $scope.jl.sectors = [];
                for (var i = 0; i < 6; i++) {
                    var s = i + 1;
                    var sec = { sector: s };
    ...
    $scope.btn_fill2 = {
        ...
        onClick: function (e) {
            ...
            flightService.getCLDataLegs(idsStr).then(function (response) {
                ...
                $scope.cl.formNo = "";
                $scope.cl.tel = "+982177665544";
                $scope.cl.email = "OpsEng@Airline.aero";
    ...
    $scope.fillDto = function (entity) {
       ...
       // var la = Enumerable.From($scope.dg_delay_ds).Where('$.Id==97').FirstOrDefault();
                    ...
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'fbaclog', bindingOptions: { disabled: 'IsApproved' }, visible: true, onClick: function (arg) {
                     ...
                     //   console.log(dto);
                        ...
                        flightService.saveFlightLog(dto).then(function (response) {
                            //console.log('DS FSDF SDF SDF SDF SDF ');
                            //console.log($scope.flight);
                            General.ShowNotify(Config.Text_SavedOk, 'success');
                            if (!$scope.ChocksOut)
                            ...
                            ganttObj.reRenderChart();
                            $('.task-' + $scope.flight.ID).parent().addClass('e-gantt-taskbarSelection');
                            // alert($scope.flight.ID + '   ' + $elem.length);
                            //  var rec = ganttObj._getRecordByTaskId($scope.flight.ID.toString());
    ...
    if (!authService.isAuthorized()) {
        ...
        $rootScope.page_title = '> Planning Board';
    ...
    $scope.renderTopTimeHeader = function () {
        ////////////////////////////////////////////////
        $.each($('.e-treegridrows.gridrowIndexlevel0').find('.e-cell'), function (key, value) {
            //alert($(value).html());
            var v = $(value).html();
            var bc = "";
            if (v.indexOf('Sat') != -1)
            ...
            if (v.indexOf('Fri') != -1)
                bc = "#ff66ff";
            $(value).parents(".e-treegridrows.gridrowIndexlevel0").css("background", bc);
        ...
        $.each(_whcs, function (_i, _d) {
           ...
           // console.log('renderTopTimeHeader');
            ...
            persianDate.toLocale('en');
            var newwc = oldwc + " (" + new persianDate(wdate).format("DD/MM/YYYY") + ")";
            var str = new persianDate($scope.selectedTab.dateFrom).format("DD/MM/YYYY ") + " TO  " + new persianDate($scope.selectedTab.dateTo).format(" DD/MM/YYYY");
            whcs.html(str);
            //ati5
        });
    };
    ...
    $scope.createGantt = function (_scale) {
       ...
       // alert($(window).width());
        ...
        if (ganttObj)
            ganttObj.destroy();
        $scope.StartNowLineTimerFirst = true;
        var h = $(window).height() - 139 - 50 +140 -130;
        h = h + 'px';
        ...
        //var _dt = General.getDayLastHour(new Date(d2));
         ...
         _df =  (new Date($scope.selectedTab.dateFrom));
         _dt = (new Date($scope.selectedTab.dateTo));
         console.log('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX');
         console.log($scope.resources);
        
        //2019 12 22 12 01
         var reskey = _df.getFullYear().toString() + (_df.getMonth() + 1).toString() + _df.getDate().toString() +
             (_dt.getMonth() + 1).toString() + _dt.getDate().toString();
         console.log(reskey);
         var iRes = [];
         var iRes = Enumerable.From($scope.resources).Where(function (x) {
             var str = x.resourceId.toString();
             return str.startsWith(reskey);
         }).ToArray();
         console.log(iRes);
         console.log('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX');
        // console.log($scope.dataSource);
         ...
         var _data = Enumerable.From($scope.dataSource).Where(function (x) {
            //var _df2 = (new Date($scope.selectedTab.dateFrom));
            var _ds = new Date(x.DateFrom);
            var _de = new Date(x.DateTo);
            //console.log('from');
            //console.log(_ds);
            //console.log(_df );
            //console.log('to');
            //console.log(_de);
            //console.log(_dt);
            //console.log(_ds == _df2 );
            return (_ds.getTime() === _df.getTime() && _de.getTime() === _dt.getTime());
        }).ToArray();
         //var _data = Enumerable.From($scope.dataSource).Where(function (x) {
         //    return true;
         //}).ToArray();
       ...
       // console.log(new Date($scope.selectedDate));
        ...
        $("#resourceGanttba").ejGantt({
            scheduleStartDate:new Date(1900, 0, 1, 0, 0, 0, 0), //_df, //$scope.datefrom,
            scheduleEndDate: new Date(1900, 0, 1, 23, 59, 59, 999), //_dt,//$scope.dateto,
          ...
          //  groupCollection: $scope.resourceGroups,
            resources:iRes, //$scope.resources, //resourceGanttResources,
            ...
            create: function (args) {
                try {
                   ...
                   // var scrollnextday = true;
                    ...
                    //ati oosk 
                    //$('#ganttviewerbodyContianerParentejGanttChartresourceGanttba').prepend("<div id='atinow'>Time Line</div>");
            ...
            if (!$scope.linkedTimerDone) {
               // console.log('linked timer');
                ...
                if (linked.length > 0) {
                    ...
                    // $('.linked-flight').show();
                    //dook
                    //$('.e-childContainer').each(function () {
                    //    var time = $(this).find(".task-body").data('time');
                    //    var mt = Number($(this).css('margin-top').replace('px', '')) - 10;
                    //    $(this).css('margin-top', mt + 'px').append('<div class="task-footer">' + time + '</div>');
                    //});
