...
app.controller('fdpsController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, $timeout, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
    $scope.xids=[
	37065,37066,37067,37068,37069,37070,37071,
	37073,37074,37077,37075,37076,37078,37079,37080,37081,
	37082,37084,37083,37085,
	37086,37087,37088,37089,37090,37091,37092,37093,37094,
	37095,37096,37097,37098,37099,37100,
	37138,37139,37140,37141,37142,37143,37144,37145,37146,37147,
	37164,37163,37160,37161,37162,
	37148,37149,
	37150,37151,37152,37153,37154,37155,37156,37157,37158,37165,
	37124,37125,37126,37129,37130,37128,37127,37137,37136,37135,37134,
	37107,37105,37104,
	37108,37109,37114,37110,37111,37112,37115,
	37082,37083,37084,37085,
	37116,37117,37118,37119,37121,37120,37122,37123,
	37103,37101,37102,37166,37132,37131,37133
    ...
    $scope.editable = true;
    $scope.isAdmin = $route.current.isAdmin || $rootScope.userName.toLowerCase().startsWith('razbani');
...
$scope.num_rev = {
  ...
  $window.open($rootScope.reportServerCrew + '?type=16&date='+_dtd+'&rev='+$scope._rev, '_blank');
    ...
    $scope.dg_cduties_columns = [
      ...
      { dataField: 'Id', caption: 'Id', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, width: 100, fixed: true, fixedPosition: 'left', },
    ...
    $scope.cellContextMenuItems = [
        ...
        { text: 'FDP', onItemClick: $scope.assign100005, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-100005' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>FDP</td></tr></table>", },
        { text: 'REST', onItemClick: $scope.assign100006, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-100006' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>REST</td></tr></table>", },
    ...
    $scope.createEvent = function (_crew, _type, _typeTitle, eventFrom, eventEnd, remark) {
		if (!$scope.isAdmin && new Date(eventFrom) < $scope.firstHour) {
...
if ($rootScope.userName.toLowerCase().startsWith('razbani'))
        ...
        if ($event.ctrlKey) {
          ...
          //  if (!crew.RemainCCRM || crew.RemainCCRM < 0)
           //     expired.push('CCRM ' + (crew.CCRMExpired ? moment(crew.CCRMExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
            ...
            if (crew.JobGroupCode.startsWith('00101')) {
                if (!crew.RemainLPC || crew.RemainLPC < 0)
				 ...
				 if (!crew.RemainOPC || crew.RemainOPC < 0)
                    expired.push('OPC ' + (crew.DateTypeExpire ? moment(crew.DateTypeExpire).format('YYYY-MM-DD') : 'UNKNOWN'));
            ...
            /////////////////////////
			//if ($rootScope.userName.toLowerCase().startsWith('razbani'))
            if (expired.length > 0 && !$rootScope.userName.toLowerCase().startsWith('razbani')) {
                var myDialog = DevExpress.ui.dialog.custom({
                    rtlEnabled: true,
    ...
    $scope.fillFilteredCrew = function () { 
        ...
        $.each(filtered, function (_i, _d) {
			...
			_d.RemainOPC=!_d.DateTypeExpire? null : $scope.getDaysDiff(dateFirst, new Date(_d.DateTypeExpire));
