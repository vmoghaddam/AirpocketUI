...
app.controller('boardController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', 'fbService', '$http','$q','logService', function ($scope, $location, $routeParams, $rootScope, $timeout, flightService, weatherService, aircraftService, authService, notificationService, $route, $window, fbService, $http,$q,logService) {
	...
	$scope.isDemo = $rootScope.userName.toLowerCase() == 'demo';
    ...
    $scope.IsCrewMobileVisible =$rootScope.roles.indexOf('Dispatch') != -1 || $rootScope.userName.toLowerCase().startsWith('dis') || $rootScope.userName.toLowerCase().startsWith('demo') || $rootScope.userName.toLowerCase().startsWith('trans');
    $scope.IsSMSVisible =$rootScope.roles.indexOf('Dispatch') != -1 ||  $rootScope.userName.toLowerCase().startsWith('dis') || $rootScope.userName.toLowerCase().startsWith('demo') || $rootScope.userName.toLowerCase().startsWith('cs.') || $rootScope.userName.toLowerCase().startsWith('razbani');
    ...
    $scope.getRealMSNs = function (cid, callback) {
        ...
        flightService.getRealRegisters(cid).then(function (response) {
            $scope.realMSNs = Enumerable.From(response)
                .Where(function (x) { return !x.Register.startsWith('CNL') && (x.Register.startsWith('V') || x.Register.startsWith('R') || x.Register.startsWith('FS') || x.Register.startsWith('SB')) ; })
 ...
 $scope.$watch("selectedTabDateIndex", function (newValue) {
        ...
        try {
            $('.tab-new-item').removeClass('tab-new-selected');
            if ($scope.selectedTabDateIndex == -1)
                return;
			$('#tab_'+($scope.selectedTabDateIndex+1)).addClass('tab-new-selected');
            $scope.selectedTab = $scope.tabs_date[newValue];
	...
	$scope.tab_click=function(id){
	  //$('#tab_'+id).css('color','red');
		/*if ($scope.selectedTabDateIndex == -1)
                return;
		 $scope.selectedTab = $scope.tabs_date[id-1];
		...
		$scope.selectedTabDateIndex=id-1;
...
itemHoldTimeout:100,
	...
	$scope.btn_hide={
		 hint: 'Hide/Visible Flights by Date',
        type: 'default',
        icon: 'fas fa-eye',
        width: '100%',
		onClick:function(e){
			$scope.popup_hide_visible=true;
		}
	}
	
	
	
	$scope.hide_visible_by_ids=function(hide){
		 if (!$scope.ati_selectedFlights || $scope.ati_selectedFlights.length == 0) {
                General.ShowNotify(Config.Text_NoFlightSelected, 'error');
                return;
            }
			 General.Confirm("The selected flights will be "+(hide==1?"hidden":"visible")+". Are you sure?", function (res) {
                            if (res) {
                         

                                       var dto={};
										dto.is_hidden= hide;
										dto.ids=Enumerable.From($scope.ati_selectedFlights).Select('$.ID').ToArray();
										$scope.loadingVisible = true;
										flightService.hideFlights(dto).then(function (response) {

												General.ShowNotify(Config.Text_SavedOk, 'success');
                                                $.each($scope.ati_selectedFlights,function(_i,_d){ _d.CPDH=hide;});
												$scope.loadingVisible = false;
										}, function (err) { $scope.loadingVisible = false; General.ShowNotify(err.message, 'error'); });
	...
	$scope.btn_hide_id={
        ...
        width: '100%',
		onClick:function(e){
			$scope.hide_visible_by_ids(1);
		}
	}
	$scope.btn_visible_id={
		 hint: 'Visible Flights',
        type: 'success',
        icon: 'far fa-eye',
        width: '100%',
		onClick:function(e){
			$scope.hide_visible_by_ids(0);
		}
	}
	...
	$scope.hide_from=null;
	$scope.time_hide_from = {
        type: "date",
        width: '100%',
	...
	$scope.hide_status="Visible";
	$scope.sb_hide={
		dataSource:['Visible','Hidden'],
		showClearButton: false,
        width: '100%',
        searchEnabled: false,
	...
	$scope.hide_by_ids=false;
	 $scope.popup_hide_visible = false;
    $scope.popup_hide_title = 'Hide/Show Flight(s)';
    ...
    $scope.popup_hide = {
        ...
        toolbarItems: [
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'fbhide', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
						...
						var dto={};
						 var from_dates = (new Date($scope.hide_from)).getDatePartArray();
						  var ifrom = new Date(from_dates[0], from_dates[1], from_dates[2], 12, 0, 0, 0);
        var  to_dates = (new Date($scope.hide_to)).getDatePartArray();
        var ito = new Date(to_dates[0], to_dates[1], to_dates[2], 12, 0, 0, 0);
		
						 
						dto.from_raw=moment(new Date(ifrom)).format("YYYY-MM-DD-12-00");
						dto.to_raw=moment(new Date(ito)).format("YYYY-MM-DD-12-00");
						dto.is_hidden= $scope.hide_status!='Visible';
						//copyFlights
						 $scope.loadingVisible = true;
                                flightService.hideFlights(dto).then(function (response) {
									...
									//	 $scope.modifyFlightTimes(_flt);
                                    //     $scope.addToGantt(_flt, response.ressq[0]);
										
									//});
									var _ifrom=moment(new Date(ifrom)).format("YYYYMMDD");
									var _ito=moment(new Date(ito)).format("YYYYMMDD");
									var _flts=Enumerable.From($scope.ganttData.flights).Where(function(x){
										  return Number(moment(new Date(x.STD)).format("YYYYMMDD"))>=_ifrom && Number(moment(new Date(x.STD)).format("YYYYMMDD"))<=_ito;
										
									}).ToArray();
									$.each(_flts,function(_i,_d){_d.CPDH=$scope.hide_status!='Visible'?1:0;});
									  $scope.popup_hide_visible = false;
        ...
        bindingOptions: {
            visible: 'popup_hide_visible',
            height: 'popup_hide_height',
            title: 'popup_hide_title',

        }
    };

    //close button
    $scope.popup_hide.toolbarItems[1].options.onClick = function (e) {
...
$scope.interval_from_readonly=true;
 $scope.btn_copy = {
        hint: 'Copy Flights',
        type: 'default',
		...
		onClick: function (e) {
			...
			var _flight = Enumerable.From($scope.ganttData.flights).Where('$.ID==' + $scope.ati_selectedFlights[0].ID).FirstOrDefault();
			  $scope.time_interval_from_date = new Date(_flight.STD);
              $scope.time_interval_to_date = new Date(_flight.STD);
              $scope.interval_days = [];
              $scope.interval_days.push((new Date(_flight.STD)).getDay());
              $scope.popup_copy_visible = true;
		}
 }
  $scope.dg_dup_columns = [
         {
            dataField: 'flt_date', caption: 'Date', allowResizing: true, alignment: 'center', dataType: 'datetime', allowEditing: true, format: 'yyyy-MM-dd'  ,sortOrder:'asc',sortIndex:0
        },
         { dataField: 'flt_no', caption: 'Flt. No', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 150  },
       
    ];
    $scope.dg_dup_selected = null;
    $scope.dg_dup_instance = null;
    $scope.dg_dup_ds = null;
    $scope.dg_dup = {
        headerFilter: {
            visible: false
        },
        ...
        //2020-10-27 1 s
        height: 150,// $(window).height() - 250,// 490 

        columns: $scope.dg_dup_columns,
        onContentReady: function (e) {
            if (!$scope.dg_dup_instance)
                $scope.dg_dup_instance = e.component;
        ...
        bindingOptions: {
            dataSource: 'dg_dup_ds',

        }
    };
  $scope.popup_copy_visible = false;
    $scope.popup_copy_title = 'Copy Flight(s)';
    ...
    $scope.popup_copy = {
        ...
        toolbarItems: [
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'fbcopy', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
        ...
        var intervalTo = new Date(interval_to_dates[0], interval_to_dates[1], interval_to_dates[2], 12, 0, 0, 0);
						...
						dto.FlightIds=Enumerable.From($scope.ati_selectedFlights).Select('$.ID').ToArray();
						dto.IntervalFromRAW=moment(new Date(intervalFrom)).format("YYYY-MM-DD-12-00");
						dto.IntervalToRAW=moment(new Date(intervalTo)).format("YYYY-MM-DD-12-00");
						dto.Days = $scope.interval_days;
						 var ref_dates = (new Date($scope._datefrom)).getDatePartArray();
					     var ref = new Date(ref_dates[0], ref_dates[1], ref_dates[2], 12, 0, 0, 0);
						dto.RefDayRAW=moment(new Date(ref)).format("YYYY-MM-DD-12-00");
						dto.FromRAW=moment(new Date($scope._datefrom)).format("YYYY-MM-DD-12-00");
						dto.ToRAW=moment((new Date($scope._datefrom)).addDays($scope.days_count - 1)).format("YYYY-MM-DD-12-00");
						$scope.dg_dup_ds=[];
						console.log(dto);
						//copyFlights
						 $scope.loadingVisible = true;
                                flightService.copyFlights(dto).then(function (response) {
									...
									if (response.duplicated && response.duplicated.length>0)
					
					                 $scope.dg_dup_ds=response.duplicated;
                                    
                                    else $scope.popup_copy_visible = false;
        ...
        onShown: function (e) {
$scope.interval_from_readonly=false;
    ...
    $scope.btn_edit = {
        ...
        onClick: function (e) {
            ...
            if ($scope.ati_flight.FlightPlanId && 1 == 2) {
				...
				$scope.is_hidden_flight=$scope.ati_flight.CPDH;
    ...
    $scope.btn_fltdocdnl = {
        ...
        text: 'Download',
        onClick: function (e) {
            $window.open('https://files.airpocket.online/varesh/flt/' + $scope.fltdoc_url, '_blank');
        },
        bindingOptions: {
 ...
 $scope.parkingPosition = {
    ...
    $scope.time_interval_from = {
        ...
        bindingOptions: {
			...
			readOnly:'interval_from_readonly'
...
////////////////////////////////////////////////////////
			...
			_flight.CPDH=$scope.is_hidden_flight;
...
////////////////////////////////////////////////////////
    ...
    $scope.saveFree_2024_02_16 = function () {
        //07-10
...
////////////////////////////////////////////////////////
	...
	$scope.is_hidden_flight = false;
    $scope.check_hidden = {
        width: '100%',
        text: "Hidden",

        bindingOptions: {
            value: 'is_hidden_flight',
        }
    };
    $scope.popup_free_visible = false;
...
////////////////////////////////////////////////////////
 ...
 //janfe
    ...
    $scope.getFlightClass = function (flt) {
		...
		else if (flt.FlightStatusID==1 && flt.CPDH) {
			return 'flt_hidden';
		}
        else 
        return flt.FlightStatus.toLowerCase();
    ...
    $scope.bindFlights = function (callback) { 
        ...
        //2024-02-19
        logService.getFlightsGanttUTC($scope.IsComm?100: Config.CustomerId, (new Date($scope.datefrom)).toUTCDateTimeDigits(), ed, /*offset*/0, null, 1, filter).then(function (response) {
            try {
                //2020-11-25
                ...
                for (i = 1; i <= $scope.days_count; i++) {
                    var str = moment(stdate).format("ddd DD-MMM").toUpperCase();
...
//10-31
 ...
 var _ids=[];
            $.each(selected, function (_i, _d) {
               ...
               // $scope.ati_selectedFlights.push($d.data('flight'));
                // $scope.ati_selectedTypes.push($d.data('type'));
				 ...
				 _ids.push($d.data('flightid'));


            });
            //$scope.ati_selectedTypes=Enumerable.From($scope.ati_selectedTypes).Distinct().ToArray();
            $scope.ati_selectedFlights=Enumerable.From($scope.ganttData.flights).Where(function(x){return _ids.indexOf(x.ID)!=-1;}).ToArray();
    ...
    $scope.IsAptRangeVisible = $rootScope.userName.toLowerCase().startsWith('comm.') || $rootScope.userName.toLowerCase().startsWith('dis.')
        || $rootScope.userName.toLowerCase().startsWith('demo')
        || $rootScope.userName.toLowerCase().includes('razbani')
		|| $rootScope.roles.indexOf('Dispatch') != -1;
...

            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Download', icon: 'save', onClick: function (e) {
                        ...
                        $scope.datefrom = General.getDayFirstHour(new Date(dfrom));
                        $scope.dateEnd = General.getDayLastHour(new Date(new Date(dfrom).addDays($scope.days_count - 1)));
                         var _tturl='https://api2.apvaresh.com/' + 'api/xls/' + '?dt1=' + moment(new Date($scope.datefrom)).format('YYYY-MM-DD') + '&dt2=' + moment(new Date($scope.dateEnd)).format('YYYY-MM-DD') + "&chr=" + $scope.tt_chr + "&crew=" + $scope.tt_crew + "&cnl=" + $scope.tt_cnl + "&sort=" + $scope.tt_sort + "&time=" + $scope.tt_time+"&sep=1";
						console.log(_tturl);
                        $window.open('https://api2.apvaresh.com/' + 'api/xls/' + '?dt1=' + moment(new Date($scope.datefrom)).format('YYYY-MM-DD') + '&dt2=' + moment(new Date($scope.dateEnd)).format('YYYY-MM-DD') + "&chr=" + $scope.tt_chr + "&crew=" + $scope.tt_crew + "&cnl=" + $scope.tt_cnl + "&sort=" + $scope.tt_sort + "&time=" + $scope.tt_time+"&sep=1", '_blank');
                    }
                }, toolbar: 'bottom'
            },
  ...
  //////////////////////
    ...
    $scope.dg_tbl = {
        ...
        onEditorPrepared: function (e) {
            ...
            //if (e.parentType == 'dataRow' && e.dataField == 'FlightNumber') {
            //    e.editorElement.dxTextBox('instance').option('onValueChanged', args => {
            //        e.setValue(args.value);
            //        console.log('fn upd', args);
            //        console.log(e);
            //        e.row.values[3] = 152331;

            //    });
            //    //console.log(e);
            //}
            //if (e.parentType == 'dataRow' && e.dataField == 'FromAirport') {
            //    e.editorElement.dxSelectBox('instance').option('onSelectionChanged', function (arg) {
            //        //your actions here 
...
$scope.uploadToDate = {
    ...
    $scope.xlsUploader = {
        ...
        uploadMode: "instantly",
        rtlEnabled: true,
		uploadUrl: "https://apixls.apvaresh.ir/api/plan/uploadxlsplan",
        //uploadUrl: "https://api.apvaresh.ir/api/uploadfile",
        onValueChanged: function (arg) {
        },
        onUploaded: function (e) {
