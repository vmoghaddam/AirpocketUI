...
$scope.destroy_menu();
    ...
    $scope.getCrewCerStatus = function (c, dt) {
		...
		if (item.Id==4042 /*&& dt.caption=='Jab-13'*/)
		{  console.log('getCrewCerStatus status',result); console.log(item.CRMExpired);}
        return result;
    };
    ...
    $scope.addStby = function (_type,_from,_end) {
        ...
        schedulingService.saveSTBY(dto).then(function (response) {
			...
			else if (response.Code == 308 || (response.data && response.data.Code==308)) {
                if (response.message) {
                    var myDialog = DevExpress.ui.dialog.custom({
                        rtlEnabled: true,
                        title: "Error",
