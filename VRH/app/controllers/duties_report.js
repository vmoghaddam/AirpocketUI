'use strict';
app.controller('duties_report', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route, $window) {
    ...
    $scope.getCrewFlightsTotal = function (df, dt) {
        ...
        flightService.get_duties_report(null,df, dt).then(function (response) {
            $scope.loadingVisible = false;
            if (!$scope.IsCockpit) {
                response = Enumerable.From(response).Where(function (x) {
            ...
            $scope.dg_duties_total_ds = response;
    ...
    $scope.dg_duties_total_columns = [
        ...
        { dataField: 'Name', caption: 'Name', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, fixed: true, fixedPosition: 'left', minWidth: 150 },
        { dataField: 'DutyType', caption: 'DutyType', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, fixed: true, fixedPosition: 'left', width: 150 },
        { dataField: 'DateStart', caption: 'Start', allowResizing: true, alignment: 'center', dataType: 'date', allowEditing: false, fixed: true, fixedPosition: 'left', width: 150 },
        { dataField: 'DateEnd', caption: 'End', allowResizing: true, alignment: 'center', dataType: 'date', allowEditing: false, fixed: true, fixedPosition: 'left', width: 150 },


    ];

    $scope.dg_duties_total_selected = null;
    $scope.dg_duties_total_instance = null;
    $scope.dg_duties_total_ds = null;
    $scope.dg_duties_total = {
        headerFilter: {
            visible: false
        },
        filterRow: {
        ...
        columns: $scope.dg_duties_total_columns,
        onContentReady: function (e) {
            if (!$scope.dg_duties_total_instance)
                $scope.dg_duties_total_instance = e.component;
        ...
        onSelectionChanged: function (e) {
            ...
            if (!data) {
                ...
                $scope.dg_duties_total_selected = data;
                ...
                $scope.getCrewFlights($scope.dg_duties_total_selected.CrewId, $scope.dt_from, $scope.dt_to);
            }
