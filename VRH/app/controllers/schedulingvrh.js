'use strict';
app.controller('schedulingvrhController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'zschedulingService', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, $timeout, schedulingService, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
    //06-13
...
//soltani
    ...
    $scope.saveNewDutyCal = function (crewid, callback) {
        ...
        schedulingService.saveDuty(dto).then(function (response) {
            $scope.loadingVisible = false;
            //2023
			console.log('save duty',response);
            if (response.Code == '406') {
                    ...
                    type: 'default', text: 'Download', icon: 'print', onClick: function (arg) {
                      ...
                      //  $window.open(serviceBaseAPI + 'api/xls/roster/all/daily/dr02?df=' + _dtd+'&rev='+$scope.report_rev, '_blank');
                         var _dtd = (moment($scope._dailydate).format('YYYY-MM-DD'));
                           ...
                           $window.open('https://xapi.apvaresh.com/' + 'api/xls/roster/all/daily?df=' + _dtd, '_blank');
