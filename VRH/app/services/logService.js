...
app.factory('logService', ['$http', '$q', 'ngAuthSettings', '$rootScope', function ($http, $q, ngAuthSettings, $rootScope) {
	...
	var _getFlightsGanttUTCNew = function (cid, from, to, tzoffset, airport, filter) {
        var url = zlog + 'api/flights/gantt/utc/customer/' + cid + '/' + from + '/' + to + '/' + tzoffset;
        if (airport)
            url += '/' + airport;
	...
	serviceFactory.getFlightsGanttUTCNew = _getFlightsGanttUTCNew;
