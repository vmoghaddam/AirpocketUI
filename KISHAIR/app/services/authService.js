...
app.factory('authService', ['$http', '$q', 'localStorageService', 'ngAuthSettings', '$location', '$rootScope', function ($http, $q, localStorageService, ngAuthSettings, $location, $rootScope) {
    ...
    var _login = function (loginData) {
        ...
        //    deferred.reject(err);
        //    });
        $http.post(apiLock + 'token', data, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }).then(function (response) {
            console.log('token');
    ...
    var _fillAuthData = function () {
        ...
        if (authData) {
			...
			var userExt = localStorageService.get('userExt');
            if (userExt)
                $rootScope.EditWindow = userExt.EditWindow;
        }
    ...
    var _resend = function (entity) {
        var deferred = $q.defer();
        $http.post($rootScope.serviceUrl + 'odata/verification/resend', entity).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    var _register2 = function (entity) {
        var deferred = $q.defer();
        $http.post($rootScope.serviceUrl + 'odata/users/register', entity).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    var _deleteUser = function (entity) {
        var deferred = $q.defer();
        $http.post($rootScope.serviceUrl + 'odata/users/delete', entity).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
	...
	var _getUserByUserName = function (userName) {
        ...
        var deferred = $q.defer();
        $http.get(serviceBase + 'odata/user/'+userName).then(function (response) {
            deferred.resolve(response.data);
            //console.log(response);
            localStorageService.set('userExt', { EditWindow: response.data[0].EditWindow });
            $rootScope.EditWindow = response.data[0].EditWindow;
            console.log(localStorageService.get('userExt'));
        }, function (err, status) {
	...
	var _checkFailedLogins = function (userName, isLoginSuccessful) {

        var deferred = $q.defer();
        $http.get( serviceBase+ 'odata/user/checklock/' + userName + '/' + isLoginSuccessful).then(function (response) {
    ...
    authServiceFactory.checkFailedLogins = _checkFailedLogins;
