...
app.controller('fixTimeReportDailyController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route) {
    ...
    $scope.dg_fix = {
        ...
        onCellPrepared: function (e) {
             if (e.rowType === "data" && e.column.dataField == "FixTimeTotal2"  )
            ...
            calculateCustomSummary: function (options) {
                ...
                if (options.name === "FixTimeTotal") {
                    ...
                    if (options.summaryProcess === "calculate") {
                        ...
                        options.totalValueMinutes = options.totalValueMinutes + options.value.FixedFlightTime;
                        options.totalValue = pad(Math.floor(options.totalValueMinutes / 60)).toString() + ':' + pad(options.totalValueMinutes % 60).toString();
    ...
    $scope.bind = function () {
        ...
        flightService.getCrewFixTimePeriodReportDaily(_df,_dt, $scope.rank,$scope.actype).then(function (response) {
            ...
            $.each(response, function (_i, _d) {
                ...
                _d.FixTime2 = $scope.formatMinutes(_d.FixedFlightTime);
                ...
                if (!_d.PositioningFixTime)
                    _d.PositioningFixTime = 0;
                _d.PositioningFixTime2 = $scope.formatMinutes(_d.PositioningFixTime);

                if (!_d.StandbyFixTime)
                    _d.StandbyFixTime = 0;
                _d.StandbyFixTime2 = $scope.formatMinutes(_d.StandbyFixTime);
				
				
				if (!_d.CanceledFixTime)
                    _d.CanceledFixTime = 0;
                _d.CanceledFixTime2 = $scope.formatMinutes(_d.CanceledFixTime);
    ...
    $scope.bind_details = function (crew) {
        ...
        $scope.loadingVisible = true;
       // flightService.getCrewFixTimePeriodReportCrewDaily(_df,_dt, crew).then(function (response) {
           ...
           // });
            ...
            var _fdps=$scope.dg_fix_selected.FDPs;
			  $.each(_fdps, function (_i, _d) {
                _d.FixTimeX = $scope.formatMinutes(_d.FixedFlightTime);
                _d.FlightTimeX = $scope.formatMinutes(_d.FlightTime);
                _d.BlockTimeX = $scope.formatMinutes(_d.BlockTime);
            ...
            $scope.dg_details_ds = _fdps;//response;
            //momo
            flightService.getCrewFixTimePeriodReportCrewNoFDPDaily(_df, _dt, crew).then(function (response2) {
                $.each(response2, function (_i, _d) {

                    _d.DurationX = $scope.formatMinutes(_d.Duration);
                    if (_d.DutyTypeTitle == 'StandBy') {
                        _d.Duration = null;
                        _d.DurationX = null;
                    }
                ...
                flightService.getCrewFixTimePeriodReportCrewOthersDaily(_df, _dt, crew).then(function (response3) {
