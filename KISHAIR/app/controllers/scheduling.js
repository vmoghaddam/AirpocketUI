...
app.controller('schedulingController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'schedulingService', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, $timeout, schedulingService, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
    ...
    if ($rootScope.userName.toLowerCase() == 'train.moradi' || $rootScope.userName.toLowerCase() == 'mohammadifard' || $rootScope.userName.toLowerCase() == '82817'
	|| $rootScope.userName.toLowerCase() == '86564' || $rootScope.userName.toLowerCase() == '93207')
        ...
        onClick: function (e) {
            ...
            if ($scope.ati_selectedFlights && $scope.ati_selectedFlights.length > 0) {
               ...
               /* persianDate.toLocale('en');
                ...
                $scope.fxmonth = _t;
				*/
				$scope.dt_fx=new Date($scope.fxFlight.STD);

            }
			else
			{
				$scope.dt_fx=new Date($scope.selectedDate);
			}
	...
	$scope.btn_req = {
        ...
        onClick: function (e) {
              $window.open('https://fms.kishairlines.ir/#!/forms/vacation', '_blank');

        }

    };
	
	$scope.setoff_df=new Date();
	$scope.setoff_dt=new Date();
	
	$scope.date_setoff_df = {
        type: "date",
        placeholder: 'From',
       width: '100%',
        displayFormat: "yyyy-MM-dd",
	...
	$scope.setoff_grps='All';
	$scope.sb_setoff_grps= {
	...
	$scope.btn_setoff = {
        ...
        onClick: function (e) {
            ...
            $scope.popup_setoff_visible = true;
        }

    };
	//setoff
	$scope.popup_setoff_visible = false;
    $scope.popup_setoff_title = 'Set Off';
    $scope.popup_setoff = {
        elementAttr: {
            //  id: "elementId",
            class: "popup_setoff"
        ...
        toolbarItems: [
			...
			{
                widget: 'dxButton', location: 'after', options: {
                    type: 'default', text: 'Save', icon: 'save', onClick: function (arg) {

                        //$scope.popup_setoff_visible = false;
						////var _dtd = (moment($scope._dailydate).format('YYYY-MM-DD'));
						
						var _df=moment(new Date($scope.setoff_df)).format('YYYY-MM-DD');
						var _dt=moment(new Date($scope.setoff_dt)).format('YYYY-MM-DD');
						$scope.loadingVisible = true;
                        schedulingService.setOff(_df,_dt,$scope.setoff_grps).then(function (response) {
                          $scope.loadingVisible = false;
                           General.ShowNotify(Config.Text_SavedOk, 'success');
						   $scope.popup_setoff_visible = false;
...

            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', validationGroup: "fxsave", onClick: function (arg) {
						...
						dto.Date=moment(new Date($scope.dt_fx)).format('YYYY-MM-DD');
                        dto.Count = 0;
                        ...
                        schedulingService.saveFixTimeByDate(dto).then(function (response) {
                            $scope.loadingVisible = false;
                            General.ShowNotify(Config.Text_SavedOk, 'success');
                            // $scope.popup_fx_visible = false;
                            $scope.fxType = null;
    ...
    $scope.addStby = function (_crew, _type,city) {
        _crew = _crew.data;
        var dto = {
    ...
    $scope.dg_crew_stby_columns = [
        ...
        },
		{
            ...
            allowFiltering: false,
            allowSorting: false,
            cellTemplate: 'addStbyAMKIHTemplate',
        ...
        },
        {
            ...
            allowFiltering: false,
            allowSorting: false,
            cellTemplate: 'addStbyPMKIHTemplate',
    ...
    $scope.popup_stby_visible = false;
	 $scope.getSTBYStyle = function (res) {
        if (res.SMSId == 140857)
            return {
                background: '#e6e600'
            };
    ...
    $scope.removeDutyCal = function (duty) {
        var type = duty.DutyType;
        if ($scope.OnlyRoster && (type != 100003 && type != 5000)) { //sim && training
    ...
    $scope.getFlightWidth = function (flt) {
		...
		if (new Date(flt.STD) > new Date(flt.STDPlanned)){
    ...
    $scope.getFlightStyle = function (f, index, res) {
        ...
        var std = f.STD;
		if(new Date(std)> new Date(f.STDPlanned)){
			std = new Date(f.STDPlanned);
		...
		if(new Date(f.STD) > new Date(f.STDPlanned)) {
			left = $scope.getDuration(new Date(dfirst), new Date(f.STDPlanned));
		}
    ...
    $scope.fillRangeFdps = function () {
        ...
        console.log('RANGE FDPS',$scope.rangeFdps);
    ...
    $scope.crewClick = function (crew, $event) {
        if ($event.ctrlKey) {
          ...
          console.log('FDP STAT', $scope.FDPStat);
            ...
            schedulingService.saveFDP(fdp).then(function (response) {
                $scope.loadingVisible = false;
				   if (response.data.dto && response.data.fdp) {
                    var _dto = response.data.dto;
                    response.data = response.data.fdp;
                }
             
                console.log('RETURENED DTO', _dto);
                console.log('RETURENED fdp', response.data);
                if (response.Code == 406) {
                    if (response.data.message) {
                        var myDialog = DevExpress.ui.dialog.custom({
                            rtlEnabled: true,
                    ...
                    if (response.Code == 501) {
                        ...
                        if (response.Code == 304) {
                            ...
                            fdp.Id = response.data.Id;
                            //mixed fdp
                            fdp.items2 = JSON.parse(response.data.Remark2);
                            if (response.data.UPD) {

                                
                                $scope.ati_fdps = Enumerable.From($scope.ati_fdps).Where('$.Id!=' + response.data.UPD).ToArray();

                                fdp.flights = _dto.flights;
                                fdp.from = _dto.from;
                                fdp.group = _dto.group;
                                fdp.ids = _dto.ids;
                                fdp.key = _dto.key;
                                fdp.no = _dto.no;
                                fdp.rank = _dto.rank;
    ...
    $scope.shiftFlight = null;
    //mixed fdp
    $scope.shiftFlightCrews = null;
    $scope.clickFlight = function (f, $event) {

        $event.stopPropagation();
        if ($event.ctrlKey) {
            $scope.shiftFlight = Enumerable.From($scope.ati_flights).Where('$.Id==' + f.ID).FirstOrDefault();
            var shid = $scope.shiftFlight.Id;
            var _fdps = Enumerable.From($scope.ati_fdps).Where('$.key.includes("' + shid + '")').ToArray();
            console.log('SHIFT FDPS', _fdps);
            $scope.shiftFlightCrews = [];
            $.each(_fdps, function (_i, _d) {
                var _obj = { name: _d.scheduleName };
                var _flt = Enumerable.From(_d.items2).Where('$.flightId==' + shid ).FirstOrDefault();
                _obj.rank = _flt.rankStr;
                $scope.shiftFlightCrews.push(_obj);
            });
    ...
    $scope.fillFilteredCrew = function () {
        ...
        switch (id) {
            ...
            case 'OBS':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                    return (x.JobGroup == 'CCM' || x.JobGroup=='SCCM' || x.JobGroup=='ISCCM') || ((x.JobGroup == 'P2' || x.JobGroup == 'P1' || x.JobGroup == 'TRE' || x.JobGroup == 'TRI' || x.JobGroup == 'LTC') && (x.ValidTypes && x.ValidTypes.indexOf($scope.ati_selectedTypes[0]) != -1 || alldh));
                }).ToArray();
                break;
            case 'CHECK':
                 _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
...

            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Default', icon: 'print', onClick: function (arg) {
                        ...
                        var _dtd = (moment($scope._dailydate).format('YYYY-MM-DD'));
                        $window.open($rootScope.reportServer + '?type=15&date=' + _dtd + '&rev=' + $scope._rev, '_blank');
            ...
            },
            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Security', icon: 'print', onClick: function (arg) {
                        ...
                        var _dtd = (moment($scope._dailydate).format('YYYY-MM-DD'));
                        $window.open($rootScope.reportServer + '?type=16&date=' + _dtd + '&rev=' + $scope._rev, '_blank');
                    }
                }, toolbar: 'bottom'
            },
    ...
    $scope.popup_report2 = {
        ...
        toolbarItems: [
            ...
            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Cockpit/Cabin', icon: 'print', onClick: function (arg) {

                        var _dtd = (moment($scope._dailydate).format('YYYY-MM-DD'));
						//if ($rootScope.userName.toLowerCase() == 'cs.kalhori' )
                           $window.open(apiDaily+ 'api/xls/roster/cockpit/daily/kih?df=' + _dtd, '_blank');
						//else
						//	 $window.open(/*'https://api2.aptaban.ir/'*/apiDaily + 'api/xls/roster/daily/kih?df=' + _dtd, '_blank');
            ...
            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Cabin', icon: 'print', onClick: function (arg) {

                        var _dtd = (moment($scope._dailydate).format('YYYY-MM-DD'));
						//if ($rootScope.userName.toLowerCase() == 'cs.kalhori' )
                        //   $window.open('https://api2.aptaban.ir/' + 'api/xls/roster/cockpit/daily/kih?df=' + _dtd, '_blank');
						//else
							 $window.open(/*'https://api2.aptaban.ir/'*/apiDaily + 'api/xls/roster/daily/kih?df=' + _dtd, '_blank');
    ...
    /////////////////////////////
    //mixed fdp
    $scope.getRank = function (fdp) {
        var ranks = Enumerable.From(fdp.items2).Select('$.rankStr').Distinct().ToArray().join(',');
         
        return ranks;
    };
    $scope.getScheduleName = function (fdp) {
        var ranks = Enumerable.From(fdp.items2).Select('$.rankStr').Distinct().ToArray();
        if (ranks.length == 1)
            return "";
        var str = "";
        $.each(ranks, function (_i, _d) {
            
            var flts = Enumerable.From(fdp.items2).Where('$.rankStr=="' + _d + '"').Select('$.no').ToArray().join(',') + "(" + _d + ") ";
            str +="["+ flts+"]  ";
        });
        return str;
    };
