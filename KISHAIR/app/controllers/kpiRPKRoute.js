'use strict';
app.controller('kpiRPKRouteController', ['$scope', '$location', 'personService', 'authService', 'biService', '$routeParams', '$rootScope', function ($scope, $location, personService, authService, biService, $routeParams, $rootScope) {
    $scope.fieldName = '';
    $scope.indexName = '';
    $scope.ds_month = [
    ...
    $scope.rebuild = function () {
        ...
        var selectedGroupes = Enumerable.From($scope.grouped).Where(function (x) { return $scope.selectedTypes.indexOf(x.Route) != -1; }).OrderBy('$.Route').ToArray();
        //var yearRoutes = [];
        //$.each($scope.years, function (_i, _y) {
        //    $.each($scope.selectedTypes, function (_j, _r) {
        //        yearRoutes.push(_y.toString()+_r);
        //    });
        //});
        ...
        $scope.mainSeries = Enumerable.From($scope.mainSeries).Where(function (x) { return $scope.years.indexOf(x.year) != -1 && $scope.selectedTypes.indexOf(x.route)!=-1; }).ToArray();
        $scope.extraSeries = Enumerable.From($scope.extraSeries).Where(function (x) { return $scope.years.indexOf(x.year) != -1 && $scope.selectedTypes.indexOf(x.route) != -1; }).ToArray();
         $scope.mainSeries = [];
         $scope.extraSeries = [];
        console.log(selectedGroupes);
        //nool
        var c = 0; //$scope.mainSeries.length + 1;
        $.each( /*newMainSeries*/  $scope.years , function (_i, _d) {
            $.each(/*$scope.grouped*/selectedGroupes, function (_j, _t) {
                var exists = 1; //Enumerable.From($scope.mainSeries).Where('$.year==' + _d && ' $.route=="' + _t.Route + '"').FirstOrDefault();
                if (!exists || true) {
                    var rpkSeries = {
                        year: _d,
                        route: _t.Route,
    ...
    */
    ...
    $scope.year = null;
    $scope.grouped = null;
    $scope.$on('InitkpiRPKRoute', function (event, prms) {
        $scope.selectedTypes = [];
        $scope.prms = null;
        $scope.total = null;
        $scope.month = null;
        ...
        $scope.selectedTypes.push( prms.type.Route );
