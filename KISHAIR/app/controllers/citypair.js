...
app.controller('citypairController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route,$window) {
    ...
    $scope.btn_printd = {
        ...
        onClick: function (e) {
            ...
            $window.open(reportServerHttp + '?type=14&year=' + $scope.year + '&month=' + ($scope.month + 1)+"&region=DOM", '_blank');
            //$scope.bindReport();
    ...
    $scope.btn_printi = {
        ...
        onClick: function (e) {
            ...
            $window.open(reportServerHttp + '?type=14&year=' + $scope.year + '&month=' + ($scope.month + 1) + "&region=INT", '_blank');
            //$scope.bindReport();
    ...
    $scope.bind = function () {
		$scope.loadingVisible = true; 
		  flightService.getCityPair($scope.year,($scope.month+1),-1).then(function (response) {
        ...
        if (!$scope.dg_flight_ds) {
            ...
            $scope.dg_flight_ds = {
                store: {
                    type: "odata",
                    //serviceBaseV2
                    url: /*$rootScope.serviceUrl*/apireportflight2 + url,
                    //key: "Id",
                    version: 4,
                    onLoaded: function (e) {
