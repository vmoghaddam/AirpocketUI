...
app.controller('boardController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', 'fbService', '$http','$q', function ($scope, $location, $routeParams, $rootScope, $timeout, flightService, weatherService, aircraftService, authService, notificationService, $route, $window, fbService, $http,$q) {
   ...
   // $scope.IsComm = true;
    ...
    $scope.time_ir_start = {
        type: "date",
        width: '100%',
        //pickerType: $(window).width() <=1200 ? 'rollers' : '',
        interval: 15,
        onValueChanged: function (arg) {
    ...
    $scope.sb_ir_fromairport = {
        ...
        searchEnabled: true,
       // dataSource:$scope._all_apts ,//$rootScope.getDatasourceAirport(),
 ...
 $scope.btn_delete = {
    ...
    $scope.popup_delete = {
        ...
        height: 500,
        width: $(window).width() >= 1200 ? 400 : $(window).width() - 20,
        fullScreen: false,
        showTitle: true,
        dragEnabled: true,
        toolbarItems: [
    ...
    $scope.saveFree = function () {
        ...
        var std_times = (new Date($scope.time_ir_std_time)).getTimePartArray();
        var std = new Date(std_dates[0], std_dates[1], std_dates[2], std_times[0], std_times[1], 0, 0);
		std = ([141866].indexOf($scope.linkEntity.FromAirportId)!=-1) ? new Date( std.addMinutes(-30)) : std; //EVN
        ...
        var sta_times = (new Date($scope.time_ir_sta_time)).getTimePartArray();
        var sta = new Date(sta_dates[0], sta_dates[1], sta_dates[2], sta_times[0], sta_times[1], 0, 0);
		sta = ([141866].indexOf($scope.linkEntity.ToAirportId)!=-1) ? new Date( sta.addMinutes(-30)) : sta; //EVN
		...
		//newtime
		_flight.isOldSTD = $scope.newtime_ref_std_edit ? 1 : 0;
	...
	$scope.newtime_ref_std_edit = false;
    $scope.check_newtime_ref_std_edit = {
        width: '100%',
        text: "New Time (Hold reference STD)",

        bindingOptions: {
            value: 'newtime_ref_std_edit',
        }
    };
    ...
    $scope.popup_free_height = $(window).width() >= 1400 ? 770 : $(window).height() - 50;
    $scope.scroll_height = $scope.popup_free_height - 120;
    $scope.scroll_popup_free = {
        scrollByContent: true,
        scrollByThumb: true,
        bindingOptions: { height: 'scroll_height', }
    };
    $scope.popup_free = {
        ...
        width: $(window).width() >= 1300 ? 490 : $(window).width() - 50,
        fullScreen: false,
        showTitle: true,
        dragEnabled: true,
        toolbarItems: [
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'fblink', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
						...
						//console.log($scope.newtime_ref_std_edit);
	...
	$scope.newtime_ref_std = true;
    $scope.check_newtime_ref_std = {
        width: '100%',
        text: "New Time (Hold reference STD)",

        bindingOptions: {
            value: 'newtime_ref_std',
        }
    };
    ...
    $scope.popup_shift = {
        ...
        //position: { my: 'left', at: 'left', of: window, offset: '5 0' },
        height: 380,
        width: $(window).width() >= 1200 ? 330 : $(window).width() - 20,
        fullScreen: false,
        showTitle: true,
        dragEnabled: true,
        toolbarItems: [
                ...
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'fbshift', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        var dto = {
							...
							isOldSTD: $scope.newtime_ref_std ? 1 : 0,
                        };
                        $scope.loadingVisible = true;
    ...
    $scope.dep_ready = {type: "date",
	//pickerType: "rollers",
	displayFormat: "MMM-dd",
	width: '100%',interval: 5,onValueChanged: function (arg) {},bindingOptions: {value: 'logFlight.Ready',
        max: 'maxDepartureDate', min: 'minDepartureDate',}};
    $scope.dep_ready_hh = {type: "time",width: '100%',
	//pickerType: "rollers",
	displayFormat: "HHmm",interval: 15,onValueChanged: function (arg) {},
        bindingOptions: {value: 'logFlight.Ready',max: 'maxDepartureDate',min: 'minDepartureDate'}};

    $scope.dep_start = {type: "date",
	//pickerType: "rollers",
	displayFormat: "MMM-dd",
	width: '100%',interval: 5,onValueChanged: function (arg) {},bindingOptions: {value: 'logFlight.Start',
        max: 'maxDepartureDate', min: 'minDepartureDate',}};
    $scope.dep_start_hh = {type: "time",width: '100%',
	//pickerType: "rollers",
	displayFormat: "HHmm",interval: 15,onValueChanged: function (arg) {},
    ...
    $scope.popup_departure = {
        ...
        bindingOptions: {
            ...
            'toolbarItems[0].visible': 'IsSave',
			 'toolbarItems[1].visible': 'IsSave',
			  'toolbarItems[2].visible': 'IsSave',
			   'toolbarItems[3].visible': 'IsSave',
			    'toolbarItems[4].visible': 'IsSave',
				 'toolbarItems[5].visible': 'IsSave',
    ...
    $scope.popup_log = {
        ...
        toolbarItems: [
			 ...
			 //05-07
            {
			 ...
			 {
                widget: 'dxButton', location: 'after', options: {
                    type: 'default', text: 'Sec. Dep',   onClick: function (e) {

                    $window.open(apixls+ $scope.logFlight.ID, '_blank');
			 ...
			 {
                widget: 'dxButton', location: 'after', options: {
                    type: 'default', text: 'Sec. Arr.',   onClick: function (e) {
...
dto.ChrCapacity = $scope.mchr.Capacity;
 ...
 //janfe
        ...
        bindingOptions: {
            ...
            //qeshm
            'toolbarItems[10].visible': 'IsSave',
    ...
    $scope.popup_history = {
        ...
        height: $(window).width() >= 1200 ? 730 : $(window).height() - 100, //function () { return $(window).height() * 0.8 },
        fullScreen: false,
        showTitle: true,
        dragEnabled: true,
        toolbarItems: [
...
$scope.jl_fuel_pilot = {
    ...
    $scope.getFlightWidth = function (flt) {
		...
		//aidin
		if (flt.STD > flt.STDPlanned){
			duration = $scope.getDuration(new Date(flt.ChocksIn ? flt.ChocksIn : flt.STA), new Date(flt.STDPlanned));
			//console.log(flt.FlightNumber);
		}
        var w = duration * hourWidth;
        return w + "px";
    }
    $scope.getDelayStyle = function (flt) {
		...
		if (!flt.Takeoff || new Date(flt.Takeoff) <= new Date(flt.STD))
            return { width: 0 };
        var duration = $scope.getDuration(new Date(flt.Takeoff), new Date(flt.STD));
        var w = duration * hourWidth;
        return { width: w + "px" };
    };
	...
	$scope.getDelayText = function (flt) {
        if (!flt.Takeoff || new Date(flt.Takeoff) <= new Date(flt.STD))
            return "";
        var duration = $scope.getDuration(new Date(flt.Takeoff), new Date(flt.STD)) * 60;
  ...
  $window.open('https://fms.kishairlines.ir/airpocket/KishAir-FlightLog-New.mp4', '_blank');
    ...
    $scope.createGantt = function () {
        ...
        for (var i = 1; i <= days_count; i++) {
            ...
            var tbl = "<table style='padding:0;width:95%'><tr>"
                + "<td style='font-size:14px;' class='qdate'>" + moment(tempDate).format('dd DD-MMM-YYYY') + " (" + new Date(tempDate).toLocaleDateString('fa-IR-u-nu-latn', { weekday: 'long', year: 'numeric', month: '2-digit', day: 'numeric' }) + ")" + "</td>"
                + ($(window).width() < 1400 ? "<td style='font-size:14px;' class='qdate'>" + moment(tempDate).format('dd DD-MMM-YYYY') + " (" + new Date(tempDate).toLocaleDateString('fa-IR-u-nu-latn', { weekday: 'long', year: 'numeric', month: '2-digit', day: 'numeric' }) + ")" + "</td>" : "")
                + "<td style='font-size:14px;' class='qdate'>" + moment(tempDate).format('dd DD-MMM-YYYY') + " (" + new Date(tempDate).toLocaleDateString('fa-IR-u-nu-latn', { weekday: 'long', year: 'numeric', month: '2-digit', day: 'numeric' }) + ")" + "</td>"
                + "<td style='font-size:14px;' class='qdate'>" + moment(tempDate).format('dd DD-MMM-YYYY') + " (" + new Date(tempDate).toLocaleDateString('fa-IR-u-nu-latn', { weekday: 'long', year: 'numeric', month: '2-digit', day: 'numeric' }) + ")" + "</td>"
    ...
    $scope.modifyFlightTimes = function (flt, utc) {
		...
		flt.STDPlanned = moment(flt.STDPlanned);
        //zooki
        if (flt.ID == 70888 || flt.ID==70889) {
            console.log(flt.STD);
		...
		flt.STDPlanned = getOffsetDate(flt.STDPlanned, m, nFrom);
    ...
    $scope.uploader_file = {
        ...
        rtlEnabled: true,
        uploadUrl: "https://fms.kishairlines.ir/kapi/api/uploadfile",
		//uploadUrl: "https://fms.kishair.aero/kapi/api/uploadfile",
        onValueChanged: function (arg) {
 ...
 //05-07
    ...
    $scope.btn_add_chr = {
        ...
        onClick: function (e) {
            ...
            var exist = Enumerable.From($scope.dg_chr_ds).Where('Number($.ChartererId)==' + Number($scope.chr.ChartererId)).FirstOrDefault();
             if (exist) {
				 $scope.chr.Id=exist.Id;
           //     General.ShowNotify("The select CHARTERER already exists.", 'error');
           //     return;
             }
			//  console.log($scope.dg_chr_ds);
			//console.log($scope.chr.ChartererId);
			// console.log(exist);
			// alert($scope.chr.Id);
            $scope.loadingVisible = true;
            ...
            flightService.saveFlightCharterer($scope.chr).then(function (response) {
                ...
                var rec = response ;
				$scope.dg_chr_ds=Enumerable.From($scope.dg_chr_ds).Where('$.Id!='+rec.Id).ToArray();
	 ...
	 $scope.btn_edit_chr = {
        ...
        onClick: function (e) {
            ...
            if (!selected) {
             ...
             $scope.chr.ChartererId = selected.ChartererId;
            $scope.chr.Capacity = selected.Capacity;
            $scope.chr.Remark = selected.Remark;
            $scope.chr.Adult = selected.Adult;
            $scope.chr.Child = selected.Child;
            $scope.chr.Infanct = selected.Infant;
    ...
    $scope.dg_chr_columns = [
        ...
        { dataField: 'Infant', caption: 'Infant', allowResizing: true, dataType: 'number', allowEditing: false, width: 120, alignment: 'center', },
...

            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Download', icon: 'save', onClick: function (e) {
                        ...
                        $window.open('https://fms.kishairlines.ir/kapixls/' + 'api/xls/' + '?dt1=' + moment(new Date($scope.datefrom)).format('YYYY-MM-DD') + '&dt2=' + moment(new Date($scope.dateEnd)).format('YYYY-MM-DD') + "&chr=" + $scope.tt_chr + "&crew=" + $scope.tt_crew + "&cnl=" + $scope.tt_cnl + "&sort=" + $scope.tt_sort + "&time=" + $scope.tt_time+ "&sep=" + ($scope.tt_sepdate?1:0), '_blank');
                    }
                }, toolbar: 'bottom'
            },
...
//08-18
    ...
    $scope.popup_clgd_title = 'J-Log/Crew List/GD';
    $scope.popup_clgd = {
        shading: true,
...
{
                widget: 'dxButton', location: 'after', options: {
                    type: 'default', text: 'J-Log', icon: '', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        var idsStr = ids.join('_');
						console.log('jl xls',idsStr);
                        $window.open(gdapi + 'api/jl/flts/xls/' + idsStr, '_blank');
                    }
...
//2022-06-21
 ...
 $scope.selectedTabLNIndex = -1;
    ...
    $scope.popup_lognew = {
		...
		{
                widget: 'dxButton', location: 'before', options: {
                type: 'default', text: $(window).width() <= 800 ? '' : 'J-Log/Crew List/GD', icon: $(window).width() <= 800 ? 'card' : '', onClick: function (e) {
...

            {
			 ...
			 {
                widget: 'dxButton', location: 'before', options: {
                     type: 'default', text: $(window).width() <= 800 ? '' : 'Sec Dep', icon: $(window).width() <= 800 ? 'fas fa-plane-departure' : ''
                     , onClick: function (e) {
    ...
    ////////////newtime popup fields
    $scope.time_ir_oldstd_date = null;
    $scope.time_ir_oldstd = {
        type: "date",
        width: '100%',
        displayFormat: "MMM-dd",
        interval: 15,
        onValueChanged: function (arg) {},
        bindingOptions: {
            value: 'time_ir_oldstd_date',
        }
    };
    $scope.time_ir_oldstd_hh = null;
    $scope.time_ir_oldstd_hh = {
        type: "time",
        width: '100%',
        displayFormat: "HHmm",
        interval: 15,
        onValueChanged: function (arg) {},
        bindingOptions: {
            value: 'time_ir_oldstd_time',
        }
    };
   

    $scope.time_ir_newstd_date = null;
    $scope.time_ir_newstd = {
        type: "date",
        width: '100%',
        displayFormat: "MMM-dd",
        interval: 15,
        onValueChanged: function (arg) { },
        bindingOptions: {
            value: 'time_ir_newstd_date',
        }
    };
    $scope.time_ir_newstd_hh = null;
    $scope.time_ir_newstd_hh = {
        type: "time",
        width: '100%',
        displayFormat: "HHmm",
        interval: 15,
        onValueChanged: function (arg) { },
        bindingOptions: {
            value: 'time_ir_newstd_time',
        }
    };
    $scope.time_ir_newsta_date = null;
    $scope.time_ir_newsta = {
        type: "date",
        width: '100%',
        displayFormat: "MMM-dd",
        interval: 15,
        onValueChanged: function (arg) { },
        bindingOptions: {
            value: 'time_ir_newsta_date',
        }
    };
    $scope.time_ir_newsta_hh = null;
    $scope.time_ir_newsta_hh = {
        type: "time",
        width: '100%',
        displayFormat: "HHmm",
        interval: 15,
...
///////////////////
            ...
            console.log($scope.ati_flight);
			if ($rootScope.EditWindow) {
				if ((Date.now() - new Date($scope.ati_flight.ChocksOut).getTime()) / 3600000 > $rootScope.EditWindow) {
					  General.ShowNotify('You don\'t have permission to edit this flight. Ask admin to extend your edit window.', 'error');
					  return;
				  }	
			}
            ...
            $scope.time_ir_oldstd_date = new Date($scope.ati_flight.STD);
            $scope.time_ir_oldstd_time = new Date($scope.ati_flight.STD);
			
			var nFrom = 0;
			nFrom = (['EVN'].indexOf( $scope.ati_flight.FromAirportIATA)!=-1) ? +30 : 0;
							
            
            $scope.time_ir_newstd_date = $scope.ati_flight.OLDSTD ? getLocalDate(new Date($scope.ati_flight.OLDSTD).addMinutes(nFrom)) : getLocalDate(new Date($scope.ati_flight.STDPlanned).addMinutes(nFrom));
            $scope.time_ir_newstd_time = $scope.ati_flight.OLDSTD ? getLocalDate(new Date($scope.ati_flight.OLDSTD).addMinutes(nFrom)) : getLocalDate(new Date($scope.ati_flight.STDPlanned).addMinutes(nFrom));
            $scope.time_ir_newsta_date = new Date($scope.ati_flight.STA);
            $scope.time_ir_newsta_time = new Date($scope.ati_flight.STA);
            $scope.popup_newtime_visible = true;
			/////////////////
    ...
    $scope.popup_newtime_visible = false;
    $scope.popup_newtime_title = 'Set New Time';
    ...
    $scope.popup_newtime = {
        ...
        toolbarItems: [
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'fbnewtime', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        //($scope.logFlight.CLCheckIn) ? moment($scope.logFlight.CLCheckIn).format('YYYY/MM/DD HH:mm') : $scope.logFlight.CLCheckIn;
                        
                        if ((new Date($scope.time_ir_oldstd_time)).getTime() != (new Date($scope.time_ir_newstd_time)).getTime()) {
                            var dto = {
                                id: $scope.ati_flight.ID,
                                oldSTD: moment(combineDate(new Date($scope.time_ir_oldstd_time), new Date($scope.time_ir_oldstd_date), 0)).format('YYYY/MM/DD HH:mm'),
                                
                                newSTD: moment(combineDate(new Date($scope.time_ir_newstd_time), new Date($scope.time_ir_newstd_date), 0)).format('YYYY/MM/DD HH:mm'),
                                newSTA: moment(combineDate(new Date($scope.time_ir_newsta_time), new Date($scope.time_ir_newsta_date), 0)).format('YYYY/MM/DD HH:mm'),
                            };
							var nFrom = 0;
							var nTo = 0;
							nFrom = (['EVN'].indexOf( $scope.ati_flight.FromAirportIATA)!=-1) ? -30 : 0;
							nTo = (['EVN'].indexOf( $scope.ati_flight.ToAirportIATA)!=-1) ? -30 : 0;
							
							dto.oldSTD = moment(new Date(dto.oldSTD).addMinutes(nFrom)).format('YYYY/MM/DD HH:mm');
							dto.newSTD = moment(new Date(dto.newSTD).addMinutes(nFrom)).format('YYYY/MM/DD HH:mm');
							dto.newSTA = moment(new Date(dto.newSTA).addMinutes(nTo)).format('YYYY/MM/DD HH:mm');
                            ...
                            flightService.saveNewTime(dto).then(function (response) {
                                ...
                                var aflt = Enumerable.From($scope.ganttData.flights).Where('$.ID==' + response.ID).FirstOrDefault();
                                var tn = (response.NewTime ? '1' : '0') + '-' + (response.NewReg ? '1' : '0');
                                aflt.taskName = tn;
                                //console.log(response);
                                //console.log(new Date(response['STD']));
                                aflt['STD'] = new Date(response['OLDSTD']);
                                aflt['STA'] = new Date(response['STA']);
                                aflt['ChocksOut'] = new Date(response['ChocksOut']);
                                aflt['ChocksIn'] = new Date(response['ChocksIn']);
                                aflt['Takeoff'] = new Date(response['Takeoff']);
                                aflt['Landing'] = new Date(response['Landing']);
                                aflt['OLDSTD'] = new Date(response['STD']);
        ...
        bindingOptions: {
            visible: 'popup_newtime_visible',

            title: 'popup_newtime_title',

        }
    };

    //close button
    $scope.popup_newtime.toolbarItems[1].options.onClick = function (e) {
    ...
    ///////responsive gantt context menu
    $scope.cellContextMenuOptions = {
        //target: ".dx-scheduler-date-table-cell",
        //dataSource: $scope.cellContextMenuItems,
        dataSource: [
            { text: "New Time",  onItemClick2: "btn_newtime"},
            { text: "New Reg.",  onItemClick2: "btn_newreg"},
            { text: "Edit", onItemClick2: "btn_edit" },
            { text: "Shift",  onItemClick2: "btn_shift" },
            { text: "Change Reg.",  onItemClick2: "btn_creg" },
            { text: "Cancel",  onItemClick2: "btn_cnl" },
            { text: "Activate",  onItemClick2: "btn_actv" },
            { text: "Delete",  onItemClick2: "btn_delete" }
        ...
        onItemClick: function (e) {
			eval("$scope."+e.itemData.onItemClick2+".onClick();");
        }
    };
	
	//Tajil dar New Time
	$scope.getFlightSTD = function(f) {
		console.log("hello");
		if((new Date(f.STD)) > (new Date(f.STDPlanned))){
			return f.STDPlanned;
		}
		return f.STD;
	}
