...
app.controller('crewTimeReportController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route','$window', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route,$window) {
	...
	$scope.search=function(){
	 $scope.dg_flight_total_ds = null;
            ...
            $scope.getCrewFlightsTotal($scope.dt_from, $scope.dt_to.addDays(1));
	};
    $scope.btn_search = {
        text: 'Search',
        type: 'success',
	 ...
	 $scope.btn_export = {
        ...
        bindingOptions: {},
        onClick: function (e) {
           $window.open(apixlsmain +'api/xls/crew/flights/-1?df='+moment(new Date($scope.dt_from)).format('YYYY-MM-DD')+'&dt='
						+moment(new Date($scope.dt_to).addDays(1)).format('YYYY-MM-DD'), '_blank');
        }
        ...
        onSelectionChanged: function (e) {
            ...
            if (!data) {
               ...
               // $scope.dg_flight_instance.columnOption('crew', 'caption', caption);
                $scope.getCrewFlights($scope.dg_flight_total_selected.CrewId, $scope.dt_from,$scope.dt_to.addDays(1));
            }
