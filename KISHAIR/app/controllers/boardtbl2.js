'use strict';
app.controller('boardtbl2Controller', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', 'fbService', '$http', '$q', function ($scope, $location, $routeParams, $rootScope, $timeout, flightService, weatherService, aircraftService, authService, notificationService, $route, $window, fbService, $http, $q) {
    ...
    $scope.dep_ready = {
        type: "date", pickerType: "rollers", width: '100%', interval: 5, onValueChanged: function (arg) { }, bindingOptions: {
            value: 'logFlight.Ready',
            max: 'maxDepartureDate', min: 'minDepartureDate',
        }
    };
    $scope.dep_ready_hh = {
        type: "time", width: '100%', pickerType: "rollers", displayFormat: "HH:mm", interval: 15, onValueChanged: function (arg) { },
...
$scope.ds_station=['All'];
        ...
        flightService.getFlightsGanttUTC(Config.CustomerId, (new Date($scope.datefrom)).toUTCDateTimeDigits(), ed, /*offset*/0, null, 1, filter).then(function (response) {
            try {
				...
				$.each(response.flights,function(_i,_d){ 
				   ...
				   _d.STN=0;
				   if (Number(moment(new Date(_d.STD)).format('YYYYMMDD'))==tdydt)
				    _nflts.push(_d);
				});
    ...
    $scope.dg_board_columns = [
        ...
        //{ dataField: 'Remark', caption: 'Remark', allowResizing: true, dataType: 'string', allowEditing: false, },
		{caption:'PAX',columns:[
			{ dataField: 'TotalPax', caption: 'Total', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90, },
		...
		{
			caption:'BAGGAGE',columns:[
			  { dataField: 'BaggageWeight', caption: 'Weight', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90, },
			  { dataField: 'BaggageCount', caption: 'Count', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90, },
			]
		},
        { dataField: 'Delay', caption: 'DLY', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 70,fixed: true, fixedPosition: 'right' },
        
        { dataField: 'FlightStatus', caption: 'Status', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 70, fixed: true, fixedPosition: 'right' },
        ...
        { dataField: 'LinkedFlightNumber', caption: '', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, minWidth: 170, fixed: true, fixedPosition: 'right'},
