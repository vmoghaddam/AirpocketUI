(function (angular) {
    ...
    function gaugeMeterDirective(ngGauge) {
        ...
        var tpl = '<div style="display:inline-block;text-align:center;position:relative;">' +
            ...
            '<span class="gauge_value" ng-if="fractionSize === null">{{value | number}}</span>' +
            '<span class="gauge_value" ng-if="fractionSize !== null">{{value | number: fractionSize}}</span>' +
            '<u class="gauge_append">{{append}}</u></span>' +
            '<b class="gauge_label">{{ label }}</b>' +
            '<canvas></canvas></div>';
