...
app.controller('personController', ['$scope', '$location', '$routeParams', '$rootScope', 'personService', 'authService', 'notificationService', '$route', 'trnService','flightService','$window','$timeout', function ($scope, $location, $routeParams, $rootScope, personService, authService, notificationService, $route, trnService,flightService,$window,$timeout) {
    ...
    $scope.IsEditable = $rootScope.IsProfileEditable(); //$rootScope.roles.indexOf('Admin') != -1;
	 $scope.IsCoursesVisible = $rootScope.roles.indexOf('Admin') != -1 || $rootScope.userName.toLowerCase() == 'i.zamani' || $rootScope.userName.toLowerCase() == 'dehghan' 
	 || $rootScope.userName.toLowerCase() == 'khorshidi'
	  || $rootScope.userName.toLowerCase() == 'r.nasiri'
	 || $rootScope.userName.toLowerCase() == 'vatanchi'
	 || $rootScope.userName.toLowerCase() == 'a.asad'
	 || $rootScope.userName.toLowerCase() == 'mohamadi'
	 || $rootScope.userName.toLowerCase() == 'abbaspour'
	 ;
	  $scope.IsAccountEdit = $rootScope.roles.indexOf('Crew Scheduling') != -1;
   ...
   // $scope.groupDs = ['Cockpit', 'Cabin' ];
    $scope.groupDs = [];
	if ($rootScope.HasAllEmployeeView())
		$scope.groupDs = ['Cockpit', 'Cabin','All' ];
	else {
		if ($rootScope.HasCockpitView()) $scope.groupDs.push('Cockpit');
		if ($rootScope.HasCabinView()) $scope.groupDs.push('Cabin');
	}
    $scope._dg_columns2 = [
...
{ dataField: 'PID', caption: 'PID', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 130 },
 ...
 $scope.dg_columns2 = [
          ...
          { dataField: 'RemainLicence', caption: 'Licence', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90 },
           { dataField: 'RemainProficiency', caption: 'LPC', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90 },
            { dataField: 'RemainProficiencyOPC', caption: 'OPC', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90 },
             ...
             { dataField: 'RemainAvSec', caption: 'TRG07B', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 110 },
			  { dataField: 'RemainLine', caption: 'TRG02', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 110 },
              { dataField: 'RemainSMS', caption: 'SMS', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90 },
       ...
       // { dataField: 'RemainSEPT', caption: 'SEPT-T', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90 },
        ...
        { dataField: 'RemainNDT', caption: 'Line', name: 'Line', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 100 },
 ...
 { dataField: 'RemainHotWeather', caption: 'HOTWX',name:'HOTWX', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90 },
  { dataField: 'RemainColdWeather', caption: 'COLDWX',name:'COLDWX', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90 },
    ...
    $scope.sb_rankgroup = {
        ...
        searchEnabled: false,
        dataSource:$scope.groupDs, //['Cockpit','Cabin','All'],
        //readOnly:true,
        onValueChanged: function (e) {
			$scope.dg_instance.beginUpdate();
			 if (e.value == 'Cockpit') {
				...
				$scope.dg_instance.columnOption('HOTWX', 'visible', true);
				$scope.dg_instance.columnOption('COLDWX', 'visible', true);
                $scope.dg_instance.columnOption('Recurrent', 'visible', false);
				 //04-30
				   $scope.dg_instance.columnOption('SEPTT', 'visible', true);
        ...
        else
            {
				...
				$scope.dg_instance.columnOption('HOTWX', 'visible', false);
				$scope.dg_instance.columnOption('COLDWX', 'visible', false);
            }
    ...
    $scope.bind = function () {
        ...
        if (!$scope.dg_ds) {
            $scope.dg_ds = {
                store: {
                    type: "odata",
                    url: /*$rootScope.serviceUrl*/'https://fleet.flypersia.aero/apiv2beta/' + 'odata/employees/light/' + ($scope.isCrew?'crew/':'') + Config.CustomerId,
   ...
   //////////////////////////////////////////
	...
	$scope.btn_trncard = {
        ...
        onClick: function (e) {
            ...
            if (obj)
				$window.open('https://fleet.flypersia.aero/trncard/frmreportview.aspx?type=11&pid='+obj.PersonId, '_blank') ;
    ...
    $scope.refreshSMSStatus2 = function () {
        $scope.stop2();
        var ids = Enumerable.From($scope.dg_history_ds).OrderByDescending(function(x){return new Date(x.DateSent);}).Where('$.RefId').Select('$.RefId').Take(50).ToArray();
        if (!ids || ids.length == 0)
            return;
        //goh
