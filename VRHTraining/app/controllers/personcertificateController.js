...
app.controller('personcertificateController', ['$scope', '$location', '$routeParams', '$rootScope', 'courseService', 'authService', 'ztrnService','zpersonService', function ($scope, $location, $routeParams, $rootScope, courseService, authService, trnService,personService) {
    ...
    $scope.bindMainGroups = function () {
        trnService.getMainGroups().then(function (response) {
         ...
         //   ds.unshift(item);
           ...
           // console.log(ds);
            $scope.mainGroup = ds[0].FullCode2;
            //console.log(response);
    ...
    $scope.bindEmployees = function () {
        ...
        personService.getProfiles(Config.CustomerId,1,grp).then(function (response2) {
            $scope.loadingVisible = false;
             $scope.dg_employees_ds =response2.data;
 ...
 bindingOptions: {
    ...
    $scope.dg_employees_columns = [
        ...
        { dataField: 'JobGroup', caption: 'Group', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 130, fixed: true, fixedPosition: 'left', },
