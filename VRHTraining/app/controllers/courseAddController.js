...
app.controller('courseAddController', ['$scope', '$location', 'courseService', 'authService', '$routeParams', '$rootScope', 'trnService', 'ztrnService', '$http', function ($scope, $location, courseService, authService, $routeParams, $rootScope, trnService, ztrnService, $http) {
    ...
    $scope.fill_syllabi = function (item) {
        ...
        $scope.syllabi_interval = item.Interval;
    ...
    $scope.sb_syl_ins2 = {
        ...
        onSelectionChanged: function (e) {
            ...
            $scope.syllabi_instructor2_title = e.selectedItem ? e.selectedItem.Name : null;;

        },
        bindingOptions: {
            value: 'syllabi_instructor2',
            dataSource: 'ds_teachers'
        }
                ...
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'crsyllabi', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        if ($scope.new_syllabi.Id <= 0) {
                            ...
                            console.log(obj);
                            var exist = Enumerable.From($scope.entity.Syllabi).Where('$.Id==' + obj.Id).FirstOrDefault();
                            if (!exist)
                                $scope.entity.Syllabi.push(obj);
                            else {
                                exist.Title = obj.Title;
                                exist.CourseTypeId = obj.CourseTypeId;
                                exist.CurrencyId = obj.CurrencyId;
                                exist.Instructor = obj.Instructor;
                                exist.Instructor2 = obj.Instructor2;
                                exist.Duration = obj.Duration;
                                exist.Remark = obj.Remark;
                                exist.Interval = obj.Interval;
                                exist.Sessions = JSON.parse(JSON.stringify(obj.Sessions));
                            }
                            ...
                            $scope.syllabi_instructor2 = null;
                            $scope.syllabi_instructor2_title = null;
