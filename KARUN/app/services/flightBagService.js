...
serviceFactory.signDr = _signDr;
    ...
    var _epGetVRByFlight = function (flightId) {
        var deferred = $q.defer();
        $http.get(serviceSKYBAG + 'api/voyage/flight/' + flightId /*+ '?from=' + _df + '&to=' + _dt*/).then(function (response) {
...
var _getMETAR = function (flt) {
	...
	/////////////////////////////////
	 var _getOFP = function (fltid) {

        var deferred = $q.defer();
        $http.get(apiapsb + 'api/ofp/flight/report/' + fltid).then(function (response) {
            if (response.data.Id == -1)
                deferred.resolve(null);
            else
                deferred.resolve(response.data);
