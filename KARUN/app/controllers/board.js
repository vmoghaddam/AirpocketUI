...
app.controller('boardController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', 'fbService', '$http','$q','logService', function ($scope, $location, $routeParams, $rootScope, $timeout, flightService, weatherService, aircraftService, authService, notificationService, $route, $window, fbService, $http,$q,logService) {
    ...
    $rootScope.setTheme();
	$scope.IsRegChange= $rootScope.HasRegChange();
	...
	$scope.IsRegisterChange =$rootScope.roles &&  $rootScope.roles.indexOf('Aircraft Type/Register Change') != -1;
	//2024-01-08
	//alert($scope.IsStaion);
	//alert($rootScope.Station);

    $scope.IsFuelReadOnly = false;
    $scope.IsJLAccess = $rootScope.HasMenuAccess('flight_board_jl', 3) || $scope.IsStaion;
    $scope.IsCLAccess = $rootScope.HasMenuAccess('flight_board_jl', 3) || $rootScope.userName.toLowerCase().startsWith('trans.') || $scope.IsStaion || $rootScope.userName.toLowerCase().startsWith('transport');
    //ops.rezabandehlou
    $scope.IsJLOG = false;
    if ($rootScope.userName.toLowerCase() == 'ops.rezabandehlou' || $rootScope.userName.toLowerCase() == 'demo' || $rootScope.userName.toLowerCase() == 'razbani'
	   || $rootScope.userName.toLowerCase() == 'ops.askari'
        ...
        $scope.IsJLOG = true;
    $scope.IsPickup = $rootScope.userName.toLowerCase().startsWith('trans.') || $rootScope.userName.toLowerCase().startsWith('transport');
    $scope.IsCrewMobileVisible = $rootScope.userName.toLowerCase().startsWith('dis') || $rootScope.userName.toLowerCase().startsWith('demo') || $rootScope.userName.toLowerCase().startsWith('trans');
	...
	$scope.IsDelay=$scope.IsEditable || $scope.IsStaion;
    $scope.NotEditable = !$scope.IsEditable;
	$scope.NotEditableDiversion=$scope.NotEditable && !$scope.IsStaion;
 ...
 $scope.isFltEditable = true;
    $scope.btn_log = {
        ...
        onClick: function (e) {
            ...
            const inputDate = moment($scope.logFlight.ChocksOut);


             $scope.diff_date_now = inputDate.isSameOrAfter(currentDate, 'day');

	         if($rootScope.HasDispatch() == false && $scope.diff_date_now == false)
		         $scope.isFltEditable = false;
			 else
                 $scope.isFltEditable = true;
 ...
 $scope.btn_delete = {
    ...
    $scope.folderItemClick = function (key) {
        if (key == 'dr') {
            var data = { FlightId: $scope.ati_selectedFlights[0].ID, isEFBReport: false };
    ...
    $scope.btn_atcdnl = {
        ...
        onClick: function (e) {
            ...
            $window.open( 'https://airpocket.karunair.ir/upload/atc/' + $scope.atcTXT, '_blank');
        },
        bindingOptions: {
            visible:'isATCFile'
        }
    ...
    $scope.btn_fltdocdnl = {
        ...
        text: 'Download',
        onClick: function (e) {
            $window.open('https://airpocket.karunair.ir/upload/flt/' + $scope.fltdoc_url, '_blank');
        },
        bindingOptions: {
	...
	$scope._positioning=0;
	$scope.txt_positioning = {
        readOnly: true,
        bindingOptions: {
            value: '_positioning'
        }
 ...
 $scope.parkingPosition = {
    ...
    $scope.statusDs = Enumerable.From(Flight.statusDataSource).Where('$.selectable').ToArray(); //$scope.IsStaion ? $scope.stationDs : Enumerable.From(Flight.statusDataSource).Where('$.selectable').ToArray();
    $scope.sb_status = {
        showClearButton: false,
        searchEnabled: true,
...
////////////////////////////////////////////////////////
    ...
    $scope.fillLog = function (entity) {
        ...
        if ($scope.logFlight.RedirectReasonId) {
            ...
            entity.RedirectDate=entity.ChocksOut; 
...
////////////////////////////////////////////////////////
 ...
 //janfe
    ...
    $scope.getCrewAbs = function (fid) {
        //5-8
        if ( $rootScope.HideCrew() && !$rootScope.HasAccessToAddFM() && !$rootScope.HasShowPastCrews()) {
            $scope.dg_crew_abs_ds = [];
            return;
        }
		if ($rootScope.HasShowPastCrews()){
			//-08-27
			var _now=new Date((new Date()).setHours(0, 0, 0));
			var _now_num=Number(moment( _now).format('YYMMDD'));
			var _fltdate_num=Number(moment( new Date($scope.logFlight.Date)).format('YYMMDD'));
			console.log('_now',_now_num);
			console.log('_logflight',_fltdate_num);
			console.log('_com',_fltdate_num>=_now_num);
			if (_fltdate_num>=_now_num)
			{
				 $scope.dg_crew_abs_ds = [];
            return;
			}
		...
		//F/M
			 if ($rootScope.HasAccessToAddFM() && $rootScope.HideCrew()){
				 $scope.dg_crew_abs_ds =Enumerable.From( response).Where('$.Position=="F/M"').ToArray();
			 }
			 else
				  $scope.dg_crew_abs_ds = response;
			var _fm=Enumerable.From(response).Where('$.PositionId==12002').ToArray().length;
			$scope._fm=_fm;
			$scope._dsp=0;
			
			
			$scope._positioning=Enumerable.From(response).Where('$.IsPositioning').ToArray().length;
    ...
    $scope.bindFlights = function (callback) { 
        ...
        logService.getFlightsGanttUTC(Config.CustomerId, (new Date($scope.datefrom)).toUTCDateTimeDigits(), ed, /*offset*/0, null, 1, filter).then(function (response) {
			...
			if ($rootScope.HasShowPastCrews()){
			       var _now=new Date((new Date()).setHours(0, 0, 0));
		            var _now_num=Number(moment( _now).format('YYMMDDHHmm'));
		        	 
					response.flights=Enumerable.From(response.flights).Where(function(x){
						var _std=Number(moment( new Date(x.STDLocal)).format('YYMMDDHHmm'));
						return _std<_now_num;
						
						
					}).ToArray();
					console.log('-8-27', response );
...
//10-31
 ...
 const currentDate = moment();
    ...
    $scope.search = function () {
        $scope.stop();
        $scope.StopUTimer();
        $scope.bindFlights(function (prm) {
			if (prm){
				 if ($(window).width() >= 1200)
    ...
    $scope.removeCrew = function () {
	 ...
	 $scope.removeCrew_lognew = function () {
        $scope.dg_crew_lognew_selected = $rootScope.getSelectedRow($scope.dg_crew_lognew_instance);
        if (!$scope.dg_crew_lognew_selected) {
            General.ShowNotify(Config.Text_NoRowSelected, 'error');
            return;
        }
        if ($scope.dg_crew_lognew_selected.JobGroupCode.startsWith('00101') || $scope.dg_crew_lognew_selected.JobGroupCode.startsWith('00102')) {
            General.ShowNotify('The flight crew can not be removed', 'error');
            return;
        }
        var entity = { userId: $scope.dg_crew_lognew_selected.CrewId, flightId: $scope.flight.ID };
        $scope.loadingVisible = true;
        flightService.deleteNoCrewFDP(entity).then(function (response) {
            $scope.loadingVisible = false;
            $scope.getCrewAbs($scope.logFlight.ID);
...
height: function () {
            ...
            return 220;//$scope.lognew_height-400;
        },
...

            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Download', icon: 'save', onClick: function (e) {
                        ...
                        $scope.datefrom = General.getDayFirstHour(new Date(dfrom));
                        $scope.dateEnd = General.getDayLastHour(new Date(new Date(dfrom).addDays($scope.days_count - 1)));
						var tt_utcRefBoolean = ($scope.tt_utcRef == 1) ? true : false;
                         var _tturl='https://airpocket.karunair.ir/api/' + 'api/xls/' + '?dt1=' + moment(new Date($scope.datefrom)).format('YYYY-MM-DD') + '&dt2=' + moment(new Date($scope.dateEnd)).format('YYYY-MM-DD') + "&chr=" + $scope.tt_chr + "&crew=" + $scope.tt_crew + "&cnl=" + $scope.tt_cnl + "&sort=" + $scope.tt_sort + "&time=" + $scope.tt_time+"&sep=1"+"&utcRef=" + tt_utcRefBoolean;
						console.log(_tturl);
                        $window.open('https://airpocket.karunair.ir/api/' + 'api/xls/' + '?dt1=' + moment(new Date($scope.datefrom)).format('YYYY-MM-DD') + '&dt2=' + moment(new Date($scope.dateEnd)).format('YYYY-MM-DD') + "&chr=" + $scope.tt_chr + "&crew=" + $scope.tt_crew + "&cnl=" + $scope.tt_cnl + "&sort=" + $scope.tt_sort + "&time=" + $scope.tt_time+"&sep=1"+"&utcRef=" + tt_utcRefBoolean, '_blank');
                    }
                }, toolbar: 'bottom'
            },
	...
	$scope.tt_utcRef = 0;
    $scope.sb_tt_utcRef = {
        placeholder: 'Reference Time Zone',
        showClearButton: false,
        searchEnabled: false,
        dataSource: [{ Id: 0, Title: 'Local' }, { Id: 1, Title: 'UTC' }],
...
////////////////////////////////
    ...
    $scope.popup_clgd = {
        ...
        toolbarItems: [
             ...
             
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'default', text: 'Crew List', icon: 'save', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        var idsStr = ids.join('_');
                        $window.open(apigd + 'api/xls/cl/karun?flts=' + idsStr, '_blank');
			...
			{
                widget: 'dxButton', location: 'after', options: {
                    type: 'default', text: 'Dispatch Release', icon: 'save', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        var idsStr = ids.join('_');
                        $window.open(apigd + 'api/xls/drc04?flts=' + idsStr, '_blank');
...
//2022-06-21
 ...
 $scope.selectedTabLNIndex = -1;
    ...
    $scope.popup_lognew_width = 1100;
    $scope.popup_lognew_height = 670;
    $scope.popup_lognew = {
        elementAttr: {
            //  id: "elementId",
            class: "popup_lognew"
        ...
        toolbarItems: [
		{
            ...
            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Crew List/GD/DR', icon: '', onClick: function (e) {
			...
			{
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Trip Info', bindingOptions: { disabled: 'IsApproved' }, visible: true, onClick: function (arg) {


                       $window.open(apigd + 'api/xls/drc05?flts=' + $scope.logFlight.ID, '_blank');
                        /////////////////////////////////////
                    }
                }, toolbar: 'bottom'
            },
            
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: '', icon: 'save', validationGroup: 'deplog', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        //2020-11-22
						//console.log("----flt edt----", $scope.isFltEditable)
                        //if($scope.isFltEditable){
						flightService.getFlightGUID($scope.logFlight.ID).then(function (response) {
                            ...
                            if (!$scope.logFlight.TotalDelayTotalMM)
                                $scope.logFlight.TotalDelayTotalMM = 0;
							console.log($scope.logFlight.TotalDelayTotalMM);
							console.log($scope.dg_delay_ds);
							console.log(sumTotalDelayCodesAmount);
							if(sumTotalDelayCodesAmount != $scope.logFlight.TotalDelayTotalMM){
								General.ShowNotify(Config.Text_DelayCodesNETotalDelay, 'error');
                                return;
							}
...
//2024-09-23
                            ...
                            flightService.saveFlightLog(dto).then(function (response) {
								...
								_ds_pax = _ds_pax.filter(p=>p.AirportId == $scope.logFlight.FromAirport);
								
								var pax_dto={id:$scope.logFlight.ID,dto:_ds_pax};
								console.log(pax_dto);
								flightService.savePax(pax_dto).then(function (responsepx) {}, function (err) { 
								$scope.loadingVisible = true; General.ShowNotify('Flight Pax cannot be saved!', 'error'); 
								});
...
//} else{
//	General.ShowNotify("YOU ARE NOT ALLOWED TO CHANGE THE FLIGHT", 'error');
//}
                    }
                }, toolbar: 'bottom'
            },
        ...
        if (_mchr)
		  ...
		  flightService.getFlightPax($scope.flight.ID).then(function (responsepx) {
			 responsepx=Enumerable.From(responsepx).Distinct('$.Id').ToArray();
			 $scope.ds_pax =responsepx;
			 while ($scope.ds_pax.length<4) {
        ...
        bindingOptions: {
            ...
            title: 'popup_lognew_title',
			 'toolbarItems[0].visible': '!hide_crew',
			  'toolbarItems[1].visible': '!hide_crew',
            'toolbarItems[6].visible': 'IsSave',
			'toolbarItems[3].visible': 'false',


        }
    };

    $scope.hide_crew=$rootScope.HideCrew();
...
{IATA:"SAR",Id:152405},
    ...
    $scope.dg_pax = {
		...
		onRowPrepared: function(e){
			if(e.rowType === "data"){
				if(e.data.Id > 0){
						if(e.data.AirportId != $scope.logFlight.FromAirport)
		...
		onEditingStart:function(e) {
			...
			if(e.data.Id > 0)
				e.cancel = (e.data.AirportId != $scope.logFlight.FromAirport) ? true : false;
			else
				e.cancel = !e.data.FlightId ? true : false;
             ...
             },
			  {
                   ...
                   // dataField: 'Total_Pax',
					 ...
					 calculateCellValue:function(e){
						// ,isnull(RES_Adult,0)+isnull(RES_Child,0)+isnull(RES_Infant,0)
//+isnull(STN_Adult,0)+isnull(STN_Child,0)+isnull(STN_Infant,0)
//+isnull(CHR_Adult,0)+isnull(CHR_Child,0)+isnull(CHR_Infant,0)
//+isnull(FOC_Adult,0)+isnull(FOC_Child,0)+isnull(FOC_Infant,0)
//+isnull(OA_Adult,0)+isnull(OA_Child,0)+isnull(OA_Infant,0) Total_Pax
						 return isnull(e.RES_Adult)+isnull(e.RES_Child)+isnull(e.RES_Infant)
						 +isnull(e.STN_Adult)+isnull(e.STN_Child)+isnull(e.STN_Infant)
						 +isnull(e.CHR_Adult)+isnull(e.CHR_Child)+isnull(e.CHR_Infant)
						 +isnull(e.FOC_Adult)+isnull(e.FOC_Child)+isnull(e.FOC_Infant)
						 +isnull(e.OA_Adult)+isnull(e.OA_Child)+isnull(e.OA_Infant)
						 ;
					 },
		...
		onRowUpdated:function(e){
			//alert(e.data.RES_Adult);
			//e.data.Total_Pax=123;
			//2023-12-15
			//console.log(e);
			//console.log($scope.ds_pax);
			$scope._calculate_pax();
		},
	 ...
	 $scope._calculate_pax=function(){
		  $scope.logFlight.PaxAdult=0;
		  $scope.logFlight.PaxChild=0;
		  $scope.logFlight.PaxInfant=0;
		  $scope.logFlight.BaggageWeight=0;
		  $scope.logFlight.CargoWeight=0;
		  $scope._fsg=0;
		  $scope._acm=0;
		  $scope._moc=0;
		  $scope.logFlight.NightTime=0;
		  
		 $.each($scope.ds_pax,function(_i,_d){
			  $scope.logFlight.PaxAdult+=isnull(_d.RES_Adult)+isnull(_d.CHR_Adult)+isnull(_d.STN_Adult)+isnull(_d.FOC_Adult)+isnull(_d.OA_Adult);
			$scope.logFlight.PaxChild+=isnull(_d.RES_Child)+isnull(_d.CHR_Child)+isnull(_d.STN_Child)+isnull(_d.FOC_Child)+isnull(_d.OA_Child);
			$scope.logFlight.PaxInfant+=isnull(_d.RES_Infant)+isnull(_d.CHR_Infant)+isnull(_d.STN_Infant)+isnull(_d.FOC_Infant)+isnull(_d.OA_Infant);
			$scope.logFlight.BaggageWeight+=isnull(_d.Baggage);
		  $scope.logFlight.CargoWeight+=isnull(_d.Cargo);
		  $scope._fsg+=isnull(_d.FSG);
		  $scope._acm+=isnull(_d.ACM);
		  $scope._moc+=isnull(_d.MOC);
		   $scope.logFlight.NightTime+=isnull(_d.WCR);
    ...
    $scope.$on('$viewContentLoaded', function () {
        //2020-11-16
		var _xmsg='کاربر گرامی با توجه به منقضی شدن لایسنس استفاده از نرم افزار ایرپاکت دسترسی شما قطع خواهد شد لطفا از اطلاعات خود نسخه پشتیبان تهیه فرمایید';
		//	  alert(_xmsg);
