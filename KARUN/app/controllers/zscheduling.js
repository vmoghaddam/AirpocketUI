'use strict';
app.controller('zschedulingController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'schedulingService', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, $timeout, schedulingService, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
    //06-13
    ...
    $scope.btn_timeline = {
        ...
        onClick: function (e) {
            ...
            $window.open('https://fleet.flypersia.aero/airpocket/#!/duty/timeline/z', '_blank');
        }
    ...
    $scope.btn_report = {
        ...
        onClick: function (e) {
            ...
            //05-11
            //$window.open('https://api.aptaban.ir/' + 'api/xls/cl?flts=' + idsStr, '_blank');
            $scope.popup_report2_visible = true;
        }
                    ...
                    type: 'default', text: 'Visible', onClick: function (arg) {
                        ...
                        General.Confirm('Are you sure?', function (res) {
                            if (res) {
                                ...
                                $scope.loadingVisible = true;
                                schedulingService.dutiesVisible(dto).then(function (response) {
                                    $scope.loadingVisible = false;
                                    $.each(selected, function (_i, _d) {
                    ...
                    type: 'default', text: 'Hide', onClick: function (arg) {
                        ...
                        General.Confirm('Are you sure?', function (res) {
                            if (res) {
                                ...
                                var dto = { Ids: ids, Date: new Date(), UserName: $rootScope.userName };
                                $scope.loadingVisible = true;
                                schedulingService.dutiesHide(dto).then(function (response) {
                                    $scope.loadingVisible = false;
                                    $.each(selected, function (_i, _d) {
                                        if (_d.DateConfirmed) { _d.IsConfirmed = null; _d.DateConfirmed = null; }
...

            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Send', validationGroup: "smsmessage", onClick: function (arg) {
                        ...
                        schedulingService.sendSMS(dto).then(function (response) {
                            $scope.loadingVisible = false;
    ...
    $scope.btn_fx = {
        ...
        onClick: function (e) {
            ...
            if ($scope.ati_selectedFlights && $scope.ati_selectedFlights.length > 0) {
                var fltId = $scope.ati_selectedFlights[0].Id;

                var flt = Enumerable.From($scope.ati_flights).Where('$.Id==' + fltId).FirstOrDefault();
                $scope.fxFlight = flt;
                $scope.smsRecsKeys = Enumerable.From(flt.crew).Select('Number($.Id)').ToArray();
                console.log('FIX', $scope.fxFlight);

                persianDate.toLocale('en');
                var newwc = new persianDate(new Date($scope.fxFlight.STD)).format("YYYY-MM-DD");
                var prts = newwc.split('-');
                $scope.fxyear = Number(prts[0]);
                //$scope.fxPeriodDs = [
                ...
                //];
                var _m = Number(prts[1]);
                var _d = Number(prts[2]);
                var _t = null;
                if (_m == 12) {
                    if (_d < 16) _t = 11; else _t = 1;
                }
                else {
                    if (_d < 16)
                        _t = _m;
    ...
    $scope.dsFxType = [
        ...
        { id: 300010, title: 'OTHER AIRLINE STANDBY' },
    ...
    ///////////////
    //10-21
    $scope.popup_grpevent_visible = false;
    $scope.popup_grpevent_title = '';
    $scope.popup_grpevent = {
        ...
        toolbarItems: [
            ...
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', validationGroup: "eventaddgrp", onClick: function (arg) {
                        ...
                        var dto = {};
                        dto.ids = Enumerable.From($scope.dg_smscrew_selected).Select('$.Id').ToArray();
                        dto.type = 5000;
                        dto.df = moment(new Date($scope.FromDateEventGrp)).format('YYYYMMDD');
                        dto.dt = moment(new Date($scope.FromDateEventGrp)).format('YYYYMMDD');
                        dto.tf = moment(new Date($scope.FromDateEventGrp)).format('HHmm');
                        dto.tt = moment(new Date($scope.ToDateEventGrp)).format('HHmm');

                        console.log(dto);
                        $scope.loadingVisible = true;

                        schedulingService.saveEventGroup(dto).then(function (response) {
                            $scope.loadingVisible = false;
                            General.ShowNotify(Config.Text_SavedOk, 'success');
                        ...
                        if ($scope.fxType != 300007 && $scope.fxType != 300010 && !$scope.fx_hh && !$scope.fx_mm) {
                            General.ShowNotify(Config.Text_FillRequired, 'error');
                            return;
                        }
                        if (($scope.fxType == 300007 || $scope.fxType == 300010) && !$scope.fx_cnt) {
                            General.ShowNotify(Config.Text_FillRequired, 'error');
                            return;
                        }
                        ...
                        dto.Count = 0;
                        if ($scope.fxType == 300007 || $scope.fxType == 300010) {
                            dto.HH = 0;
                            dto.MM = 0;
        ...
        bindingOptions: {
            visible: 'popup_grpevent_visible',

            title: 'popup_grpevent_title',
    ...
    $scope.btn_grpevent = {
        ...
        onClick: function (e) {
            ...
            $scope.bindSMSRecs();
            $scope.popup_grpevent_visible = true;
    ...
    $scope.FromDateEventGrp = null;
    $scope.ToDateEventGrp = null;
    $scope.RemarkEventGrp = null;





    $scope.date_from_eventgrp = {
        type: "date",
        width: '100%',
        ...
        bindingOptions: {
            value: 'FromDateEventGrp',

        }
    };
    $scope.date_to_eventgrp = {
        type: "date",
        width: '100%',
        ...
        bindingOptions: {
            value: 'ToDateEventGrp',

        }
    };
    $scope.date_from_eventgrp_hh = {
        type: "time",
        width: '100%',
    ...
    $scope.remark_eventgrp = {
        height: 60,
        bindingOptions: {
            value: 'RemarkEventGrp',
    ...
    $scope.getDaySTBYs = function (year, month, day, callback) {
        ...
        $scope.loadingVisible = true;
        schedulingService.getDutiesByDay('stby', year, month, day).then(function (response) {
            $scope.loadingVisible = false;
            //var row = {
            //    CrewId: crewid,
            //    DateStartYear: year,
    ...
    $scope.ReservedDs = [];
    //06-13
    $scope.getFilteredStbyDs = function (ds) {
        if ($scope.IsCockpit && $scope.IsCabin)
            return ds;
        if ($scope.IsCabin)
            return Enumerable.From(ds).Where("$.JobGroupCode.startsWith('00102')").ToArray();
        if ($scope.IsCockpit)
            return Enumerable.From(ds).Where("$.JobGroupCode.startsWith('00101')").ToArray();
    };
    $scope.setAmPmDs = function (day, type) {
        var prts = (new Date(day)).getDatePartArray();
        console.log('stby', $scope.ds_crew);
        $scope.getDaySTBYs(prts[0], prts[1] + 1, prts[2], function (data) {
            ...
            $scope.dg_crew_stby_ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                return stbyCrews.indexOf(x.Id) == -1 && (($scope.IsCabin && x.JobGroupCode.startsWith('00102')) || ($scope.IsCockpit && x.JobGroupCode.startsWith('00101')));
    ...
    //06-13
    //2023 stby
	 $scope.addStby2024 = function (_type,_from,_end) {
       
        $scope.selected_crew = Enumerable.From($scope.crews).Where('$.CrewId==' + $scope.selected_crew_id).FirstOrDefault();

        var _cell_start = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        var _cell_end = new Date(_cell_start).addMinutes((23 * 60) + 59);
        if ($scope.cell_duties_rest && $scope.cell_duties_rest.length > 0) {
            _cell_start = (new Date($scope.cell_duties_rest[0].InitRestTo)).addMinutes(1);
        }

         

        var dto = {
            date: new Date(_cell_start),
            crewId: $scope.dg_calcrew_selected.Id,
            type: _type,
            isgantt: 0,
            date_from: moment(new Date(_from)).format("YYYYMMDDHHmm"),
            date_end: moment(new Date(_end)).format("YYYYMMDDHHmm"),
        };
    ...
    $scope.addStby = function (_crew, _type) {
        ...
        schedulingService.saveSTBY(dto).then(function (response) {
            $scope.loadingVisible = false;
             console.log('stby add',response);
            if (response.Code == 406) {
                if (response.message) {
                    var myDialog = DevExpress.ui.dialog.custom({
                        rtlEnabled: true,
                        title: "Error",
                        message: _crew.ScheduleName+": "+ response.message,
                        buttons: [{ text: "OK", onClick: function () { } }]
                    });
                    myDialog.show();
                }
    ...
    $scope.bindDailyReport = function () {
        ...
        $scope.loadingVisible = true;
        schedulingService.getRosterSheetReport(_dt).then(function (response) {
            $scope.loadingVisible = false;
    ...
    $scope.bindDailyReportFP = function () {
        ...
        //odata/crew/report/main?date=' + _dt
        $scope.loadingVisible = true;
        schedulingService.getRosterSheetReportFP(_dt).then(function (response) {
            $scope.loadingVisible = false;
    ...
    $scope.getCrewDuties = function (callback) {
        ...
        $scope.loadingVisible = true;
        schedulingService.getCrewDuties(_df, _dt, ca, co).then(function (response) {
            $scope.loadingVisible = false;
            $.each(response, function (_i, _d) {
    ...
    $scope.dg_cduties_columns = [
        ...
        { dataField: 'JobGroup', caption: 'Rank', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 100 },
        { dataField: 'DateLocal', caption: 'Date Start', allowResizing: true, alignment: 'center', dataType: 'date', format: 'yy-MM-dd', allowEditing: false, width: 150 },
        { dataField: 'DateEndLocal', caption: 'Date End', allowResizing: true, alignment: 'center', dataType: 'date', format: 'yy-MM-dd', allowEditing: false, width: 150 },
        //DateEndLocal
    ...
    $scope.cal_change = function () {
        ...
        $scope.getCrewFDPs(prts[0], prts[1] + 1, crewid, function (data) {
             $.each(data,function(_i,_d){
				 ...
				 _d.End2=_d.End;
				 if ([1165,300051].indexOf(_d.DutyType)!=-1){
					 _d.End2=General.getDayLastHour(new Date(new Date(_d.Start)));
				 }
			 });
            $scope.cal_crew_ds = data;
			console.log('2024-01-31',$scope.cal_crew_ds);
        });
    ...
    $scope.getCrewFDPs = function (year, month, crewid, callback) {
        ...
        if (year, month, crewid) {
            ...
            if (!data) {
                $scope.loadingVisible = true;
                schedulingService.getCrewDutiesByYearMonth(crewid, year, month).then(function (response) {
                    $scope.loadingVisible = false;
                    var row = {
                        CrewId: crewid,
                        DateStartYear: year,
    ...
    $scope.dg_calcrew_columns = [
        // { dataField: 'Selected', caption: '', allowResizing: true, alignment: 'center', dataType: 'boolean', allowEditing: false, width:45},
        { dataField: 'ScheduleName', caption: 'Schedule Name', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, sortIndex: 1, sortOrder: 'asc' },
        { dataField: 'JobGroup', caption: 'GRP', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 70 },
		{ dataField: 'ValidTypes', caption: 'TYP', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 70 },
        { dataField: 'GroupOrder', caption: 'O', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 120, visible: false, sortIndex: 0, sortOrder: 'asc' },
        {
            dataField: "Id", caption: '',
            width: 55,
	...
	//2024-01-20
	$scope.assign300013 = function (e) {
        $scope.event_status = 300013;
        $scope.FromDateEvent = (new Date($scope.contextMenuCellData.startDate)).setHours(6, 0, 0, 0);
        $scope.ToDateEvent = (new Date($scope.FromDateEvent)).addMinutes((11 * 60) + 59);
        $scope.popup_event_title = 'STBY C';
        $scope.popup_event_visible = true;
    };
	...
	//Positioning
	$scope.assign300050 = function (e) {
        $scope.event_status = 300050;
        $scope.FromDateEvent = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        $scope.ToDateEvent = (new Date($scope.FromDateEvent)).addMinutes(0);
        $scope.popup_event_title = 'Positioning';
        $scope.popup_event_visible = true;
    };
	$scope.assign300051 = function (e) {
        $scope.event_status = 300051;
        $scope.FromDateEvent = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        $scope.ToDateEvent = (new Date($scope.FromDateEvent)).addMinutes(23 * 60 + 59);
        $scope.popup_event_title = 'Note';
        $scope.popup_event_visible = true;
    };
	$scope.apt_from_event=null;
	$scope.apt_to_event=null;
	$scope.airline_event=null;
	$scope.ticket_no=null;
	 $scope.sb_from_event = {
        showClearButton: true,
        searchEnabled: true,
    ...
    $scope.cellContextMenuItems = [
     ...
     //   { visible: !$scope.OnlyRoster, text: 'STBY AM', onItemClick: $scope.assign1168, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-1168' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>STBY AM</td></tr></table>", },
      //  { visible: !$scope.OnlyRoster, text: 'STBY PM', onItemClick: $scope.assign1167, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-1167' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>STBY PM</td></tr></table>", },
		...
		{ text: 'STBY C', onItemClick: $scope.assign300013, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-300013' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>STBY C</td></tr></table>", },
        ...
        { visible: !$scope.OnlyRoster, text: 'OFF', onItemClick: $scope.assign1166, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-10000' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>OFF</td></tr></table>", },
...
//soltani
		...
		{ visible: !$scope.OnlyRoster, text: 'Positioning', onItemClick: $scope.assign300050, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-300050' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>Positioning</td></tr></table>", },
		{ visible: !$scope.OnlyRoster, text: 'Note', onItemClick: $scope.assign300051, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-300051' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>Note</td></tr></table>", },
		...
		//{ visible: !$scope.OnlyRoster, text: 'Duty AM', onItemClick: $scope.assign300052, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-300052' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>Duty AM</td></tr></table>", },
		//{ visible: !$scope.OnlyRoster, text: 'Duty PM', onItemClick: $scope.assign300053, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-300053' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>Duty PM</td></tr></table>", },
    ...
    $scope.dblclick = function (cell) {
        ...
        if (cdate >= bdate && cdate <= edate)
            return;
        var eventFrom = (new Date(cdate)).setHours(0, 0, 0, 0);
        var eventEnd = (new Date(eventFrom)).setHours(23, 59, 59, 0);
    ...
    $scope.cal_crew = {
        ...
        //    console.log('SCH SCHD SDFKKSDF ', itemData);
		...
		onAppointmentRendered:function(e){
			...
			var ap_date=moment(new Date(e.appointmentData.Start)).format('YYYYMMDD');
			var aps=Enumerable.From($scope.cal_crew_ds).Where(function(x){return moment(new Date(x.Start)).format('YYYYMMDD')==ap_date;}).Select('$.Id').ToArray();
			//alert(aps.indexOf(e.appointmentData.Id));
			var idx=aps.indexOf(e.appointmentData.Id);
			var ty=idx*7;
			//moment(new Date(e.appointmentData.Start)).format('YYYYMMDD')
			console.log('ad',e.appointmentData);
			var parent=$(e.appointmentElement) ;
			var matrix = $(parent).css("transform");  
    ...
    $scope.createEvent = function (_crew, _type, _typeTitle, eventFrom, eventEnd, remark) {
		var _now=Number(moment(new Date()).format('YYYYMMDD')) ;
		var _cell=Number(moment(new Date(eventFrom)).format('YYYYMMDD')) ;
	 
		 
        if ([300007, 300010].indexOf(_type) == -1 && _cell < _now && !$scope.isAdmin) {

            var myDialog = DevExpress.ui.dialog.custom({
                rtlEnabled: true,
                title: "Error",
                message: "You cannot edit the content of selected cell.Please contact the administrator.",
                buttons: [{ text: "OK", onClick: function () { } }]
            });
            myDialog.show();
            return;
    ...
    $scope.removeDutyCal = function (duty) {
        ...
        var dto = { fdp: duty.Id,Id:duty.Id };
    ...
    $scope.saveNewDutyCal = function (crewid, callback) {
		...
		if ($scope.event_status==300050){
			
			dto.apt_from=$scope.apt_from_event;
			dto.apt_to=$scope.apt_to_event;
			dto.airline=$scope.airline_event;
			dto.ticket_no=$scope.ticket_no;
        ...
        schedulingService.saveDuty(dto).then(function (response) {
            ...
            response.dutyType = response.DutyType;
			 ...
			 response.End2=response.End;
				 if ([1165,300051].indexOf(response.DutyType)!=-1){
					 response.End2=General.getDayLastHour(new Date(new Date(response.Start)));
				 }
                    ...
                    type: 'success', text: 'Ok', icon: 'check', validationGroup: 'eventadd2', onClick: function (arg) {
                        ...
                        if ($scope.event_status == 10000) {
                            ...
                            if (check) {
								...
								//2024-01-20
								var _end=Number (moment(new Date($scope.get_event_to())).format('YYYYMMDDHHmm')) ;
							    var _end_u=Number (moment(new Date(eventEnd)).format('YYYYMMDDHHmm')) ;
							    //alert(_end);
								//alert(_end_u);
							    if (_end>_end_u){
								   General.ShowNotify('The defined duration does not satisfy RERRP conditions.', 'error');
                                  return;
							    }
						...
						else if ($scope.event_status == 300013) {
                            $scope.addStby2024(300013, eventFrom, eventEnd);
                        }
                        else if ($scope.event_status == 1170) {
                            $scope.addStby2024(1170, eventFrom, eventEnd);
                        }
						else if ($scope.event_status == 1168) {
                            $scope.addStby2024(1168, eventFrom, eventEnd);
                        }
						else if ($scope.event_status == 1167) {
                            $scope.addStby2024(1167, eventFrom, eventEnd);
                        }
                        else if ($scope.event_status == 5000 || $scope.event_status == 5001 || $scope.event_status==300014
                            || $scope.event_status == 100001 || $scope.event_status == 100003 
                        ) {
                            //nool
                    ...
                    type: 'success', text: 'Ok', icon: 'check', validationGroup: 'datevisible', onClick: function (arg) {
                        ...
                        General.Confirm('Are you sure?', function (res) {
                            if (res) {
                                ...
                                $scope.loadingVisible = true;
                                schedulingService.dutiesVisibleByDates(dto).then(function (response) {
                                    $scope.loadingVisible = false;
                                    $scope.getCrewDuties(function (ds) {
                                        $scope.dg_cduties_ds = ds;
    ...
    //2024-01-20
	$scope.event_from_changed=function(){
		//ex48
		if ($scope.event_status==10000){
			
			
			
			var start_h=Number(moment(new Date($scope.FromDateEvent)).format('HH'));
			var start_m=Number(moment(new Date($scope.FromDateEvent)).format('mm'));
			if ( !$scope.ex48){
				if (start_h>=18){
				     var  _end = (new Date($scope.FromDateEvent)).addMinutes(36 * 60);
				     $scope.ToDateEvent=_end;
			    }
			    else
			    {
                   ...
                   //var _lnr = 18 * 60 - (_start.Hour * 60 + _start.Minute);
				    ...
				    var  _end = (new Date($scope.FromDateEvent)).addMinutes(36 * 60);
				    var _lnr = 18 * 60 - (start_h * 60 + start_m);
				    _end = (new Date(_end)).addMinutes(_lnr);
				    $scope.ToDateEvent=_end;
			...
			{
				 var  _end = (new Date($scope.FromDateEvent)).addMinutes(48 * 60);
                 var _lnr = 24 * 60 - (start_h * 60 + start_m);
                  _end = (new Date(_end)).addMinutes(_lnr);
				 $scope.ToDateEvent=_end;
			}
    ...
    $scope.getFlightClass = function (flt) {
		...
		if (flt.FlightStatusID == 6)
		{   cls += ' inactive'; console.log('inactive');}
		else {
    ...
    $scope.bindFlights = function (callback) {
        ...
        schedulingService.getFlightsGantt(Config.CustomerId, (new Date($scope.datefrom)).toUTCDateTimeDigits(), ed, offset, null, filter).then(function (response) {
            try {
                $scope.loadingVisible = false;
                $scope.tabsdatefirst = true;
                $scope.tabs_date = [];
    ...
    $scope.FDPClick = function (f) {
        ...
        if (_id != f.FLTGroupId) {
            ...
            var els = [];
            var fids = [];
            $.each(f.items, function (_i, _d) {
                var el = $("div[data-flightid='" + _d.FlightId + "']");
                els.push(el.get(0));
                el.addClass('selected');
                fids.push(_d.FlightId);
            });
            ...
            $.each(selected, function (_i, _d) {
                ...
                $scope.ati_selectedFlights.push({
                    ...
                    , std: new Date($d.data('std'))
                    , offblock: new Date($d.data('blockoff'))
                    , onblock: new Date($d.data('blockon'))
                    , no: $d.data('no'), FromAirport: $d.data('from'), ToAirport: $d.data('to')
                });
    ...
    $scope.StartUTimer = function () {
        ...
        $scope.utimer = setTimeout(function () {
            ...
            schedulingService.getUpdatedFlights(dto).then(function (response) {
    ...
    $scope.formatCellDatePersian = function (dt, duty) {
        var _start = moment(new Date(dt)).format('DD');
        if (_start == duty.text) {
            persianDate.toLocale('en');
            return new persianDate(new Date(dt)).format("DD-MM-YY");
        }
        else {
            var _s = (new Date(dt)).addDays(1);
            persianDate.toLocale('en');
            return new persianDate(new Date(_s)).format("DD-MM-YY");
    ...
    $scope.getCrewAbs = function (fid) {
        ...
        $scope.loadingVisible = true;
        schedulingService.getFlightCrews(fid).then(function (response) {
            $scope.loadingVisible = false;
    ...
    $scope.setSelectedFlightsKey = function () {
        console.log('SELECTED KEY', $scope.ati_selectedFlights);
        if (!$scope.ati_selectedFlights || $scope.ati_selectedFlights.length == 0) {
            $scope.selectedFlightsKey = null;
            ...
            //2022-01-23
            var _fs = Enumerable.From($scope.ati_selectedFlights).OrderBy(function (x) { return new Date(x.onblock); }).ToArray();
            //old
        ...
        //   alert($scope.selectedFlightsKey.Id);
        //   alert($scope.selectedFlightsKey.no);
    };
        ...
        $.each(flts, function (_i, _f) {
            $.each(_f.crew, function (_j, _c) {
                ...
                if (exist)
                    ...
                    var _x = { Id: _c.Id, count: 1, rank: _c.rank, name: _c.name, order: _c.order, };
                    var _xc = Enumerable.From($scope.ds_crew).Where('$.Id==' + _c.Id).FirstOrDefault();
                    if (_xc)
                        _x.group = _xc.JobGroup;
                    table.push(_x);
                }
            });
        });
    ...
    $scope.doExtend = function (y) {
        ...
        schedulingService.saveFDP(fdp).then(function (response) {
            $scope.loadingVisible = false;
            if (response.config)
    ...
    $scope.extendFDP = function (y) {
        ...
        //4-27
        //if ($scope.FDPStat.IsOver && !$scope.isAdmin) {
        ...
        //4-27
        //if ($scope.FDPStat.WOCLError == 1 && !$scope.isAdmin) {
        ...
        if ($scope.FDPStat.IsOver && !$scope.isAdmin) {
            if ($scope.FDPStat.AllowedExtension == 0 || !$scope.useExtension) {
                ...
                General.Confirm('The FDP is OVER. Are you sure?', function (res) {
                    if (res) {
            ...
            if ($scope.selectedPos.rank != 'OBS') {
                ...
                if (crew.JobGroupCode.startsWith('00101')) {
                    ...
                    if (!crew.RemainLPR || crew.RemainLPR < 0) {
                        var _mestr = 'LPR ' + (crew.LPRExpired ? moment(crew.LPRExpired).format('YYYY-MM-DD') : 'UNKNOWN');
                        var _myDialog = DevExpress.ui.dialog.custom({
                            rtlEnabled: true,
                            title: "Confirm",
                            message: _mestr,
                ...
                if (crew.JobGroupCode.startsWith('00101'))
                    if (!crew.RemainSEPT || crew.RemainSEPT < 0) {
                        ...
                        expired.push('SEPT-T ' + (crew.SEPTExpired ? moment(crew.SEPTExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
                    }

                if (!crew.RemainSEPTP || crew.RemainSEPTP < 0) {

                    expired.push('SEPT-P ' + (crew.SEPTPExpired ? moment(crew.SEPTPExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
                }
    ...
    $scope.doFDP = function (crew) {
        ...
        //$scope.ati_flights
        //2022-01-23
        var ordered = Enumerable.From($scope.ati_selectedFlights).OrderBy(function (x) { return new Date(x.onblock); }).ToArray();
        //old
        //var ordered = Enumerable.From($scope.ati_selectedFlights).OrderBy(function (x) { return new Date(x.sta); }).ToArray();
        ...
        fdp.key2 = _tmp.join('_');
        //2022-01-14
        if ($scope.useExtension || $scope.useSplit)
            // fdp.Extension = $scope.FDPStat.AllowedExtension;
            fdp.Extension = Math.round($scope.FDPStat.Extended);
        else
            fdp.Extension = 0;
        //alert($scope.useExtension);
        // alert(fdp.Extension);
        console.log('FDP STAT', $scope.FDPStat);
        $scope.loadingVisible = true;
        ...
        schedulingService.saveFDP(fdp).then(function (response) {
            $scope.loadingVisible = false;
			var _code=response.config? response.data.Code:response.Code;
              console.log('SAVE FDP',response);
            if (_code == 406) {
                if (response.data.message) {
                    var myDialog = DevExpress.ui.dialog.custom({
                        rtlEnabled: true,
                ...
                if (_code == 501) {
                    ...
                    General.Confirm("The selected crew is on STANDBY. Do you want to activate him/her?", function (res) {
                        if (res) {
                            ...
                            $scope.activeStby(crew, response.data.data.Id, fdp.rank, fdp.index, fdp);
                            //personService.delete(dto).then(function (response) {
    ...
    $scope.checkConflict2 = function (flights) {
        ...
        $.each(flights, function (_i, _d) {
            ...
            var f = Enumerable.From(flights).Where(function (x) {
                return x.Id != _d.Id && (
                    (new Date(x.offblock) >= new Date(_d.offblock) && new Date(x.offblock) <= new Date(_d.onblock))
                    ||
                    (new Date(x.onblock) >= new Date(_d.offblock) && new Date(x.onblock) <= new Date(_d.onblock))
                );
            }).FirstOrDefault();
            if (f)
                hasConflict = true;
    ...
    $scope.fillFilteredCrew = function () {
        ...
        var filtered = Enumerable.From(_ds).Where(function (x) {
            ...
            if ($scope.searchStr) {
                fltr = fltr && x.ScheduleName.toLowerCase().startsWith($scope.searchStr.toLowerCase());
            }
            return fltr;
    ...
    $scope.getRosterFDPs = function (callback) {
        ...
        $scope.ati_offitems = [];
        $scope.loadingVisible = true;
        schedulingService.getRosterFDPs(_df, _dt).then(function (response) {

            schedulingService.getOffItems(_df, _dt).then(function (response2) {
                $scope.loadingVisible = false;
                $.each(response, function (_i, _d) {
                    $scope.ati_fdps.push(_d);
                });
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        $('.scheduling').fadeIn(400, function () {
            ////////////////////////////////
            setTimeout(function () {
                $scope.bindFlights(function () {
                    $scope.fillCrew();
    ...
    $scope.bindDutyTimeLine = function () {
        ...
        schedulingService.getDutyTimeLine(2021, 2, 'P1', -1).then(function (response) {
...
$scope.num_rev = {
    ...
    $scope.popup_report2_visible = false;
    $scope.popup_report2_title = 'Daily Report';
    $scope.popup_report2_other = {
        elementAttr: {
            //  id: "elementId",
            class: "popup_reports"
        },
        ...
        toolbarItems: [
            ...
            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Download', icon: 'print', onClick: function (arg) {

                        var _dtd = (moment($scope._dailydate).format('YYYY-MM-DD'));
                        //if ($rootScope.userName.toLowerCase() == 'cs.kalhori' )
                        $window.open(serviceBaseAPI + 'api/xls/roster/all/daily/dr02?df=' + _dtd+'&rev='+$scope.report_rev, '_blank');
                        //else
                        //	 $window.open('https://api.apvaresh.ir/' + 'api/xls/roster/daily?df=' + _dtd, '_blank');
...

            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Default', icon: 'print', onClick: function (arg) {
                        ...
                        var _dtd=(moment($scope._dailydate).format('YYYY-MM-DD'));
						  $window.open($rootScope.reportServerCrew + '?type=15&date='+_dtd+'&rev='+$scope._rev, '_blank');
            ...
            },
            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Security', icon: 'print', onClick: function (arg) {
                          ...
                          var _dtd=(moment($scope._dailydate).format('YYYY-MM-DD'));
  $window.open($rootScope.reportServerCrew + '?type=16&date='+_dtd+'&rev='+$scope._rev, '_blank');
                    }
                }, toolbar: 'bottom'
            },
