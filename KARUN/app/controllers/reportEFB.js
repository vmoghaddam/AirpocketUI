...
$scope.btn_dispatch = {
        ...
        onClick: function (e) {
           var data = { FlightId: $scope.selectedFlight.FlightId, isEFBReport: true };
           $rootScope.$broadcast('InitDrAdd', data);
                },
bindingOptions: {
            disabled: '!selectedFlight'
    ...
    $scope.btn_jl = {
        ...
        onClick: function (e) {
           ...
           // $scope.bindJL(flt.FlightId);
		   	  var _url = 'https://airpocket.karunair.ir/apiapsb/api/jl/v2/xls/' + flt.FlightId;
                     // window.open(_url);
					 setTimeout(() => {
...
$scope.jlObj = null;
    ...
    function printElem($elem,ofp) {
        ...
        frameDoc.document.write('</head><body style="padding:20px;font-size:12px !important; -webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;">');
   ...
   /////////////////////////
    ...
    $scope.$watch("selectedTabIndex", function (newValue) {
        ...
        try {
            ...
            $('#' + id).fadeIn();
$scope.is_print_visible=$scope.selectedTabId =='ofp';
            switch (id) {
                case 'calendar':
                    $scope.bindCrew();
                    break;
	...
	//08-19
	 $scope.formatDateTimeOFP = function (dt) {
        return moment(new Date(dt)).format('HH:mm');
    };
	 $scope.formatDateOFP = function (dt) {
		// console.log('formatDateOFP',dt);
	...
	$scope.bindOFP =function(flightId){
		flightBagService.getOFP(flightId).then(function (resdata) {
			 var data=resdata.Data;
			 $scope.requested_fuel = $scope.selectedFlight.FuelPlanned;
        ...
        data.alt2_wts_grp = Enumerable.From(data.alt2_wts)
            ...
            .GroupBy(function (item) { return item.WayPoint; }, null, (key, g) => {
			 ...
			 $scope.ofp_data=data;
			 //alert($scope.ofp_data.root.AircraftType);
			 console.log('***** OFP *****',$scope.ofp_data);
    ...
    $scope.bindOFP_OLD = function (flightId) {
        $scope.url_sign = null;
        $scope.PIC = null;
        $scope.signDate = null;
        $scope.OFP = null;
...
$scope.stations = [];
    ...
    $scope.bindStations = function () {
		...
		console.log('-----bind station-----', $scope.selectedFlight)
