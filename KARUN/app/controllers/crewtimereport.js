...
app.controller('crewTimeReportController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route','$window', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route,$window) {
	...
	$scope.search=function(){
	 $scope.dg_flight_total_ds = null;
            ...
            $scope.getCrewFlightsTotal($scope.dt_from, new Date( $scope.dt_to).addDays(0));
	};
    $scope.btn_search = {
        text: 'Search',
        type: 'success',
    ...
    $scope.getCrewFlights = function (id, df, dt) {
        ...
        flightService.getCrewFlights(id, df, dt).then(function (response) {
            ...
            $.each(response, function (_i, _d) {
				...
				_d.IsDiverted=_d.FlightStatusID==7 || _d.FlightStatusID==17;
    ...
    $scope.dg_flight_total_columns = [
		  ...
		  { dataField: 'Type', caption: 'Type', allowResizing: true, alignment: 'center', dataType: 'string', width: 90, allowEditing: false, fixed: true, fixedPosition: 'left' },
{ dataField: 'HomeBase', caption: 'H/B', allowResizing: true, alignment: 'center', dataType: 'string', width: 90, allowEditing: false, fixed: true, fixedPosition: 'left' },
				...
				{ dataField: 'Diverted', caption: 'DIV', allowResizing: true, dataType: 'number', allowEditing: false, width: 80, alignment: 'center', },
    ...
    $scope.dg_flight_total = {
		...
		onRowPrepared: function (e) {
			...
			if (e.data && e.data.ERRS>0)
                e.rowElement.css('background', '#ff9999');
			//#ff9999
    ...
    $scope.dg_flight_columns = [
        {
				...
				{ dataField: 'IsDiverted', caption: 'DIV', allowResizing: true, alignment: 'center', dataType: 'boolean', allowEditing: false, width: 60 },
...
{ dataField: 'Register', caption: 'Reg.', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 100, fixed: true, fixedPosition: 'left' },
    ...
    $scope.dg_flight = {
        ...
        onRowPrepared: function (e) {
			...
			if (e.data && e.data.IsDiverted)
                e.rowElement.css('background', '#ffd480');
			if (e.data && e.data.SITATime>0)
                e.rowElement.css('background', '#ff9999');
			//#ff9999
