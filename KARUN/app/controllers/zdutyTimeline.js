    ...
    $scope.getCrew = function (callback) {
        ...
        schedulingService.getCrewForGanttByDateNew(_code, '', _dt).then(function (response) {
            ...
            if ($scope.ac_type != 'ALL') {
                response = Enumerable.From(response).Where('$.ac_type.includes("' + $scope.ac_type + '")').ToArray();
            }
...
//		   result.RemainRecurrent = !item.RecurrentExpired ? null : $scope.getDaysDiffMoment(new Date(item.RecurrentExpired), _dt);
 ...
 $scope.get_ac_type = function (message, rank) {
        ...
        if (type_part.includes('310') || type_part.includes('320') || type_part.includes('330') || type_part.includes('340'))
		 ...
		 if (type_part.includes('F100') )
            result.push('F100');
		if (type_part.includes('F50') )
            result.push('F50');
 ...
 $scope.ac_type = 'ALL';
    $scope.sb_type = {
        ...
        showClearButton: false,
        searchEnabled: false,
        dataSource: ['ALL','F100','F50','MD', '737', 'AIRBUS', 'UNKNOWN'],
