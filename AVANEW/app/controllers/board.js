...

            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Download', icon: 'save', onClick: function (e) {
                        ...
                        $scope.datefrom = General.getDayFirstHour(new Date(dfrom));
                        $scope.dateEnd = General.getDayLastHour(new Date(new Date(dfrom).addDays($scope.days_count - 1)));
                         var _tturl='https://aerotango.app/api/' + 'api/xls/' + '?dt1=' + moment(new Date($scope.datefrom)).format('YYYY-MM-DD') + '&dt2=' + moment(new Date($scope.dateEnd)).format('YYYY-MM-DD') + "&chr=" + $scope.tt_chr + "&crew=" + $scope.tt_crew + "&cnl=" + $scope.tt_cnl + "&sort=" + $scope.tt_sort + "&time=" + $scope.tt_time+"&sep=1";
						console.log(_tturl);
                        $window.open('https://aerotango.app/api/' + 'api/xls/' + '?dt1=' + moment(new Date($scope.datefrom)).format('YYYY-MM-DD') + '&dt2=' + moment(new Date($scope.dateEnd)).format('YYYY-MM-DD') + "&chr=" + $scope.tt_chr + "&crew=" + $scope.tt_crew + "&cnl=" + $scope.tt_cnl + "&sort=" + $scope.tt_sort + "&time=" + $scope.tt_time+"&sep=1", '_blank');
                    }
                }, toolbar: 'bottom'
            },
