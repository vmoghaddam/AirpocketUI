'use strict';
app.controller('expiringGroupController', ['$scope', '$location', '$routeParams', '$rootScope', 'courseService', 'authService', 'trnService','ztrnService', function ($scope, $location, $routeParams, $rootScope, courseService, authService, trnService,ztrnService) {
    ...
    $scope.dg_maingroup = {
        ...
        onSelectionChanged: function (e) {
            ...
            if (!data) {
                ...
                $scope.bindGroups(data.JobGroupMainCode, "-1");
    ...
    $scope.dg_group = {
        ...
        onSelectionChanged: function (e) {
            ...
            if (!data) {
                ...
                //$scope.bindMainGroups(data.TypeId);
                $scope.bindCourseType(data.JobGroupMainCode, data.JobGroupCode2, -1);
            }
    ...
    $scope.dg_course_columns = [
        { dataField: 'Name', caption: 'Name', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, minWidth: 300, fixed: false, fixedPosition: 'left' },
        { dataField: 'JobGroup', caption: 'Group', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 150, fixed: false, fixedPosition: 'left' },
        { dataField: 'CertificateType', caption: 'Type', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 200, fixed: false, fixedPosition: 'left' },
    ...
    $scope.dg_course = {
        ...
        onCellPrepared: function (e) {
            ...
            //    $scope.styleCell(e, e.data.Remain);
            //}
        },
    ...
    $scope.bind = function () {
       ...
       // if (!$scope.isDepManager)
            $scope.bindMainGroups($scope._mainGroup);
       /* else {
            if (!$scope.employee) {
                trnService.getEmployee($rootScope.employeeId).then(function (response) {
                    $scope.employee = response;
                    $scope._mainGroup = $scope.employee.JobGroupMainCode;
                    $scope.bindMainGroups($scope._mainGroup);
    ...
    $scope.bindMainGroups = function (code) {
        $scope.ds_maingroup = [];
        $scope.ds_group = [];
        $scope.ds_employee = [];
        $scope.loadingVisible = true;

        trnService.getMainGroupsExpiring(code).then(function (response) {
            $scope.loadingVisible = false;
			...
			if  (!$scope.isManager){
				...
				ztrnService.getManagerGroups( $rootScope.employeeId).then(function (response2) {
			        $scope.ds_maingroup =Enumerable.From(response).Where(function(x){ return response2.indexOf(x.JobGroupMain)!=-1;}).ToArray();
    ...
    $scope.bindGroups = function (main,group) {

        $scope.ds_group = [];
        $scope.ds_employee = [];
        $scope.loadingVisible = true;

        trnService.getGroupsExpiring(main, group).then(function (response) {
            $scope.loadingVisible = false;
    ...
    $scope.bindCourseType = function (main, group,type) {

        $scope.ds_main = [];
        $scope.ds_employee = [];
        $scope.loadingVisible = true;

        trnService.getGroupsExpiringCourseTypes(main, group,type).then(function (response) {
            $scope.loadingVisible = false;
            $.each(response, function (_i, _d) {
                _d.UnknownCount = _d.EmployeesCount - _d.ValidCount - _d.ExpiredCount - _d.ExpiringCount;
            });
    ...
    $scope._bindMainGroups = function (type) {
        $scope.ds_maingroup = [];
        $scope.ds_group = [];
        $scope.ds_employee = [];
        $scope.loadingVisible = true;
    ...
    $scope._bindGroups = function (type, group) {
    ...
    $scope.btn_employees = {
        ...
        bindingOptions: {},
        onClick: function (e) {
            var selected = $rootScope.getSelectedRow($scope.dg_main_instance);
            if (!selected) {
