'use strict';
app.controller('fuelReportController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route','fuelService', 
function ($scope, $location, $routeParams, $rootScope, flightService, weatherService, aircraftService, authService, notificationService, $route,fuelService) {
 ...
 $scope.dg_columnsX = [
        ...
        { dataField: 'P2Name', caption: 'P2', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 150, fixed: false, fixedPosition: 'left' },
...

        {
            ...
            columns: [
               ...
               // { dataField: 'Taxi', caption: 'Taxi', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 80, },
				...
				{ dataField: 'DifTotalFuel', caption: 'Diff OFP Total', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 120,   },
				{ dataField: 'DifTotalFuelPercent', caption: '%', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 70,   },

                //   { dataField: 'FPVar', caption: 'Diff OFP Trip', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90, customizeText: function (e) { return e.value ? e.value + ' %'  : '0' } },
    ...
    $scope.dg = {
        ...
        onCellPrepared: function (e) {
			 ...
			 if (e.rowType === "data" && e.column.dataField == "DifTripFuelPercent" && e.data.DifTripFuelPercent >0)
                e.cellElement.css("backgroundColor", "#ffcccc");
			else  if (e.rowType === "data" && e.column.dataField == "DifTripFuelPercent" && e.data.DifTripFuelPercent <0)
			      e.cellElement.css("backgroundColor", "#99ffeb");
			  
			  
			  
			  if (e.rowType === "data" && e.column.dataField == "DifTotalFuelPercent" && e.data.DifTotalFuelPercent >0)
                e.cellElement.css("backgroundColor", "#ffcccc");
			else  if (e.rowType === "data" && e.column.dataField == "DifTotalFuelPercent" && e.data.DifTotalFuelPercent <0)
			      e.cellElement.css("backgroundColor", "#99ffeb");
            ...
            if (e.rowType === "data" && e.column.dataField == "UsedFuel" && e.data.UsedFuel > e.data.FPTripFuel)
                e.cellElement.css("backgroundColor", "#ffcccc");
			else  if (e.rowType === "data" && e.column.dataField == "UsedFuel" && e.data.UsedFuel < e.data.FPTripFuel)
			      e.cellElement.css("backgroundColor", "#99ffeb");
			  
            if (e.rowType === "data" && e.column.dataField == "FuelTotal" && e.data.FuelTotal > e.data.FPFuel)
                e.cellElement.css("backgroundColor", "#ffcccc");
			else if (e.rowType === "data" && e.column.dataField == "FuelTotal" && e.data.FuelTotal < e.data.FPFuel)
			      e.cellElement.css("backgroundColor", "#99ffeb");
...
$scope.formatMinutesXXX = function (mm) {
    ...
    $scope.bind = function () {
        ...
        var _dt = moment(dt).format('YYYY-MM-DD');
		 $scope.loadingVisible = true;
        fuelService.getFuelReport(_df,_dt).then(function (response) {
                ...
                store: {
                    ...
                    onLoaded: function (e) {
                       $.each(e, function (_i, _d) {
                            _d.BlockTime2 = $scope.formatMinutesXXX(_d.BlockTime);
                            _d.FlightTime2 = $scope.formatMinutesXXX(_d.FlightTime);
							_d.RampFuel=(_d.Remaining?_d.Remaining:0)+(_d.UpLift?_d.UpLift:0);
