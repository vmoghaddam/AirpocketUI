...
app.controller('expiringCourseTypeController', ['$scope', '$location', '$routeParams', '$rootScope', 'courseService', 'authService', 'trnService', function ($scope, $location, $routeParams, $rootScope, courseService, authService, trnService) {
 ...
 $scope.isDepManager = $rootScope.HasDepartmentManager();
    ...
    $scope.dg_course_columns = [
        { dataField: 'Name', caption: 'Name', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, width: 270, fixed: false, fixedPosition: 'left' },
        { dataField: 'JobGroup', caption: 'Group', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 100, fixed: false, fixedPosition: 'left' },
    ...
    $scope.btn_employees = {
        ...
        onClick: function (e) {
			...
			console.log('sdfsdfsdfsdfsdfsdf',selected);
            if (!selected) {
                General.ShowNotify(Config.Text_NoRowSelected, 'error');
                return;
            }
            $scope.bindCourseTypeEmployee(/*selected.TypeId*/selected.CertificateTypeId, selected.JobGroupId);
