...
$scope.active = 1;
    $scope.sb_active = {
        showClearButton: false,
        searchEnabled: true,
        dataSource: [{ Id: -1, Title: 'InActive' }, { Id: 1, Title: 'Active' },],
        displayExpr: "Title",
        valueExpr: 'Id',
    ...
    $scope.dg_course_columns = [
        ...
        { dataField: 'JobGroup', caption: 'Group', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, width: 100, fixed: true, fixedPosition: 'left' },
        { dataField: 'LastName', caption: 'LastName', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, width: 170, fixed: true, fixedPosition: 'left' },
        { dataField: 'FirstName', caption: 'FirstName', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, width: 120, fixed: true, fixedPosition: 'left' },
        ...
        { dataField: 'No', caption: 'ClassID', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 90, fixed: true, fixedPosition: 'left' },
		{ dataField: 'SexId', caption: 'Count', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90, fixed: true, fixedPosition: 'left' },
        { dataField: 'Status', caption: 'Status', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 130, },
        ...
        { dataField: 'Organization', caption: 'Organization', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 130, },
        { dataField: 'Instructor1', caption: 'Instructor', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 130, },
        { dataField: 'Instructor2', caption: 'Instructor 2', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 130, },
        { dataField: 'TrainingDirector', caption: 'Director', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 130, },
        ...
        { dataField: 'CoursePeopleStatus', caption: 'Result', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 100, fixed: false, fixedPosition: 'right' },
        { dataField: 'DateIssue', caption: 'Issue', allowResizing: true, alignment: 'center', dataType: 'datetime', allowEditing: false, format: 'yyyy-MMM-dd', width: 120, fixed: false, fixedPosition: 'right' },
        ...
        { dataField: 'Remains', caption: 'Rem.', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 90, fixedPosition: 'right', fixed: false },
        //04-30
    ...
    $scope.dg_course = {
        ...
        onCellClick: function (e) {
            ...
            if (clmn.name == "ImageUrl" && e.data.ImageUrl)
		     ...
		     $window.open($rootScope.clientsFilesUrl + e.data.ImageUrl, '_blank');
    ...
    $scope.bind = function () {
        if ($scope.selectedTab.id == 'course') {
            ...
            $scope.loadingVisible = true;
             trnService.getTrnStatCoursePeople(_df, _dt, $scope.coursetype ? $scope.coursetype : -1, result, $scope.status ? $scope.status : -1, '-1', '-1', ($scope.inst1 ? $scope.inst1 : -1), ($scope.inst2 ? $scope.inst2 : -1), ($scope.rank ? $scope.rank : 1), ($scope.active ? $scope.active : 1), ($scope.grp ? $scope.grp:-1),$scope.isManager?"1":"-1").then(function (response) {
                $scope.loadingVisible = false;
	...
	if (!$scope.isManager){
        ...
        store: {
            type: "odata",
            url: $rootScope.serviceUrl + 'odata/base/jobgroups/' + '1',
	...
	{
		 ztrnService.getManagerGroups( $rootScope.employeeId).then(function (response2) {
			       $.each(response2,function(_i,_d){
					   $scope.group_ds.push({TitleFormated:_d, Remark:_d});
					    
					   
				   })
				   $scope.grp = response2[0];
         }, function (err) { $scope.loadingVisible = false; General.ShowNotify(err.message, 'error'); }); 
	}
