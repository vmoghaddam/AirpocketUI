'use strict';
app.controller('zschedulingController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'zschedulingService', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, $timeout, schedulingService, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
    //06-13
    $scope.IsCabin = true;
    ...
    $scope.btn_timeline = {
        ...
        onClick: function (e) {
            ...
            $window.open('#!/duty/timeline/z', '_blank');
        }
...
console.log('SAVE FDP',response);
            if (response.Code == 406 || (response.data && response.data.Code == 406)) {
                ...
                if (response.Code == 501 || (response.data && response.data.Code == 501)) {
                    General.Confirm("The selected crew is on STANDBY. Do you want to activate him/her?", function (res) {
                        if (res) {
                    ...
                    if (response.Code == 304 || (response.data && response.data.Code == 304)) {
                        ...
                        if (response.Code == 308 ||   (response.data && response.data.Code == 308)) {
                            var myDialog = DevExpress.ui.dialog.custom({
                                rtlEnabled: true,
                                title: "Error",
                                message: "RERRP Error.",
    ...
    $scope.fillFilteredCrew = function () {
        ...
        switch (id) {
            ...
            case 'P1':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                    return (x.JobGroup == 'TRE' || x.JobGroup == 'TRI' || x.JobGroup == 'LTC') || ((x.JobGroup == 'P1'));
                }).ToArray();
                break;
            case 'P2':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
            ...
            case 'OBS':
                ...
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                    return ($scope.IsCabin && (x.JobGroup == 'CCM' || x.JobGroup == 'SCCM' || x.JobGroup == 'ISCCM')) || ($scope.IsCockpit && (x.JobGroup == 'P2' || x.JobGroup == 'P1' || x.JobGroup == 'TRE' || x.JobGroup == 'TRI' || x.JobGroup == 'LTC'));
                }).ToArray();
                break;
            case 'CHECK':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
...

            {
                widget: 'dxButton', location: 'before', options: {
                    type: 'default', text: 'Download', icon: 'print', onClick: function (arg) {
                        ...
                        $window.open('https://xapi.apvaresh.ir/' + 'api/xls/roster/all/daily?df=' + _dtd, '_blank');
                        //else
