'use strict';
app.controller('trnScheduleCourseTypeController', ['$scope', '$location', '$routeParams', '$rootScope', 'courseService', 'authService', 'ztrnService', '$window', '$compile', function ($scope, $location, $routeParams, $rootScope, courseService, authService, trnService, $window, $compile) {
    ...
    $scope.get_schedulecrs_ds = function () {
		...
		var mng_id=$scope.isManager?$rootScope.employeeId:-1;


        trnService.getTrnScheduleYear(y,mng_id).then(function (response) {
            $scope.loadingVisible = false;
    ...
    $scope.get_scheduleyear_ds = function () {
        ...
        trnService.getTrnYearScheduleType(y, t,mng_id).then(function (response) {
            $scope.loadingVisible = false;
            console.log(response);
    ...
    $scope.get_schedulemonth_ds = function (m,t,callback) {
        ...
        var mng_id=$scope.isManager?$rootScope.employeeId:-1;
        //var t = $scope.coursetypeid ? $scope.coursetypeid : -1;
        trnService.getTrnYearMonthScheduleType(y,m, t,mng_id).then(function (response) {
            $scope.loadingVisible = false;
            console.log(response);
            callback(response);
    ...
    $scope.get_schedulemonthcrs_ds = function (m, callback) {
        ...
        var mng_id=$scope.isManager?$rootScope.employeeId:-1;
        //var t = $scope.coursetypeid ? $scope.coursetypeid : -1;
        trnService.getTrnSchedule(y, m,mng_id).then(function (response) {
            $scope.loadingVisible = false;
            console.log(response);
            callback(response);
    ...
    $scope.get_scheduleyear_emp_ds = function () {
		if (!$scope.dg_employees_selected)
			return;
         $scope.loadingVisible = true;
        var y = $scope.schedule_year_emp.year;
    ...
    $scope.rank = '';
	/*var ranks=['Cockpit', 'Cabin', 'F/D', 'GRND', 'COMM', 'CAMO', 'MAINTENANCE', 'TRAINING', 'LEGAL', 'QA'
            , 'CATERING'
            , 'SECURITY'
            , 'SUPPORT'
            , 'ADMINISTRATIVE'
            ...
            , 'IT'
            , 'All'];*/
	var ranks=	['Cockpit','Cabin','F/D','GRND','COMM','CAMO','MAINTENANCE','TRAINING','LEGAL','QA','FINANCIAL','HR','IT','SECURITY','MANAGEMENT',
	'FLIGT_OPS_STAFF',
	'ENG_AND_MAINT_STAFF',
	'MNG_STAFF',
	'SECURITY_STAFF',
    ...
    $scope.sb_rank = {
	 ...
	 if ($scope.isManager){ 
	    ...
	    trnService.getManagerGroups( $rootScope.employeeId).then(function (response) {
            ...
            $scope.loadingVisible = false;
            console.log('manager grps',response);
            //$scope.schedule_crs_ins.dataSource = response;
			$scope.ds_rank=response;
        }, function (err) { $scope.loadingVisible = false; General.ShowNotify(err.message, 'error'); });
    ...
    $scope.preparePeopleGrid = function () {
        ...
        trnService.getCoursePeopleSessions($scope.selectedCourse.Id).then(function (response) {
            ...
            $.each(response.Data.people, function (_i, _d) {
              if ($scope.ds_rank.indexOf(_d.JobGroupRoot)!=-1 || !$scope.isManager)
			 {
				 console.log('person',_d);
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        setTimeout(function () {
            $scope.rank =$scope.ds_rank && $scope.ds_rank.length>0? $scope.ds_rank[0]:'';
        }, 1200);
