...
app.factory('authService', ['$http', '$q', 'localStorageService', 'ngAuthSettings', '$location', '$rootScope', function ($http, $q, localStorageService, ngAuthSettings, $location, $rootScope) {
    ...
    var _login = function (loginData) {
        ...
        $http.post(/*serviceBase*/zauth + 'token', data, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }).then(function (response) {
            console.log('token');
            ...
            localStorageService.set('userData', { Name: responseData.Name, UserId: responseData.UserId, EmployeeId: responseData.EmployeeId, JobGroup: responseData.JobGroup,Position: responseData.Position });
    ...
    var _changePassword = function (entity) {
        var deferred = $q.defer();
        $http.post( /*$rootScope.serviceUrl*/zauth  + 'api/Account/ChangePassword', entity).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
