...
app.factory('generalService', ['$http', '$q', 'ngAuthSettings', '$rootScope', function ($http, $q, ngAuthSettings, $rootScope){
    ...
    ordersServiceFactory.getEmployeeCertificates = _getEmployeeCertificates;
	 ...
	 var _getTrainingCard = function () {
        var id=$rootScope.userId*2+1237;
        var deferred = $q.defer();
        $http.get('https://fleet.flypersia.aero/ztrn/' + 'api/trainingcard/' + id).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    ordersServiceFactory.getTrainingCard = _getTrainingCard;
    //////////////////////////////
