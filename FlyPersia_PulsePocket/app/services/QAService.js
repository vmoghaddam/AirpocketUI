...
app.factory('QAService', ['$http', '$q', 'ngAuthSettings', '$rootScope', function ($http, $q, ngAuthSettings, $rootScope) {
	...
	var _saveFeedBack = function (entity) {
        entity.Feedback= "ضمن سپاس از ارسال گزارش، پس از بررسی  نتیجه اقدامات انجام شده در صورت لزوم به شما ابلاغ خواهد شد. با تشکر، مدیریت ایمنی و تضمین کیفیت هواپیمایی فلای پرشیا";
		console.log(entity);
		
		var ananymous=entity.Anonymous;
		
		//entity.Mobile
        var deferred = $q.defer();
        $http.post(apiQA +'api/qa/save/feedback', entity).then(function (response) {
			if ( ananymous!=1){
			//$http.get('https://apimsg.apvaresh.com/api/qa/feedback/first/'+entity.Mobile+'/'+$rootScope.employeeId).then(function (xresponse) {
