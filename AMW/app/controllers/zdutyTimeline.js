...
app.controller('zdutyTimelineController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', 'zschedulingService', function ($scope, $location, $routeParams, $rootScope, $timeout, flightService, weatherService, aircraftService, authService, notificationService, $route, $window, schedulingService) {
   ...
   $scope.OnlyRoster = false;
    ...
    $scope.getDateCellStyle = function (_id) {
        var bcolor = ($scope._selected_cell_id && _id == $scope._selected_cell_id) ? '#eeffcc' : '';
        return {
...
$scope.destroy_menu();
    ...
    $scope.getCrewCerStatus = function (c, dt) {
		...
		if (item.Id==4759 && dt.caption=='Dec-22')
		{
			console.log('getCrewCerStatus',item);
			//console.log(dt);
		}
        ...
        if (!result.RemainCMC || result.RemainCMC<0)
		...
		if (item.Id==4759 && dt.caption=='Dec-22')
		 console.log('getCrewCerStatus status',result);
        return result;
    };
    ...
    $scope.bindDutyTimeLine = function (callback) {
        ...
        $scope.getCrew(function (crews) {
            ...
            $scope.getDuties(_df, _dt, function (dts) {
                ...
                $scope.resources_filtered = crews;
                console.log('______WQSSDD_____',crews);
 ...
 $scope.ac_type = 'ALL';
    ...
    $scope.btn_search = {
	 ...
	 $scope.btn_export = {
        ...
        onClick: function (e) {
             ...
             //serviceBaseAPI
			  var _df  = (moment(new Date($scope.dt_from)).format('YYYY-MM-DD'));
			   var _dt  = (moment(new Date($scope.dt_to)).format('YYYY-MM-DD'));
                        //if ($rootScope.userName.toLowerCase() == 'cs.kalhori' )
                        $window.open(serviceBaseAPI + 'api/xls/sch01?df='+_df+'&dt='+_dt  , '_blank');
...

            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Ok', icon: 'check', validationGroup: 'eventadd2', onClick: function (arg) {
                        ...
                        //////////////////////////////
                        if ($scope.event_status == 300013) {
                            $scope.addStby(300013, eventFrom, eventEnd);
                        }
                        else if ($scope.event_status == 1170) {
                            $scope.addStby(1170, eventFrom, eventEnd);
                        }
						else if ($scope.event_status == 1168) {
                            $scope.addStby(1168, eventFrom, eventEnd);
                        }
						else if ($scope.event_status == 1167) {
                            $scope.addStby(1167, eventFrom, eventEnd);
                        }
                        else if ($scope.event_status == 10000) {
                            //nool
    ...
    $scope.assign10000 = function (e) {
       ...
       // if (e && moment(e).format('YYYYMMDD') == moment(new Date($scope.contextMenuCellData.startDate)).format('YYYYMMDD'))
        ...
        if ($scope.cell_duties_rest && $scope.cell_duties_rest.length > 0) {
            $scope.FromDateEvent = (new Date($scope.cell_duties_rest[0].InitRestTo)).addMinutes(1);
        }
        $scope.ToDateEvent = _cell_end; //(new Date($scope.FromDateEvent)).addMinutes((23 * 60) + 59);
       ...
       // $scope.ToDateEvent = (new Date($scope.FromDateEvent)).addMinutes((23 * 60) + 59);
			...
			try{$scope.popup_event_visible = true;} catch(eeee){alert(eeee);}
    ...
    $scope.assign300008 = function (e) {
        ...
        //$scope.ToDateEvent = (new Date($scope.FromDateEvent)).addMinutes(23 * 60 + 59);
        var _cell_start = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        var _cell_end =(new Date($scope.contextMenuCellData.startDate)).setHours(23, 59, 0, 0); //new Date(_cell_start).addMinutes((23 * 60) + 59);
    ...
    $scope.assign1168_old = function (e) {
	  ...
	  $scope.assign1168 = function (e) {
        ...
        var _cell_start = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        var _cell_end = new Date(_cell_start).addMinutes((11 * 60) + 59);
        $scope.FromDateEvent = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        if ($scope.cell_duties_rest && $scope.cell_duties_rest.length > 0) {
            $scope.FromDateEvent = (new Date($scope.cell_duties_rest[0].InitRestTo)).addMinutes(1);
        }
        $scope.ToDateEvent = _cell_end;
        $scope.popup_event_title = 'STBY AM';
        $scope.popup_event_visible = true;
    };
    $scope.assign1167_old = function (e) {
	...
	$scope.assign1167 = function (e) {
        ...
        //$scope.FromDateEvent = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        //$scope.ToDateEvent = (new Date($scope.FromDateEvent)).addMinutes(24 * 60);
        var _cell_start = (new Date($scope.contextMenuCellData.startDate)).setHours(12, 0, 0, 0);
        var _cell_end = new Date(_cell_start).addMinutes((11 * 60) + 59);
        $scope.FromDateEvent = (new Date($scope.contextMenuCellData.startDate)).setHours(12, 0, 0, 0);
        if ($scope.cell_duties_rest && $scope.cell_duties_rest.length > 0) {
            $scope.FromDateEvent = (new Date($scope.cell_duties_rest[0].InitRestTo)).addMinutes(1);
 ...
 $scope.assign100007 = function (e) {
        ...
        $scope.FromDateEvent = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        $scope.ToDateEvent = (new Date($scope.FromDateEvent)).addMinutes(23 * 60+59);
        $scope.popup_event_title = 'NO FLIGHT';
        $scope.popup_event_visible = true;
    ...
    $scope.assign300013 = function (e) {
        ...
        //$scope.FromDateEvent = (new Date($scope.contextMenuCellData.startDate)).setHours(0, 0, 0, 0);
        //$scope.ToDateEvent = (new Date($scope.FromDateEvent)).addMinutes(24 * 60);
        var _cell_start = (new Date($scope.contextMenuCellData.startDate)).setHours(6, 0, 0, 0);
        var _cell_end = new Date(_cell_start).addMinutes((11 * 60) + 59);
        $scope.ToDateEvent = _cell_end;
    ...
    $scope.prepare_fdp_inserting = function () {
        ...
        switch ($scope.selected_crew.JobGroup) {
            ...
            case 'CCE':
            case 'CCI':
                $scope.position_ds = ['CCI', 'SCCM', 'CHECK', 'OBS', 'D/H'];
                $scope.default_pos = 'CCI';
                break;
            case 'SCCM':
            case 'SCC':
    ...
    $scope.visit=0; 
	$scope.do_new_event=function(key, id){
    ...
    $scope.new_event = function (key, id) {
		/* $scope.visit++;
         ...
         $scope.$apply(function () {
            ...
            $scope.do_new_event(key, id);

         });
		 else {    $scope.do_new_event(key, id); }
    ...
    //11-18
    $scope.addStby = function (_type,_from,_end) {
    ...
    $scope.doFDP = function () {
        ...
        schedulingService.getFDPIndex(dto.key2, dto.rank).then(function (responsex) {
            ...
            schedulingService.saveFDP(dto).then(function (response) {
                ...
                if ([413, 412, 428, 314, 307, 328,308].indexOf(response.Code)!=-1 || (response.data && [413, 412, 428, 314, 307, 328,308].indexOf(response.data.Code)!=-1  )) {
                    var myDialog = DevExpress.ui.dialog.custom({
                        rtlEnabled: true,
                        title: "Error",
                        message: response.data.message,
    ...
    /////////////////////////////////////////////////
    //menux
	$scope.destroy_menu=function(){
		
		$.contextMenu( 'destroy', '.context-menu-one' );
		$.contextMenu( 'destroy','.obj-duty' );
	};
                ...
                items:
                {
                  ...
                  //  "sep1004": "---------",
                   ...
                   // "sep1": "---------",
                    ...
                    "REST": {
                        name: "<span style='font-weight: bold'>REST</span>",
                        isHtmlName: true, icon: "add"
                    },
                    "OFFICE": {
                   ...
                   // "sep4": "---------",
                    ...
                    "GROUND": {
					 ...
					 "REFUSE": {
                        name: "<span style='font-weight: bold'>REFUSE</span>",
                        isHtmlName: true, icon: "add"
                    },
					 "POSITIONING": {
                        name: "<span style='font-weight: bold'>POSITIONING</span>",
                        isHtmlName: true, icon: "add"
                    },
					 "NOTE": {
                        name: "<span style='font-weight: bold'>NOTE</span>",
                        isHtmlName: true, icon: "add"
                    },
					 "NO FLIGHT": {
                        name: "<span style='font-weight: bold'>NO FLIGHT</span>",
                        isHtmlName: true, icon: "add"
                    },
    ...
    $scope.fillFilteredCrew = function () {
        $scope.resources_filtered = Enumerable.From($scope.ganttData.resources).Where(function (x) {
            return x.ScheduleName.toLowerCase().indexOf($scope.searchStr.toLowerCase()) != -1;
        }).ToArray();
    }
    $scope.text_search = {
        placeholder: 'Search',
    ...
    $scope._selected_cell_id = null;
    $scope._cell_click = function (x, dt, duties, evt) {
        var cell_date = moment(new Date(dt.date)).format("YYYY-MM-DD");
        $scope.cell_duties_rest = Enumerable.From(duties).Where(function (x) {
            return moment(new Date(x.InitRestTo)).format("YYYY-MM-DD") == cell_date ;
        }).OrderByDescending(function (x) { return Number(moment(new Date(x.InitRestTo)).format("YYYYMMDDHHmm")); }).ToArray();

        $scope.cell_duties = Enumerable.From(duties).Where(function (x) {
            return moment(new Date(x.InitEnd)).format("YYYY-MM-DD") == cell_date;
        }).OrderByDescending(function (x) { return Number(moment(new Date(x.InitEnd)).format("YYYYMMDDHHmm")); }).ToArray();

        
        //alert(x);
        $scope._selected_cell_id = x;
    ...
    $scope.$on('$viewContentLoaded', function () {
		...
		// if ($rootScope._load>1)
		// { $rootScope._load=0;  $window.location.reload();}
		 //if ($rootScope._load==1)
		 $scope.destroy_menu();
       $scope.create_menu();
