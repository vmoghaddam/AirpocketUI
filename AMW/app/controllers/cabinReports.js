'use strict';
app.controller('cabinReports', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', 'flightBagService', '$sce', 'neerjaService', '$window', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route, flightBagService, $sce, neerjaService, $window) {
    ...
    $scope.bindEval = function () {
        ...
        neerjaService.get_eval($scope.selectedFlight.FlightId, $scope.entity.crew).then(function (res) {
			var _ds=Enumerable.From(res.Data).Where(function(x){return ['ISCCM','SCCM','CCM'].indexOf(x.position)!=-1 && x.crew_id!=$scope.entity.crew;}).ToArray();
            $scope.loadingVisible = false;
            $scope.dg_eval_ds = _ds;
        });
    }
    ...
    $scope.dg_eval_columns = [
        ...
        { caption: 'نام', dataField: 'Name', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, minWidth:200 },
        { caption: 'موقیت', dataField: 'position', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 100 },
        ...
        { caption: 'نحوه انحام وظایف قبل و هنگام ورود مسافران', dataField: 'item_2', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 120 },
        { caption: 'کیفیت ارائه خدمات و جذب رضایت مسافر', dataField: 'item_3', allowResizing: true, alignment: 'center', dataType: 'number', allowEditing: false, width: 150 },
        ...
        { caption: 'مجموع نمرات', dataField: 'TotalScore', alignment: 'center', dataType: 'number', allowEditing: false, width: 100,fixed:true, fixedPosition:'right' },
    ...
    $scope.dg_flight_columns = [
        ...
        { dataField: 'Register', caption: 'Reg', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 80, sortIndex: 2, sortOrder: 'asc',fixed: true, fixedPosition: 'left' },
        { dataField: 'FromAirportIATA', caption: 'From', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 80, fixedPosition: 'left',fixed: true, fixedPosition: 'left' },
        { dataField: 'ToAirportIATA', caption: 'To', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 80, fixedPosition: 'left',fixed: true, fixedPosition: 'left' },
