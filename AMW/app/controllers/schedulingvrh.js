...
app.controller('schedulingvrhController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'schedulingService', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, $timeout, schedulingService, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
    ...
    $scope.getActionShow = function (grp, x) {
        ...
        if (['CCI', 'SCCM', 'CCM', 'CCI', 'CCE'].indexOf(grp) != -1) {
...
/////////request//////
 ...
 //2024-06-23
    ...
    $scope.dg_fp_columns = [
        ...
        { dataField: 'CCI', caption: 'CCI', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, fixed: false, fixedPosition: 'left', minWidth: 150 },
        { dataField: 'SCCM', caption: 'SCCM', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, fixed: false, fixedPosition: 'left', minWidth: 150 },
        { dataField: 'CCE', caption: 'CCE', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, fixed: false, fixedPosition: 'left', minWidth: 350 },
...
onValueChanged: function (arg) {
    ...
    $scope.posVisibleClick2 = function (pos) {
        ...
        if (pos !='CCM' && $scope.indexPos[pos] == 5)
            return;
    ...
    $scope.refreshPos = function () {
        ...
        $scope.posVisible.CCI1 = false;
        $scope.posVisible.CCI2 = false;
        $scope.posVisible.CCI3 = false;
        $scope.posVisible.CCI4 = false;
        $scope.posVisible.CCI5 = false;
    ...
    $scope.fillPos = function (keep) {
        //magu39
		console.log('       ___________ selectedFlightsKey',$scope.selectedFlightsKey);
        $scope.refreshPos();
...
console.log($scope.currentAssigned);
            //2021-1-4
...
//2024-01-20
            ...
            $.each(fdp.flights, function (_j, flt) {
                ...
                if (flight) {
                    ...
                    if (dh == 0) {
                        ...
                        if (fdp.rank == 'CCI' || fdp.rank == 'SCCM' || fdp.rank == 'CCM' || fdp.rank == 'CCE')
    ...
    $scope.fillFilteredCrew = function () {
        ...
        switch (id) {
            ...
            case 'CCI':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                    return x.JobGroup == 'CCI';
                }).ToArray();
                break;
            case 'SCCM':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                    return (x.JobGroup == 'SCCM' || x.JobGroup == 'CCI') && (x.ValidTypes && x.ValidTypes.indexOf($scope.ati_selectedTypes[0])!=-1 || alldh);
                }).ToArray();
                break;
            case 'CCE':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                    return (x.JobGroup == 'CCE') && (x.ValidTypes && x.ValidTypes.indexOf($scope.ati_selectedTypes[0])!=-1 || alldh) ;
                }).ToArray();
                break;
           case 'CCM':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
            ...
            case 'OBS':
                ...
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                    return ($scope.IsCabin && (x.JobGroup == 'CCM' || x.JobGroup == 'SCCM' || x.JobGroup == 'CCI')) 
