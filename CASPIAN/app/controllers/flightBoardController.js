'use strict';
app.controller('flightBoardController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', function ($scope, $location, $routeParams, $rootScope, $timeout, flightService, weatherService, aircraftService, authService, notificationService, $route) {
    ...
    $scope.IsCLAccess = $rootScope.HasMenuAccess('flight_board_jl', 3) || $rootScope.userName.toLowerCase().startsWith('trans.');
    $scope.IsJLOG = false;
    ...
    $scope.midnightLines = [];
    $scope.bindFlights = function (saveState,callback) {
        $scope.midnightLines = [];
        ...
        flightService.getFlightsGantt2(Config.CustomerId, (new Date($scope.datefrom)).toUTCDateTimeDigits(), ed, offset, /*($scope.IsAdmin ? null : $scope.airportEntity.Id)*/-1, ($scope.doUTC ? 1 : 0), filter).then(function (response) {
            try {
                ...
                for (i = 1; i <= $scope.days_count; i++) {
                    ...
                    //if (i > 1)
                    {
                        $scope.midnightLines.push({ day: (new Date(stdate.getFullYear(), stdate.getMonth(), stdate.getDate(), 23, 59, 0, 0)).yyyymmddtime($scope.IsUTC), lineWidth: "1", lineColor: "#000", lineStyle: "solid" });
                    }
    ...
    $scope.search = function (callback) {
        ...
        $scope.bindFlights(false,callback);
    };
    //$scope.btn_search = {
    //    text: '',
    //    type: 'success',
    ...
    $scope.StartUTimer = function () {
        ...
        $scope.utimer = setTimeout(function () {
            ...
            //chooko
            //////////////////////////
            var _df = General.getDayFirstHour(new Date($scope.datefrom));
            var _dt = General.getDayLastHour(new Date($scope.dateEnd));
            ///////
            var dto = {
                from: (new Date(/*$scope.datefrom*/_df)).toUTCString(),
                to: (new Date(/*$scope.dateto*/_dt)).toUTCString(),
            ...
            flightService.getUpdatedFlights(dto).then(function (response) {
                ...
                $scope.baseDate = (new Date(Date.now())).toUTCString();
                var ganttObj = $("#resourceGanttba").data("ejGantt");
                console.log('response.flights.length');
                console.log(response.flights.length);
                $.each(response.flights, function (_i, _d) {
                    ...
                    if (data) {
                        ...
                        //chook
                        $scope.currentScroll=($('.e-ganttviewerbodyContianer').data("ejScroller").scrollLeft());
    ...
    $scope.StartNowLineTimer = function (interval) {
        ...
        $scope.nowlinetimer = setTimeout(function () {
            ...
            if ($scope.StartNowLineTimerFirst) {
                ...
                var ganttObj = $("#resourceGanttba").data("ejGantt");
                //yyyymmddtimenow2(new Date(2020,6,19,0,0,0))
                var slines=[{ day: dtstr, lineWidth: "2", lineColor: "#ff0066", lineStyle: "solid" }
                        
                ];
                $.each($scope.midnightLines, function (_i, _d) {
                    slines.push(_d);
                })
                //, { day: (new Date(2020, 6, 18, 10, 0, 0, 0)).yyyymmddtime($scope.IsUTC), lineWidth: "2", lineColor: "#bbb", lineStyle: "solid" }
               ...
               // $('#stripline0').height($('#ganttviewerbodyContianerejGanttChartresourceGanttba').height());
                // $('#stripline1').height($('#ganttviewerbodyContianerejGanttChartresourceGanttba').height());
                $("[id^='stripline']").height($('#ganttviewerbodyContianerejGanttChartresourceGanttba').height());
    ...
    $scope.$watch("selectedTabDateIndex", function (newValue) {
            ...
            if ($scope.firstCreate) {
                ...
                $scope.scrollGanttFirst(new Date($scope.selectedDate));
    ...
    $scope.BeginSearch = function (callback) {
 ...
 },
 {
     widget: 'dxButton', location: 'after', options: {
        ...
        bindingOptions: {
            ...
            'toolbarItems[6].visible': 'IsEditable',
    ...
    $scope.createGantt = function (_scale) {
        try {
            ...
            var _dt = General.getDayLastHour(new Date($scope.selectedDate));
             ...
             _df = General.getDayFirstHour(new Date($scope.datefrom));
               _dt = General.getDayLastHour(new Date($scope.dateEnd));
            ...
            $("#resourceGanttba").ejGantt({
                ...
                create: function (args) {
                    try {
                        ...
                        if ($scope.currentScroll) {
                            $('.e-ganttviewerbodyContianer').data("ejScroller").scrollX($scope.currentScroll);
                        }
                        else {
                           // var _nowt = new Date();
                          //  if (_nowt >= _df && _nowt <= _dt)
                            //    $scope.scrollGanttToday();
                            $scope.scrollGanttFirst(_df);
                        }
                        //$('.e-ganttviewerbodyContianer').data("ejScroller").wheelMove = function (args) { alert('x'); };
                        ...
                        if (nw.getTime() >= nf.getTime() && nw.getTime() <= nt.getTime()) {
						 ...
						 //doolrahm
						   console.log('last std last std');
						  console.log($scope.lastSTD);
                        if ($scope.lastSTD) {
                          
                            setTimeout(
							
                                    function () {
										console.log('scroll x 2');
                                        //$scope.scrollGantt2($scope.lastSTD);
                                        $scope.scrollGanttX($scope.lastScroll);
                                        $scope.lastSTD = null;
                            }, 500);
 ...
 //7-29
   ...
   $scope.popup_jlog_visible = false;
    ...
    $scope.popup_jlog = {
        ...
        onShown: function (e) {
            $scope.calculatejlog();
            $scope.dg_fuelflt_ds = Enumerable.From($scope.dataSource)
