...
app.controller('reportEFBController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', 'flightBagService', '$sce', function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route, flightBagService, $sce) {
    $scope.prms = $routeParams.prms;
	  var _fbexist = Enumerable.From($rootScope.claims).Where('$.page=="' + 'efb_asr_vr' + '"').FirstOrDefault();
    ...
    $scope.isOPSStaff = $rootScope.userName.toLowerCase().includes("ops.shakoory");  //ops.shakoory;
	...
	$scope.scroll_atc_height = $(window).height() - 100;
	    $scope.scroll_atc = {
        width: '100%',
        bounceEnabled: false,
        showScrollbar: 'never',
 ...
 height: 'scroll_atc_height',
	  ...
	  $scope.btn_fpl = {
        ...
        onClick: function (e) {
            ...
            if (flt.DepartureRemark) {
				//alert(flt.DepartureRemark);
				//return;
				 
				if (!flt.DepartureRemark.includes('atc-')) {
					 
					 $scope.ATCHtml = flt.DepartureRemark; // $sce.trustAsHtml($scope.selectedFlight.ATCPlan);
				...
				else
				{
			         ...
			         var _url = 'https://fleet.caspianairlines.com/apiatc/upload/' + flt.DepartureRemark.replace('"', '').replace('"', '').replace("'", "").replace("'", "");
                      window.open(_url);
				}
            }
	...
	function printElem($elem,ofp) {
        ...
        var sobprops = ['prop_pax_adult', 'prop_pax_child', 'prop_pax_infant', crewAId, crewBId, crewCId];
            ...
            if (_d.PropValue){
				...
				console.log( $('#_iframe').contents().find('#'+_d.PropName).length);
			}
...
$scope.stations = [];
    ...
    $scope.bindStations = function () {
		...
		} catch(eee){alert(eee); console.log(eee);}
    ...
    $scope.dg_flight = {
        ...
        onRowPrepared: function (e) {
            ...
            if (!$scope.isOPSStaff && e.rowType == 'data' && e.data && (e.data.AttASR == 1 || e.data.AttVoyageReport == 1)) {
              ...
              if (e.rowType == 'data' && e.data &&  e.data.CommanderNote  ) {
