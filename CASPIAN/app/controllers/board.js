'use strict';
app.controller('boardController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route','$window','$http', function ($scope, $location, $routeParams, $rootScope, $timeout, flightService, weatherService, aircraftService, authService, notificationService, $route,$window,$http) {
   ...
   //ops.rezabandehlou
	...
	|| $rootScope.userName.toLowerCase().startsWith('it.entezari');
	...
	//console.log('---------REMARK---------------------');
	//console.log($scope.IsRemark);
	//console.log($scope.IsSave);
	//console.log($rootScope.userName);
    //alert((new Date()).yyyymmddtime(false));
	//divargar
    $scope.IsComm = $rootScope.userName.toLowerCase().startsWith('comm.') || $rootScope.userName.toLowerCase()=='demo';
    //  return;
    $scope.btn_filter = {
        text: '',
        type: 'default',
    ...
    ////////////////////////////////////////////
    $scope.getRealMSNs_x = function (cid) {
        return new DevExpress.data.DataSource({
            store:
        ...
        if ($scope.linkEntity && $scope.linkEntity.FromAirportId && $scope.linkEntity.ToAirportId) {
            ...
            flightService.getRoute($scope.linkEntity.FromAirportId, $scope.linkEntity.ToAirportId).then(function (response) {
                ...
                if (response) {
					var nr=false;
					try{
						var _df=Number( moment($scope._datefrom).format('YYYYMMDD'));
						nr=_df>=20221030;
					}
					catch(eeee){
					...
					//$scope._datefrom
					if (nr){
						var rec= Enumerable.From($scope.newRoutes).Where('$.FromId=='+$scope.linkEntity.FromAirportId+' && $.ToId=='+$scope.linkEntity.ToAirportId).FirstOrDefault();
						if (rec){
								  $scope.linkEntity.FlightH = rec.FlightH;
                                  $scope.linkEntity.FlightM = rec.FlightM;
						} else{
    ...
    $scope.btn_free = {
        ...
        onClick: function (e) {
			...
			console.log('api/xls/sec?df=' +  moment(new Date($scope.selectedDate)).format('YYYY-MM-DD'));
                    $window.open('https://fleet.caspianairlines.com/apisec/api/xls/sec?flts=' +$scope.ati_selectedFlights[0].ID, '_blank');
        }
    ...
    $scope.btn_edit = {
        ...
        onClick: function (e) {
            ...
            if ($scope.ati_flight.FlightPlanId  && 1==2 ) {
				...
				//04-30
                $scope.linkEntity.STD = new Date($scope.ati_flight.STD);
...
$scope.btn_addemp = {
    ...
    $scope.btn_creg = {
        ...
        onClick: function (e) {
            ...
            if ($scope.IsComm) {
				...
				//if ((new Date( ati_flight1.STD)).getDay()!=(new Date( ati_flight2.STD)).getDay())
    ...
    var getOffset=function(dt){
		//console.log('ooffssseeet');
        var _oof=(new Date(dt)).getTimezoneOffset();
		//if ( moment(new Date(dt)).format('YYYYMMDD')=='20220922' || moment(new Date(dt)).format('YYYYMMDD')=='20220921')
	...
	//2023
	var _nodstdate=20230321;//new Date(2023,2,22,0,0,0);
     var getOffsetDate = function (dt, m, n) {
        if (!n)
            n = 0;
        var diff = m * getOffset(dt);
        diff = diff + n;
		if (Number(moment( new Date(dt)).format('YYYYMMDD')) >=_nodstdate)
			diff=210;
                    ...
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'fbcreg', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        if ($scope.IsComm) {
                            ...
                            flightService.saveFlightRegisterChangeGroup(entity).then(function (response) {
                                ...
                                $http.post(serviceBaseV2 + 'odata/flight/register/change/notify', notifyObj);
 ...
 height:'cregHeight',
    ...
    $scope.fillLog = function (entity) {
        if ($scope.timeBase=='UTC')
            $scope.convertToLCL();
		 var n = 0;
        if ($scope.logFlight.ID == 72365 || $scope.logFlight.ID == 72367)
            n = -60;
        if ($scope.logFlight.ChocksOut)
                ...
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'deplog', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        flightService.saveFlightLog(dto).then(function (response) {
							...
							flightService.sendCaoMVT($scope.ati_flight.ID);
							flightService.sendDelayNira($scope.ati_flight.ID);
                            $scope.popup_departure_visible = false;
                       ...
                       type: 'default', text: 'Crew List', icon: '', onClick: function (e) {
                           ...
                           flightService.getFlightsLine($scope.flight.ID).then(function (response) {
                               ...
                               $.each(response, function (_i, _d) {
                                   ...
                                   if ($scope.IsPickup && $rootScope.userName.toLowerCase() != 'aps.kouhsar')
               ...
               },
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'fbaclog', bindingOptions: { disabled: 'IsApproved' }, visible: true, onClick: function (arg) {
                      ...
                      //  dto.ArrivalRemark=($scope.p_in?$scope.p_in:'')+'_'+($scope.p_out1?$scope.p_out1:'')+'_'
						//        +($scope.p_out2?$scope.p_out2:'');
                        ...
                        flightService.saveFlightLog(dto).then(function (response) {
                            ...
                            $scope.sms_cancel = false;
                            $scope.sms_nira = false;
							//console.log('MVT     ',$scope.ati_flight);
							if ($scope.ati_flight.FromAirportIATA=='ISU' || $scope.ati_flight.ToAirportIATA=='ISU'){
								  flightService.sendMVTAPT($scope.ati_flight.ID, dto.UserName).then(function (response) {
...
flightService.sendDelayNira($scope.ati_flight.ID);
        ...
        onShowing: function (e) {
            ...
            $scope.depReadOnly = false;
             console.log('arr rem',$scope.logFlight.ArrivalRemark);
			/* if ($scope.logFlight.ArrivalRemark){
				 var prts=$scope.logFlight.ArrivalRemark.split('_');
				 $scope.p_in=prts[0];
				 $scope.p_out1=prts[1];
				 $scope.p_out2=prts[2];
			 }*/
        ...
        onShown: function (e) {
            ...
            flightService.getFlightDelays($scope.logFlight.ID).then(function (response) {
				...
				var churl = 'https://fleet.caspianairlines.com/xapi/api/nira/chrs?flt=' + $scope.flight.FlightNumber+'&dt=' + moment(new Date($scope.flight.Date)).format("YYYY-MM-DD");
                $http.get(churl).then(function (response) {
                    console.log(response);
                    try {
                        $scope.nira_charterers = response.data.NRSCharterOfficesCapacity;
                        var _chsum = Enumerable.From($scope.nira_charterers).Sum('Number($.Book)');
                        
                        if (_chsum)
                            $scope.nira_charterers.push({ OfficeCode: '', Capacity: 'Sum:', Book: _chsum });
                    }
                    catch (xee) {
    ...
    $scope.getCrewAbs = function (fid) {
       ...
       //$scope.loadingVisible = true;
        flightService.getFlightCrews(fid, archived_crews).then(function (response) {
			...
			if (!$scope.IsACM){
				response=Enumerable.From(response).Where('$.JobGroupCode!="005"').ToArray();
 ...
 /////////////////////////
                ...
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Save', icon: 'check', validationGroup: 'jlog', bindingOptions: { disabled: 'IsApproved' }, onClick: function (arg) {
                        ...
                        var dto = {
                            ...
                            PFLR: !$scope.jlogEntity.PFLR ? -1 :$scope.getPFLRId( $scope.jlogEntity.PFLR),
       ...
       // console.log(flt);
        ...
        if ($scope.timeType == 2) {
            var offset = getOffset(new Date(dt.getFullYear(), dt.getMonth(), dt.getDate(), 1, 0, 0, 0));
			 ...
			 if (flt.ID==84948)
                console.log('dep: ' +flt.STD+'  ' + offset + '   ' + flt.GWLand);
	...
	$scope.getDep = function (flt) {
       // console.log(flt);
        ...
        if ($scope.timeType == 1) {
			...
			var  _xdt=Number(moment(new Date(flt.STD)).format('YYYYMMDD'))  ;
			if (_xdt>=20220922 && _xdt<=20221029 && (flt.FromAirportIATA=='NJF' || flt.FromAirportIATA=='IST' || flt.ToAirportIATA=='NJF' || flt.ToAirportIATA=='IST')){
			//n=-60;
		     // offset = offset - 60;
		    }
			if (flt.ID == 72365 || flt.ID == 72367) {
    ...
    $scope.getArr = function (flt) {
        ...
        if ($scope.timeType == 1) {
			...
			if (flt.ID==94243 || flt.ID==94255 || flt.ID==94221){
				offset=offset-60;
        ...
        if ($scope.timeType == 2) {
            ...
            var offset = getOffset(new Date(dt.getFullYear(), dt.getMonth(), dt.getDate(), 1, 0, 0, 0));
			 //if (flt.FlightNumber == '7900' || flt.FlightNumber == '0038')
    ...
    $scope.getDelayText = function (flt) {
        ...
        return duration && duration != 0 ? Number(duration).toFixed(0) : "";
    };
    ...
    $scope.IsConflict_x = function (flt, x) {
            ...
            try {
                ...
                $.each(response.resources, function (_i, _d) {
                    ...
                    $.each(flights, function (_j, _q) {
                        ...
                        _q.GWLand=210;
						_q.GWTO=210;
						var _nstd=Number(moment(new Date(_q.STD)).format('YYYYMMDD'));
						if (_nstd>=20220322)
						{
						...
						if (_q.FromAirportIATA=='NJF' || _q.FromAirportIATA=='BSR' || _q.FromAirportIATA=='IST' || _q.FromAirportIATA=='ESB')
							_q.GWLand=180;
						if (_q.ToAirportIATA=='NJF' || _q.ToAirportIATA=='BSR' || _q.ToAirportIATA=='IST' || _q.ToAirportIATA=='ESB')
							_q.GWTO=180;
						...
						if (_q.FromAirportIATA=='EVN'  )
							_q.GWLand=240;
						if (_q.ToAirportIATA=='EVN' ) 
						{_q.GWTO=240; console.log('EVN2');}
						
						
						
						if (_q.FromAirportIATA=='ADB'  )
							_q.GWLand=180;
    ...
    $scope.IsNiraVisible = $rootScope.userName.toLowerCase().startsWith('comm.') || $rootScope.userName.toLowerCase().startsWith('sale.')
        ...
        || $rootScope.userName.toLowerCase().includes('razbani')
		 || $rootScope.userName.toLowerCase().startsWith('nira')
		  || $rootScope.userName.toLowerCase().startsWith('it');
    $scope.btn_nira = {
        text: 'NIRA',
        //hint: 'Airport Weekly Report',
        type: 'default',
 ...
 searchExpr: ["IATA", "Country", "SortName", "City"],
        ...
        bindingOptions: {
            value: 'aptrptday',
            //2020-11-16
          //  dataSource: 'ds_aptrptday',

        }
    };
	
	
	$scope.popup_aptday_visible = false;
	
	 
	
	
	    $scope.popup_aptday_title = 'Daily Report';
    $scope.popup_aptday = {
        elementAttr: {
            //  id: "elementId",
            class: "popup_aptday"
        ...
        toolbarItems: [
            {
                widget: 'dxButton', location: 'after', options: {
                    type: 'success', text: 'Export',   validationGroup: 'fbaptrptday', onClick: function (arg) {
        ...
        onHiding: function () {
            ...
            $scope.popup_aptrptday_visible = false;

        },
        bindingOptions: {
            visible: 'popup_aptday_visible',

            title: 'popup_aptday_title',

        }
    };

    //close button
    $scope.popup_aptday.toolbarItems[1].options.onClick = function (e) {
    ...
    $scope.uploader_atc = {
        ...
        rtlEnabled: true,
        uploadUrl: "https://fleet.caspianairlines.com/apiatc/api/upload/atc/flightplan",
        onValueChanged: function (arg) {
    ...
    $scope.btn_atcdnl = {
        ...
        text: 'Download',
        onClick: function (e) {
            $window.open('https://fleet.caspianairlines.com/apiatc/upload/' + $scope.atcTXT, '_blank');
        },
        bindingOptions: {
            visible:'isATCFile'
        }
    ...
    $scope.uploaderValueATL = [];
    $scope.uploadedATL = null;
    $scope.uploader_atl = {
        //uploadedMessage: 'بارگزاری شد',
        multiple: false,
        selectButtonText: 'Upload ATL',
        // selectButtonText: 'انتخاب تصویر',
        labelText: '',
        accept: "*",
        uploadMethod: 'POST',
        uploadMode: "instantly",
        rtlEnabled: true,
        uploadUrl: "https://fleet.caspianairlines.com/apiatc/api/upload/atl",
        onValueChanged: function (arg) {

        },

        onUploaded: function (e) {
            $scope.uploadedATL = e.request.responseText;
            console.log('upload atl', e.request.responseText);
            if (e.request.responseText) {
                $scope.loadingVisible = true;
                flightService.updateATL($scope.ati_selectedFlights[0].ID, e.request.responseText.replace('"', '').replace("'", "").replace('.', 'X')).then(function (response) {
                    $scope.loadingVisible = false;
                    alert("ATL Uploaded.")
                    e.component.reset();
                }, function (err) { $scope.loadingVisible = false; General.ShowNotify(err.message, 'error'); });
