...
app.controller('delaykpiController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'biService', 'aircraftService', 'authService', 'notificationService', '$route', function ($scope, $location, $routeParams, $rootScope, flightService, biService, aircraftService, authService, notificationService, $route) {
    ...
    $scope.fuelPivotGridOptions = {
        ...
        dataSource: {
            ...
            store: DevExpress.data.AspNet.createStore({
                key: "ID",
                loadUrl: serviceBaseBi + "odata/pivot/delay/total",
    ...
    $scope.ds_yearstr = ['1398', '1399', '1400','1401','1402'];
    ...
    ////////////////////////////////
    if (!authService.isAuthorized() && 1==2) {
    ...
    $scope.pdate = null;
    $scope.dobind = false;
    $scope.ds_year = [1398, 1399, 1400,1401,1402];
    $scope.ds_month = [
        { title: 'فروردین', id: 1 },
        { title: 'اردیبهشت', id: 2 },
        { title: 'خرداد', id: 3 },
    ...
    $scope.fillDsYearMonth = function () {
        ...
        switch ($scope.year) {
			case 1402:
                ...
                $scope.ds_yearmonth = Enumerable.From($scope.ds_month)/*.Where('$.id==1 || $.id==2 || $.id== ')*/.OrderBy('$.id').ToArray();
                break;
			 case 1401:
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        $scope.nowMonth = $scope.pdate.month();
            //alert( $scope.nowMonth);
        $scope.year = 1402;// $scope.pdate.year();
         $scope.month = 9;
        setTimeout(function () {
            $scope.dobind = true;
            $scope.bindmain();
