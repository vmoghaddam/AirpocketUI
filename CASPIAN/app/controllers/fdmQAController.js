'use strict';
app.controller('fdmQAController', ['$http', '$scope', '$routeParams', '$rootScope', 'fdmService', 'authService', 'notificationService', function ($http, $scope, $routeParams, $rootScope, fdmService, authService) {
    ...
    $scope.btn_confirm = {
        ...
        width: 120,
        onClick: function () {
            if ($scope.selectedTabId == "boeing") {
                fdmService.lockBoeingFDM($scope.year, $scope.month + 1).then(function (response) {

                    if (response.IsSuccess == false)
                        General.ShowNotify(Config.Text_UnableToLock, 'error');
                    else {
                        General.ShowNotify(Config.Text_Locked, 'error');
                    }
                    if ($scope.dg_boeing_instance)
                        $scope.dg_boeing_instance.refresh();
                    if ($scope.dg_md_instance)
                ...
                fdmService.lockMDFDM($scope.year, $scope.month + 1).then(function (response) {
    ...
    $scope.btn_save = {
        ...
        onClick: function () {
            ...
            fdmService.makeVisible($scope.year, $scope.month + 1).then(function (response) {
    ...
    $scope.btn_close = {
        ...
        type: 'danger',
        onClick: function () {
            $scope.popup_menu_visible = false
        }
    }
    /////////////////////////////////////////
    ...
    $scope.bind = function () {
        ...
        fdmService.getFdm($scope.year, $scope.month + 1).then(function (response) {
            $scope.loadingVisible = false;
            $scope.dg_boeing_ds = response.Data.Boeing;
            $scope.dg_md_ds = response.Data.MD;
            response.Data.Boeing.forEach(function (e) {
                $scope.ds_validity.push(e.Validity);
            });

            response.Data.MD.forEach(function (e) {
                $scope.ds_validity.push(e.Validity);
            });
    ...
    $scope.dg_boeing = {
        ...
        onEditingStart(arg) {
            ...
            if (arg.key.Confirmation == 1 || arg.key.Validity != 2) {
                arg.cancel = true;
            }
        },
    ...
    $scope.acceptEvent = function (row) {
        General.Confirm(Config.Text_DeleteConfirm, function (res) {
            if (res == true) {
                fdmService.DeleteEvent(row.data.Id).then(function (response) {

                })

                $scope.dg_removed_ds = Enumerable.From($scope.dg_removed_ds).Where(function (x) {
                    return x.Id != row.data.Id;
                }).ToArray();

                $scope.dg_removed_instance.refresh();
            }

        });
    }

    $scope.rejectEvent = function (row) {
        General.Confirm(Config.Text_DeleteConfirm, function (res) {
            if (res == true) {

                fdmService.confirmFDM(row.data.Id).then(function (response) {
                    $scope.dg_confirmed_ds.push(response.data);
                    console.log(response);
                })
