...
app.controller('delayReportFlightController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', '$window',function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route,$window) {
    ...
    $scope.btn_summary = {
        ...
        bindingOptions: {},
        onClick: function (e) {
          $scope.popup_summary_visible = true;
        }
    ...
    $scope.popup_date = {
     ...
     $scope.popup_summary_visible = false;
    $scope.popup_summary_title = 'Summary';
    $scope.popup_summary = {
        title: 'Summary',
        shading: true,
        height: 200,
        ...
        onShown: function (e) {
            ...
            if ($scope.bar_station_instance)  $scope.bar_station_instance.render();
    if ($scope.bar_sum_date_instance) $scope.bar_sum_date_instance.render();
    ...
    $scope.getChartBoxStyle = function () {
	 ...
	 $scope.bar_station_instance = null;
    $scope.bar_station = {
        "export": {
            enabled: true
        },
        onInitialized: function (e) {
            if (!$scope.bar_station_instance)
                $scope.bar_station_instance = e.component;
        },
        palette: "Dark Violet",
        title: {
            text: "Delay Per Station",
            font: {
                size: 20,
            }
        ...
        commonSeriesSettings: {
            type: "bar",
            valueField: "total_delay",
            argumentField: "station",
        ...
        bindingOptions: {
            "dataSource": "total_station_ds",
        }
    };



	 $scope.bar_sum_date_instance = null;
	$scope.bar_sum_date = {
        "export": {
            enabled: true
        },
        onInitialized: function (e) {
             if (!$scope.bar_sum_date_instance)
                $scope.bar_sum_date_instance = e.component;
        },
        palette: "Ocean",
        title: {
            text: "Total Delay By Date",
            font: {
                size: 20,
            }
        ...
        series: [
           { valueField: "total_delay", name: "Delay/Date", type:'line' },
    ...
    label: {
      customizeText: function (arg) {
        return $scope.formatDateYYYYMMDD(arg.value);
      },
      overlappingBehavior: "rotate",
      rotationAngle: 90,
      font: { size: 14 }
    ...
    $scope.bind = function () {
		...
		flightService.getDelayedFlightsSummary(_df, _dt,_reg,'',_flts,_route,$scope.range).then(function (response) {
            $scope.loadingVisible = false;

           
            $scope.total_date_ds = response.total_date;
            $scope.total_station_ds = response.total_station;
