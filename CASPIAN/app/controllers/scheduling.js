...
app.controller('schedulingController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'schedulingService', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, $timeout, schedulingService, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
    ...
    if ($rootScope.userName.toLowerCase() == 'train.moradi' || $rootScope.userName.toLowerCase() == 'mohammadifard' || $rootScope.userName.toLowerCase() == 'train.barzegar' || $scope.isRoster)
  ...
  $scope.Operator=$rootScope.CustomerName.toUpperCase(); 
    ...
    $scope.popup_cduties = {
        ...
        bindingOptions: {
			...
			'toolbarItems[0].visible':'!isLimited',
'toolbarItems[1].visible':'!isLimited'
   ...
   $scope.popup_fx = {
    ...
    $scope.addStby = function (_crew, _type) {
		...
		if (dto.type==1170){
		  schedulingService.saveRES(dto).then(function (response) {
            $scope.loadingVisible = false;
    ...
    $scope.dg_calcrew_columns = [
        // { dataField: 'Selected', caption: '', allowResizing: true, alignment: 'center', dataType: 'boolean', allowEditing: false, width:45},
        { dataField: 'ScheduleName', caption: 'Schedule Name', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false, sortIndex: 1, sortOrder: 'asc' },
        { dataField: 'JobGroup', caption: 'Group', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 100 },
        { dataField: 'GroupOrder', caption: 'O', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 120, visible: false, sortIndex: 0, sortOrder: 'asc' },
        {
            dataField: "Id", caption: '',
            width: 60,
    ...
    $scope.cellContextMenuItems = [
      ...
      //  { visible: !$scope.OnlyRoster, text: 'STBY Other Airline', onItemClick: $scope.assign300010, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-1167' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>STBY Other Airline</td></tr></table>", },
        ...
        { visible: $scope.OnlyTraining,text: 'Simulator', onItemClick: $scope.assign100003, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-100003' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>Simulator</td></tr></table>", },
        ...
        { visible: !$scope.OnlyRoster, visible: !$scope.OnlyRoster || $scope.IsTRN, text: 'Expired Medical', onItemClick: $scope.assign100005, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-100005' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>Expired Medical</td></tr></table>", },
        { visible: !$scope.OnlyRoster || $scope.IsTRN, text: 'Expired Licence/Training', onItemClick: $scope.assign100004, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-100004' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>Expired Licence/Training</td></tr></table>", },
        { visible: !$scope.OnlyRoster || $scope.IsTRN, text: 'Expired Passport', onItemClick: $scope.assign100006, template: "<table><tr><td style='vertical-align:middle;'><div class='duty-100006' style='width:15px;height:15px;border-radius:50%;'></div></td><td style='vertical-align:top;padding-left:5px;'>Expired Passport</td></tr></table>", },
    ...
    $scope.notifyDutyCal = function (duty) {
        ...
        //    return;
       // }
	    if ($scope.OnlyTraining && [5000,100003].indexOf( type)==-1)
		 {
			 General.ShowNotify('You dont have enough permission to notify this type of events.', 'error');
            return;
		 }
    ...
    $scope.removeDutyCal = function (duty) {
        var type = duty.DutyType;
        if (!$scope.OnlyTraining && [100003,5000].indexOf(type) != -1) {
   ...
   /** $scope.formatCellDateDay = function (dt) {
    ...
    $scope.fillFlightCrews = function () {
        ...
        $.each($scope.ati_flights, function (_i, _d) {
            ...
            _d.hasCabinExtra = _d.SCCM > 1 || _d.CCM > 4;
                ...
                if (crew.JobGroupCode.startsWith('00101')) {
					...
					{  
				      if ($scope.selectedPos.rank.toLowerCase()!='check' 
					   //&& crew.Id!=3661 && crew.Id!=3754 && crew.Id!=3758 && crew.Id!=3756 && crew.Id!=3757 && crew.Id!=3772 && crew.Id!=3760 && crew.Id!=3771 && crew.Id!=3759  && crew.Id!=3788
				      // && crew.Id!=3809 && crew.Id!=3805 && crew.Id!=3806 && crew.Id!=3807
					     && crew.Code=='0'
				      )
                        expired.push('LINE ' + (crew.LineExpired ? moment(crew.LineExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
					}

                    if (!crew.RemainColdWx || crew.RemainColdWx < 0 && crew.Id!=3788)
                    ...
                    //2023
					 if (!crew.RemainCCRM || crew.RemainCCRM < 0)
					  ...
					  if (!crew.RemainFMT || crew.RemainFMT < 0)
                          expired.push('FMT ' + (crew.TRIExpired ? moment(crew.TRIExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
					  if (!crew.RemainGRT || crew.RemainGRT < 0)
                          expired.push('GRT ' + (crew.TREExpired ? moment(crew.TREExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
					
                    //ColdWXExpired
                }
                else {
                    if (!crew.RemainRecurrent || crew.RemainRecurrent < 0)
                        expired.push('RECURRENT ' + (crew.RecurrentExpired ? moment(crew.RecurrentExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
					// if ($scope.selectedPos.rank != 'OBS')
				    
				     var _sel_ids=Enumerable.From($scope.ati_selectedFlights).Select('Number($.Id)').ToArray();
					 var _sel_types=Enumerable.From($scope.ati_flights).Where(function(x){return _sel_ids.indexOf(Number(x.ID)) !=-1;}).Select('$.OAircraftType[0]').Distinct().ToArray();
					 
					 if (_sel_types.length>1){
					    if ((!crew.Remain737 || crew.Remain737< 0) && $scope.selectedPos.rank != 'OBS' && $scope.selectedPos.rank != 'CHECK') 
                        expired.push('LINE 737 ' + (crew.LineExpired ? moment(crew.LineExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
					
					 if ((!crew.RemainMD || crew.RemainMD < 0) && $scope.selectedPos.rank != 'OBS' && $scope.selectedPos.rank != 'CHECK')
                        expired.push('LINE MD ' + (crew.LPRExpired ? moment(crew.LPRExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
					 }
					 else  if (_sel_types[0]=='M'){
						  if ((!crew.RemainMD || crew.RemainMD < 0) && $scope.selectedPos.rank != 'OBS' && $scope.selectedPos.rank != 'CHECK')
                        expired.push('LINE MD ' + (crew.LPRExpired ? moment(crew.LPRExpired).format('YYYY-MM-DD') : 'UNKNOWN'));
					 }
					 else {
                 ...
                 console.log('xxxxxxxxx',_sel_types);
				  console.log('xeeeexxxxxxxx',$scope.ati_flights);
                /////////////////////////
                if (expired.length > 0) {
                    var myDialog = DevExpress.ui.dialog.custom({
                        rtlEnabled: true,
    ...
    $scope.fillFilteredCrew = function () {
        ...
        switch (id) {
            ...
            case 'OBS':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
                    return x.JobGroup == 'CCM' || x.JobGroup == 'P2' || x.JobGroup == 'P1' || x.JobGroup == 'TRE' || x.JobGroup == 'TRI' || x.JobGroup == 'LTC' || x.JobGroup == 'ISCCM' || x.JobGroup == 'SCCM';
                }).ToArray();
                break;
            case 'CHECK':
                _ds = Enumerable.From($scope.ds_crew).Where(function (x) {
        ...
        $.each(filtered, function (_i, _d) {
			...
			_d.RemainFMT = !_d.TRIExpired ? null : $scope.getDaysDiff(dateFirst, new Date(_d.TRIExpired));
			_d.RemainGRT = !_d.TREExpired ? null : $scope.getDaysDiff(dateFirst, new Date(_d.TREExpired));
            ...
            _d.RemainRecurrent = !_d.RecurrentExpired ? null : $scope.getDaysDiff(dateFirst, new Date(_d.RecurrentExpired));
			 ...
			 _d.Remain737 = !_d.LineExpired ? null : $scope.getDaysDiff(dateFirst, new Date(_d.LineExpired));
			 _d.RemainMD = !_d.LPRExpired ? null : $scope.getDaysDiff(dateFirst, new Date(_d.LPRExpired));
