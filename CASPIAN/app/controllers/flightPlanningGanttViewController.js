'use strict';
app.controller('flightPlanningGanttViewController', ['$scope', '$location', 'flightService', 'aircraftService', 'authService', '$routeParams', '$rootScope', function ($scope, $location, flightService, aircraftService, authService, $routeParams, $rootScope) {
    ...
    $scope.gapoverlapsn =
        $scope.popup_add = {
            ...
            toolbarItems: [
                ...
                {
                    widget: 'dxButton', location: 'after', options: {visible:false,
    ...
    $scope.bindDto = function () {
        ...
        aircraftService.getAircraftTypes(Config.CustomerId).then(function (response) {
            ...
            aircraftService.getMSNs(Config.CustomerId).then(function (response) {
                ...
                $scope.msns = response;
                flightService.getPlanItemsGantt($scope.plansDto).then(function (response) {
                    $scope.loadingVisible = false;
    ...
    $scope.IsBase = false;
    $scope.plansDto = null;
    $scope.$on('InitFlightPlanningGanttView', function (event, prms) {
        ...
        $scope.IsEditView = true;
        if ($scope.IsBase)
            $scope.planId = prms.FlightPlanId;
        else {
            var offset = -1 * (new Date()).getTimezoneOffset();
            $scope.plansDto = {
                CustomerId: Config.CustomerId,
                Date: new Date(prms.DateFrom).ToUTC(),
                Offset: offset,
                Design:true,
            };
        }
    ...
    $scope.$on('InitFlightPlanningGanttViewBase', function (event, prms) {
