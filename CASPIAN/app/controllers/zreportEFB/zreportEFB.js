    ...
    $scope.jl = {asr:false,vr:false,pos1:false,pos2:false,sign:''};
	 $scope.bindJL = function (fid) {
        ...
        flightBagService.getJL(fid).then(function (response) {
            ...
            //    _d.u						rl_sign = signFiles + _d.PICId + ".jpg";
            //    _d.PIC2 = _d.PIC;
           ...
           $scope._PIC = response.legs[0].JLSignedBy;
						$scope._SgnCPTLicNo = response.legs[0].Airline;
                        $scope._signDate = moment(new Date(response.legs[0].JLDatePICApproved)).format('YYYY-MM-DD HH:mm');
		   
            //response.legs[i]
            //$scope.jl.ReportingTime
			if (response.legs[0].ToAirportIATA.startsWith('OI'))
			{ $scope.jl_etd='1:00'; var rp=-60;}
				else 
				{$scope.jl_etd='1:30'; var rp=-90;}
			
            $scope.jl.StartTime = (CreateDate(response.legs[0].STD)).addMinutes(rp);
			//alert(rp);
			//alert($scope.jl.StartTime);
			...
			$scope.PIC = $scope._PIC ;
						$scope.SgnCPTLicNo = $scope._SgnCPTLicNo ;
                        $scope.signDate = $scope._signDate ;
			}
