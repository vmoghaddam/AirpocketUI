...
app.controller('delayReportController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'aircraftService', 'authService', 'notificationService', '$route', '$window',function ($scope, $location, $routeParams, $rootScope, flightService, aircraftService, authService, notificationService, $route,$window) {
    ...
    $scope.isprintvisible = $rootScope.userName.toLowerCase().startsWith('comm.daliriyan') || $rootScope.userName.toLowerCase().startsWith('demo');
 ...
 $scope.btn_airport = {
    ...
    $scope.btn_print = {
        ...
        onClick: function (e) {
            ...
            if ($scope.range)
                range = $scope.range;
            $window.open( "https://fleet.caspianairlines.com/reportdelayint/frmreportview.aspx" + '?type=3&p=' + days + '&df=' + _dfrom+'&dt='+_dto+'&cats='+cts+'&range='+range, '_blank');
	  ...
	  $scope.btn_print2 = {
        ...
        onClick: function (e) {
            ...
            if ($scope.range)
                range = $scope.range;
            $window.open( "https://fleet.caspianairlines.com/reportdelayint/frmreportview.aspx" + '?type=30&p=' + days + '&df=' + _dfrom+'&dt='+_dto+'&cats='+cts+'&range='+range, '_blank');
            //flightService.getDelayReportPeriodic(_dfrom, _dto, 5).then(function (response) {
            //    $scope.loadingVisible = false;
            //    $scope.total = response;
            //}, function (err) { $scope.loadingVisible = false; General.ShowNotify(err.message, 'error'); });
